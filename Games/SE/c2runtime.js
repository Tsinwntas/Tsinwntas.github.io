// Generated by Construct 2, the HTML5 game and app creator :: http://www.scirra.com
'use strict';
var ba, ca, da, x, ea, fa, ga, J, ha, ia, ja, ka, ma, na, N, oa, pa, Q, qa, ra, sa, va, S, wa, ya, za, Aa, Ba, T, Ca, Da, Ea, Fa, Ga, Ia, Ja, Ka, La, Ma, Na, Oa, Pa, Qa, Ra, Sa, Ta, Ua, Va, Wa, Xa, Za, $a, ab, bb, cb, db, eb, fb, gb, hb, ib, jb, kb, lb, mb, nb, ob, pb, qb, rb, sb, tb, ub, vb, wb, xb, yb, zb, Ab, Bb, Cb, Db, Eb, Fb, Gb, Hb, Ib, Jb, Kb, Lb, Mb, Nb, Ob, Pb, Qb, Rb, W, Sb, Tb = {},
    Ub = {};
"function" !== typeof Object.getPrototypeOf && (Object.getPrototypeOf = "object" === typeof "test".__proto__ ? function(c) {
    return c.__proto__
} : function(c) {
    return c.constructor.prototype
});
(function() {
    function c(a, e, f, d) {
        this.set(a, e, f, d)
    }

    function l() {
        this.ab = this.$a = this.fb = this.eb = this.lb = this.kb = this.Na = this.Ma = 0
    }

    function h(a, e, f, d) {
        a < e ? f < d ? (y = a < f ? a : f, F = e > d ? e : d) : (y = a < d ? a : d, F = e > f ? e : f) : f < d ? (y = e < f ? e : f, F = a > d ? a : d) : (y = e < d ? e : d, F = a > f ? a : f)
    }

    function m() {
        this.items = this.Aa = null;
        this.Fg = 0;
        z && (this.Aa = new Set);
        this.Sf = [];
        this.Kd = !0
    }

    function r(a) {
        t[L++] = a
    }

    function p() {
        this.Y = this.Yi = this.y = this.vj = 0
    }

    function b(a) {
        this.La = [];
        this.xh = this.zh = this.Ah = this.yh = 0;
        this.gh(a)
    }

    function a(a, e) {
        this.zj =
            a;
        this.yj = e;
        this.cells = {}
    }

    function d(a, e) {
        this.zj = a;
        this.yj = e;
        this.cells = {}
    }

    function u(a, e, f) {
        var d;
        return V.length ? (d = V.pop(), d.gk = a, d.x = e, d.y = f, d) : new ba(a, e, f)
    }

    function e(a, e, f) {
        this.gk = a;
        this.x = e;
        this.y = f;
        this.Sa = new ca
    }

    function f(a, e, f) {
        var d;
        return Z.length ? (d = Z.pop(), d.gk = a, d.x = e, d.y = f, d) : new da(a, e, f)
    }

    function n(a, e, f) {
        this.gk = a;
        this.x = e;
        this.y = f;
        this.Sa = [];
        this.xf = !0;
        this.Zc = new ca;
        this.Wf = !1
    }

    function g(a, e) {
        return a.Nc - e.Nc
    }
    x = function(a) {
        window.console && window.console.log && window.console.log(a)
    };
    ea = function(a) {
        window.console && window.console.error && window.console.error(a)
    };
    fa = function(a) {
        return "undefined" === typeof a
    };
    ga = function(a) {
        return "number" === typeof a
    };
    J = function(a) {
        return "string" === typeof a
    };
    ha = function(a) {
        return 0 < a && 0 === (a - 1 & a)
    };
    ia = function(a) {
        --a;
        for (var e = 1; 32 > e; e <<= 1) a = a | a >> e;
        return a + 1
    };
    ja = function(a) {
        return 0 > a ? -a : a
    };
    ka = function(a, e) {
        return a > e ? a : e
    };
    ma = function(a, e) {
        return a < e ? a : e
    };
    na = Math.PI;
    N = function(a) {
        return 0 <= a ? a | 0 : (a | 0) - 1
    };
    oa = function(a) {
        var e = a | 0;
        return e === a ? e : e + 1
    };
    pa = function(a, e, f, d, w, v, b, g) {
        var u, k, n, c;
        a < f ? (k = a, u = f) : (k = f, u = a);
        w < b ? (c = w, n = b) : (c = b, n = w);
        if (u < c || k > n) return !1;
        e < d ? (k = e, u = d) : (k = d, u = e);
        v < g ? (c = v, n = g) : (c = g, n = v);
        if (u < c || k > n) return !1;
        u = w - a + b - f;
        k = v - e + g - d;
        a = f - a;
        e = d - e;
        w = b - w;
        v = g - v;
        g = ja(e * w - v * a);
        return ja(w * k - v * u) > g ? !1 : ja(a * k - e * u) <= g
    };
    c.prototype.set = function(a, e, f, d) {
        this.left = a;
        this.top = e;
        this.right = f;
        this.bottom = d
    };
    c.prototype.nf = function(a) {
        this.left = a.left;
        this.top = a.top;
        this.right = a.right;
        this.bottom = a.bottom
    };
    c.prototype.width = function() {
        return this.right -
            this.left
    };
    c.prototype.height = function() {
        return this.bottom - this.top
    };
    c.prototype.offset = function(a, e) {
        this.left += a;
        this.top += e;
        this.right += a;
        this.bottom += e;
        return this
    };
    c.prototype.normalize = function() {
        var a = 0;
        this.left > this.right && (a = this.left, this.left = this.right, this.right = a);
        this.top > this.bottom && (a = this.top, this.top = this.bottom, this.bottom = a)
    };
    c.prototype.Lm = function(a) {
        return !(a.right < this.left || a.bottom < this.top || a.left > this.right || a.top > this.bottom)
    };
    c.prototype.Mm = function(a, e, f) {
        return !(a.right +
            e < this.left || a.bottom + f < this.top || a.left + e > this.right || a.top + f > this.bottom)
    };
    c.prototype.nb = function(a, e) {
        return a >= this.left && a <= this.right && e >= this.top && e <= this.bottom
    };
    c.prototype.hg = function(a) {
        return this.left === a.left && this.top === a.top && this.right === a.right && this.bottom === a.bottom
    };
    Q = c;
    l.prototype.cf = function(a) {
        this.Ma = a.left;
        this.Na = a.top;
        this.kb = a.right;
        this.lb = a.top;
        this.eb = a.right;
        this.fb = a.bottom;
        this.$a = a.left;
        this.ab = a.bottom
    };
    l.prototype.jo = function(a, e) {
        if (0 === e) this.cf(a);
        else {
            var f =
                Math.sin(e),
                d = Math.cos(e),
                w = a.left * f,
                v = a.top * f,
                b = a.right * f,
                f = a.bottom * f,
                g = a.left * d,
                u = a.top * d,
                k = a.right * d,
                d = a.bottom * d;
            this.Ma = g - v;
            this.Na = u + w;
            this.kb = k - v;
            this.lb = u + b;
            this.eb = k - f;
            this.fb = d + b;
            this.$a = g - f;
            this.ab = d + w
        }
    };
    l.prototype.offset = function(a, e) {
        this.Ma += a;
        this.Na += e;
        this.kb += a;
        this.lb += e;
        this.eb += a;
        this.fb += e;
        this.$a += a;
        this.ab += e;
        return this
    };
    var y = 0,
        F = 0;
    l.prototype.Sl = function(a) {
        h(this.Ma, this.kb, this.eb, this.$a);
        a.left = y;
        a.right = F;
        h(this.Na, this.lb, this.fb, this.ab);
        a.top = y;
        a.bottom = F
    };
    l.prototype.nb =
        function(a, e) {
            var f = this.Ma,
                d = this.Na,
                w = this.kb - f,
                v = this.lb - d,
                b = this.eb - f,
                g = this.fb - d,
                u = a - f,
                k = e - d,
                n = w * w + v * v,
                c = w * b + v * g,
                v = w * u + v * k,
                q = b * b + g * g,
                y = b * u + g * k,
                h = 1 / (n * q - c * c),
                w = (q * v - c * y) * h,
                n = (n * y - c * v) * h;
            if (0 <= w && 0 < n && 1 > w + n) return !0;
            w = this.$a - f;
            v = this.ab - d;
            n = w * w + v * v;
            c = w * b + v * g;
            v = w * u + v * k;
            h = 1 / (n * q - c * c);
            w = (q * v - c * y) * h;
            n = (n * y - c * v) * h;
            return 0 <= w && 0 < n && 1 > w + n
        };
    l.prototype.Id = function(a, e) {
        if (e) switch (a) {
            case 0:
                return this.Ma;
            case 1:
                return this.kb;
            case 2:
                return this.eb;
            case 3:
                return this.$a;
            case 4:
                return this.Ma;
            default:
                return this.Ma
        } else switch (a) {
            case 0:
                return this.Na;
            case 1:
                return this.lb;
            case 2:
                return this.fb;
            case 3:
                return this.ab;
            case 4:
                return this.Na;
            default:
                return this.Na
        }
    };
    l.prototype.yn = function() {
        return (this.Ma + this.kb + this.eb + this.$a) / 4
    };
    l.prototype.zn = function() {
        return (this.Na + this.lb + this.fb + this.ab) / 4
    };
    l.prototype.ok = function(a) {
        var e = a.yn(),
            f = a.zn();
        if (this.nb(e, f)) return !0;
        e = this.yn();
        f = this.zn();
        if (a.nb(e, f)) return !0;
        var d, w, v, b, g, u, k, n;
        for (k = 0; 4 > k; k++)
            for (n = 0; 4 > n; n++)
                if (e = this.Id(k, !0), f = this.Id(k, !1), d = this.Id(k + 1, !0), w = this.Id(k + 1, !1), v = a.Id(n, !0), b = a.Id(n, !1), g = a.Id(n + 1, !0), u = a.Id(n + 1, !1), pa(e, f, d, w, v, b, g, u)) return !0;
        return !1
    };
    qa = l;
    ra = function(a, e) {
        for (var f in e) e.hasOwnProperty(f) && (a[f] = e[f]);
        return a
    };
    sa = function(a, e) {
        var f, d;
        e = N(e);
        if (!(0 > e || e >= a.length)) {
            f = e;
            for (d = a.length - 1; f < d; f++) a[f] = a[f + 1];
            va(a, d)
        }
    };
    va = function(a, e) {
        a.length = e
    };
    S = function(a) {
        va(a, 0)
    };
    wa = function(a, e) {
        S(a);
        var f, d;
        f = 0;
        for (d = e.length; f < d; ++f) a[f] = e[f]
    };
    ya = function(a, e) {
        a.push.apply(a, e)
    };
    za = function(a, e) {
        var f, d;
        f = 0;
        for (d = a.length; f < d; ++f)
            if (a[f] === e) return f;
        return -1
    };
    Aa = function(a, e) {
        var f = za(a, e); - 1 !== f && sa(a, f)
    };
    Ba = function(a, e, f) {
        return a < e ? e : a > f ? f : a
    };
    T = function(a) {
        return a / (180 / na)
    };
    Ca = function(a) {
        return 180 / na * a
    };
    Da = function(a) {
        a %= 360;
        0 > a && (a += 360);
        return a
    };
    Ea = function(a) {
        a %= 2 * na;
        0 > a && (a += 2 * na);
        return a
    };
    Fa = function(a) {
        return Da(Ca(a))
    };
    Ga = function(a) {
        return Ea(T(a))
    };
    Ia = function(a, e, f, d) {
        return Math.atan2(d - e, f - a)
    };
    Ja = function(a, e) {
        if (a === e) return 0;
        var f = Math.sin(a),
            d = Math.cos(a),
            w = Math.sin(e),
            v = Math.cos(e),
            f = f * w + d * v;
        return 1 <= f ? 0 : -1 >= f ? na : Math.acos(f)
    };
    Ka = function(a, e, f) {
        var d = Math.sin(a),
            w = Math.cos(a),
            v = Math.sin(e),
            b = Math.cos(e);
        return Math.acos(d * v + w * b) > f ? 0 < w * v - d * b ? Ea(a + f) : Ea(a - f) : Ea(e)
    };
    La = function(a, e) {
        var f = Math.sin(a),
            d = Math.cos(a),
            w = Math.sin(e),
            v = Math.cos(e);
        return 0 >= d * w - f * v
    };
    Ma = function(a, e, f, d) {
        a = f - a;
        e = d - e;
        return Math.sqrt(a * a + e * e)
    };
    Na = function(a, e) {
        return !a !== !e
    };
    Oa = function(a) {
        for (var e in a)
            if (a.hasOwnProperty(e)) return !0;
        return !1
    };
    Pa = function(a) {
        for (var e in a) a.hasOwnProperty(e) && delete a[e]
    };
    var I = +new Date;
    Qa = function() {
        if ("undefined" !==
            typeof window.performance) {
            var a = window.performance;
            if ("undefined" !== typeof a.now) return a.now();
            if ("undefined" !== typeof a.webkitNow) return a.webkitNow();
            if ("undefined" !== typeof a.mozNow) return a.mozNow();
            if ("undefined" !== typeof a.msNow) return a.msNow()
        }
        return Date.now() - I
    };
    var k = !1,
        q = k = !1,
        D = !1;
    "undefined" !== typeof window && (k = /chrome/i.test(navigator.userAgent) || /chromium/i.test(navigator.userAgent), k = !k && /safari/i.test(navigator.userAgent), q = /(iphone|ipod|ipad)/i.test(navigator.userAgent), D = window.c2ejecta);
    var z = !k && !D && !q && "undefined" !== typeof Set && "undefined" !== typeof Set.prototype.forEach;
    m.prototype.contains = function(a) {
        return this.pd() ? !1 : z ? this.Aa.has(a) : this.items && this.items.hasOwnProperty(a)
    };
    m.prototype.add = function(a) {
        if (z) this.Aa.has(a) || (this.Aa.add(a), this.Kd = !1);
        else {
            var e = a.toString(),
                f = this.items;
            f ? f.hasOwnProperty(e) || (f[e] = a, this.Fg++, this.Kd = !1) : (this.items = {}, this.items[e] = a, this.Fg = 1, this.Kd = !1)
        }
    };
    m.prototype.remove = function(a) {
        if (!this.pd())
            if (z) this.Aa.has(a) && (this.Aa["delete"](a),
                this.Kd = !1);
            else if (this.items) {
            a = a.toString();
            var e = this.items;
            e.hasOwnProperty(a) && (delete e[a], this.Fg--, this.Kd = !1)
        }
    };
    m.prototype.clear = function() {
        this.pd() || (z ? this.Aa.clear() : (this.items = null, this.Fg = 0), S(this.Sf), this.Kd = !0)
    };
    m.prototype.pd = function() {
        return 0 === this.count()
    };
    m.prototype.count = function() {
        return z ? this.Aa.size : this.Fg
    };
    var t = null,
        L = 0;
    m.prototype.Ft = function() {
        if (!this.Kd) {
            if (z) S(this.Sf), t = this.Sf, L = 0, this.Aa.forEach(r), t = null, L = 0;
            else {
                var a = this.Sf;
                S(a);
                var e, f = 0,
                    d = this.items;
                if (d)
                    for (e in d) d.hasOwnProperty(e) && (a[f++] = d[e])
            }
            this.Kd = !0
        }
    };
    m.prototype.zd = function() {
        this.Ft();
        return this.Sf
    };
    ca = m;
    new ca;
    Ra = function(a, e) {
        z ? Sa(a, e.Aa) : Ta(a, e.zd())
    };
    Sa = function(a, e) {
        var f, d, w, v;
        d = f = 0;
        for (w = a.length; f < w; ++f) v = a[f], e.has(v) || (a[d++] = v);
        va(a, d)
    };
    Ta = function(a, e) {
        var f, d, w, v;
        d = f = 0;
        for (w = a.length; f < w; ++f) v = a[f], -1 === za(e, v) && (a[d++] = v);
        va(a, d)
    };
    p.prototype.add = function(a) {
        this.y = a - this.vj;
        this.Yi = this.Y + this.y;
        this.vj = this.Yi - this.Y - this.y;
        this.Y = this.Yi
    };
    p.prototype.reset = function() {
        this.Y =
            this.Yi = this.y = this.vj = 0
    };
    Ua = p;
    Va = function(a) {
        return a.replace(/[-[\]{}()*+?.,\\^$|#\s]/g, "\\$&")
    };
    b.prototype.gh = function(a) {
        this.Tn = a;
        this.Jc = a.length / 2;
        this.La.length = a.length;
        this.Ch = this.Dh = -1;
        this.Vl = 0
    };
    b.prototype.Ke = function() {
        return !this.Tn.length
    };
    b.prototype.Z = function() {
        for (var a = this.La, e = a[0], f = e, d = a[1], w = d, v, b, g = 1, u = this.Jc; g < u; ++g) b = 2 * g, v = a[b], b = a[b + 1], v < e && (e = v), v > f && (f = v), b < d && (d = b), b > w && (w = b);
        this.yh = e;
        this.zh = f;
        this.Ah = d;
        this.xh = w
    };
    b.prototype.cf = function(a, e, f) {
        this.La.length =
            8;
        this.Jc = 4;
        var d = this.La;
        d[0] = a.left - e;
        d[1] = a.top - f;
        d[2] = a.right - e;
        d[3] = a.top - f;
        d[4] = a.right - e;
        d[5] = a.bottom - f;
        d[6] = a.left - e;
        d[7] = a.bottom - f;
        this.Dh = a.right - a.left;
        this.Ch = a.bottom - a.top;
        this.Z()
    };
    b.prototype.bf = function(a, e, f, d, w) {
        this.La.length = 8;
        this.Jc = 4;
        var v = this.La;
        v[0] = a.Ma - e;
        v[1] = a.Na - f;
        v[2] = a.kb - e;
        v[3] = a.lb - f;
        v[4] = a.eb - e;
        v[5] = a.fb - f;
        v[6] = a.$a - e;
        v[7] = a.ab - f;
        this.Dh = d;
        this.Ch = w;
        this.Z()
    };
    b.prototype.io = function(a) {
        this.Jc = a.Jc;
        wa(this.La, a.La);
        this.yh = a.yh;
        this.Ah - a.Ah;
        this.zh = a.zh;
        this.xh =
            a.xh
    };
    b.prototype.ve = function(a, e, f) {
        if (this.Dh !== a || this.Ch !== e || this.Vl !== f) {
            this.Dh = a;
            this.Ch = e;
            this.Vl = f;
            var d, w, v, b, g, u = 0,
                k = 1,
                n = this.Tn,
                c = this.La;
            0 !== f && (u = Math.sin(f), k = Math.cos(f));
            f = 0;
            for (v = this.Jc; f < v; f++) d = 2 * f, w = d + 1, b = n[d] * a, g = n[w] * e, c[d] = b * k - g * u, c[w] = g * k + b * u;
            this.Z()
        }
    };
    b.prototype.nb = function(a, e) {
        var f = this.La;
        if (a === f[0] && e === f[1]) return !0;
        var d, w, v, b = this.Jc,
            g = this.yh - 110,
            u = this.Ah - 101,
            k = this.zh + 131,
            n = this.xh + 120,
            c, q, y = 0,
            h = 0;
        for (d = 0; d < b; d++) w = 2 * d, v = (d + 1) % b * 2, c = f[w], w = f[w + 1], q = f[v], v =
            f[v + 1], pa(g, u, a, e, c, w, q, v) && y++, pa(k, n, a, e, c, w, q, v) && h++;
        return 1 === y % 2 || 1 === h % 2
    };
    b.prototype.He = function(a, e, f) {
        var d = a.La,
            w = this.La;
        if (this.nb(d[0] + e, d[1] + f) || a.nb(w[0] - e, w[1] - f)) return !0;
        var v, b, g, u, k, n, c, q, y, h, l, t;
        v = 0;
        for (u = this.Jc; v < u; v++)
            for (b = 2 * v, g = (v + 1) % u * 2, q = w[b], b = w[b + 1], y = w[g], h = w[g + 1], g = 0, c = a.Jc; g < c; g++)
                if (k = 2 * g, n = (g + 1) % c * 2, l = d[k] + e, k = d[k + 1] + f, t = d[n] + e, n = d[n + 1] + f, pa(q, b, y, h, l, k, t, n)) return !0;
        return !1
    };
    Wa = b;
    a.prototype.Td = function(a, e, f) {
        var d;
        d = this.cells[a];
        return d ? (d = d[e]) ? d : f ? (d = u(this,
            a, e), this.cells[a][e] = d) : null : f ? (d = u(this, a, e), this.cells[a] = {}, this.cells[a][e] = d) : null
    };
    a.prototype.tb = function(a) {
        return N(a / this.zj)
    };
    a.prototype.ub = function(a) {
        return N(a / this.yj)
    };
    a.prototype.update = function(a, e, f) {
        var d, w, v, b, g;
        if (e)
            for (d = e.left, w = e.right; d <= w; ++d)
                for (v = e.top, b = e.bottom; v <= b; ++v)
                    if (!f || !f.nb(d, v))
                        if (g = this.Td(d, v, !1)) g.remove(a), g.pd() && (g.Sa.clear(), 1E3 > V.length && V.push(g), this.cells[d][v] = null);
        if (f)
            for (d = f.left, w = f.right; d <= w; ++d)
                for (v = f.top, b = f.bottom; v <= b; ++v) e && e.nb(d,
                    v) || this.Td(d, v, !0).mk(a)
    };
    a.prototype.Mi = function(a, e) {
        var f, d, w, v, b, g;
        f = this.tb(a.left);
        w = this.ub(a.top);
        d = this.tb(a.right);
        for (b = this.ub(a.bottom); f <= d; ++f)
            for (v = w; v <= b; ++v)(g = this.Td(f, v, !1)) && g.dump(e)
    };
    Xa = a;
    d.prototype.Td = function(a, e, d) {
        var b;
        b = this.cells[a];
        return b ? (b = b[e]) ? b : d ? (b = f(this, a, e), this.cells[a][e] = b) : null : d ? (b = f(this, a, e), this.cells[a] = {}, this.cells[a][e] = b) : null
    };
    d.prototype.tb = function(a) {
        return N(a / this.zj)
    };
    d.prototype.ub = function(a) {
        return N(a / this.yj)
    };
    d.prototype.update =
        function(a, e, f) {
            var d, w, v, b, g;
            if (e)
                for (d = e.left, w = e.right; d <= w; ++d)
                    for (v = e.top, b = e.bottom; v <= b; ++v)
                        if (!f || !f.nb(d, v))
                            if (g = this.Td(d, v, !1)) g.remove(a), g.pd() && (g.reset(), 1E3 > Z.length && Z.push(g), this.cells[d][v] = null);
            if (f)
                for (d = f.left, w = f.right; d <= w; ++d)
                    for (v = f.top, b = f.bottom; v <= b; ++v) e && e.nb(d, v) || this.Td(d, v, !0).mk(a)
        };
    d.prototype.Mi = function(a, e, f, d, w) {
        var v, b;
        a = this.tb(a);
        e = this.ub(e);
        f = this.tb(f);
        for (v = this.ub(d); a <= f; ++a)
            for (d = e; d <= v; ++d)(b = this.Td(a, d, !1)) && b.dump(w)
    };
    d.prototype.zs = function(a) {
        var e,
            f, d, w, v;
        e = a.left;
        d = a.top;
        f = a.right;
        for (w = a.bottom; e <= f; ++e)
            for (a = d; a <= w; ++a)
                if (v = this.Td(e, a, !1)) v.xf = !1
    };
    Za = d;
    var V = [];
    e.prototype.pd = function() {
        return this.Sa.pd()
    };
    e.prototype.mk = function(a) {
        this.Sa.add(a)
    };
    e.prototype.remove = function(a) {
        this.Sa.remove(a)
    };
    e.prototype.dump = function(a) {
        ya(a, this.Sa.zd())
    };
    ba = e;
    var Z = [];
    n.prototype.pd = function() {
        if (!this.Sa.length) return !0;
        if (this.Sa.length > this.Zc.count()) return !1;
        this.Vj();
        return !0
    };
    n.prototype.mk = function(a) {
        this.Zc.contains(a) ? (this.Zc.remove(a),
            this.Zc.pd() && (this.Wf = !1)) : this.Sa.length ? (this.Sa[this.Sa.length - 1].Vc() > a.Vc() && (this.xf = !1), this.Sa.push(a)) : (this.Sa.push(a), this.xf = !0)
    };
    n.prototype.remove = function(a) {
        this.Zc.add(a);
        this.Wf = !0;
        30 <= this.Zc.count() && this.Vj()
    };
    n.prototype.Vj = function() {
        this.Wf && (this.Zc.count() === this.Sa.length ? this.reset() : (Ra(this.Sa, this.Zc), this.Zc.clear(), this.Wf = !1))
    };
    n.prototype.Mq = function() {
        this.xf || (this.Sa.sort(g), this.xf = !0)
    };
    n.prototype.reset = function() {
        S(this.Sa);
        this.xf = !0;
        this.Zc.clear();
        this.Wf = !1
    };
    n.prototype.dump = function(a) {
        this.Vj();
        this.Mq();
        this.Sa.length && a.push(this.Sa)
    };
    da = n;
    var E = "lighter xor copy destination-over source-in destination-in source-out destination-out source-atop destination-atop".split(" ");
    $a = function(a) {
        return 0 >= a || 11 <= a ? "source-over" : E[a - 1]
    };
    ab = function(a, e, f) {
        if (f) switch (a.jb = f.ONE, a.gb = f.ONE_MINUS_SRC_ALPHA, e) {
            case 1:
                a.jb = f.ONE;
                a.gb = f.ONE;
                break;
            case 3:
                a.jb = f.ONE;
                a.gb = f.ZERO;
                break;
            case 4:
                a.jb = f.ONE_MINUS_DST_ALPHA;
                a.gb = f.ONE;
                break;
            case 5:
                a.jb = f.DST_ALPHA;
                a.gb =
                    f.ZERO;
                break;
            case 6:
                a.jb = f.ZERO;
                a.gb = f.SRC_ALPHA;
                break;
            case 7:
                a.jb = f.ONE_MINUS_DST_ALPHA;
                a.gb = f.ZERO;
                break;
            case 8:
                a.jb = f.ZERO;
                a.gb = f.ONE_MINUS_SRC_ALPHA;
                break;
            case 9:
                a.jb = f.DST_ALPHA;
                a.gb = f.ONE_MINUS_SRC_ALPHA;
                break;
            case 10:
                a.jb = f.ONE_MINUS_DST_ALPHA, a.gb = f.SRC_ALPHA
        }
    };
    bb = function(a) {
        return Math.round(1E6 * a) / 1E6
    };
    cb = function(a, e) {
        return "string" !== typeof a || "string" !== typeof e || a.length !== e.length ? !1 : a === e ? !0 : a.toLowerCase() === e.toLowerCase()
    };
    db = function(a) {
        a = a.target;
        return !a || a === document || a ===
            window || document && document.body && a === document.body || cb(a.tagName, "canvas") ? !0 : !1
    }
})();
var Vb = "undefined" !== typeof Float32Array ? Float32Array : Array;

function Wb(c) {
    var l = new Vb(3);
    c && (l[0] = c[0], l[1] = c[1], l[2] = c[2]);
    return l
}

function Xb(c) {
    var l = new Vb(16);
    c && (l[0] = c[0], l[1] = c[1], l[2] = c[2], l[3] = c[3], l[4] = c[4], l[5] = c[5], l[6] = c[6], l[7] = c[7], l[8] = c[8], l[9] = c[9], l[10] = c[10], l[11] = c[11], l[12] = c[12], l[13] = c[13], l[14] = c[14], l[15] = c[15]);
    return l
}

function Yb(c, l) {
    l[0] = c[0];
    l[1] = c[1];
    l[2] = c[2];
    l[3] = c[3];
    l[4] = c[4];
    l[5] = c[5];
    l[6] = c[6];
    l[7] = c[7];
    l[8] = c[8];
    l[9] = c[9];
    l[10] = c[10];
    l[11] = c[11];
    l[12] = c[12];
    l[13] = c[13];
    l[14] = c[14];
    l[15] = c[15]
}

function Zb(c, l) {
    var h = l[0],
        m = l[1];
    l = l[2];
    c[0] *= h;
    c[1] *= h;
    c[2] *= h;
    c[3] *= h;
    c[4] *= m;
    c[5] *= m;
    c[6] *= m;
    c[7] *= m;
    c[8] *= l;
    c[9] *= l;
    c[10] *= l;
    c[11] *= l
}

function $b(c, l, h, m) {
    m || (m = Xb());
    var r, p, b, a, d, u, e, f, n = c[0],
        g = c[1];
    c = c[2];
    p = h[0];
    b = h[1];
    r = h[2];
    h = l[1];
    u = l[2];
    n === l[0] && g === h && c === u ? (c = m, c[0] = 1, c[1] = 0, c[2] = 0, c[3] = 0, c[4] = 0, c[5] = 1, c[6] = 0, c[7] = 0, c[8] = 0, c[9] = 0, c[10] = 1, c[11] = 0, c[12] = 0, c[13] = 0, c[14] = 0, c[15] = 1) : (h = n - l[0], u = g - l[1], e = c - l[2], f = 1 / Math.sqrt(h * h + u * u + e * e), h *= f, u *= f, e *= f, l = b * e - r * u, r = r * h - p * e, p = p * u - b * h, (f = Math.sqrt(l * l + r * r + p * p)) ? (f = 1 / f, l *= f, r *= f, p *= f) : p = r = l = 0, b = u * p - e * r, a = e * l - h * p, d = h * r - u * l, (f = Math.sqrt(b * b + a * a + d * d)) ? (f = 1 / f, b *= f, a *= f, d *= f) : d = a = b = 0,
        m[0] = l, m[1] = b, m[2] = h, m[3] = 0, m[4] = r, m[5] = a, m[6] = u, m[7] = 0, m[8] = p, m[9] = d, m[10] = e, m[11] = 0, m[12] = -(l * n + r * g + p * c), m[13] = -(b * n + a * g + d * c), m[14] = -(h * n + u * g + e * c), m[15] = 1)
}
(function() {
    function c(a, d, b) {
        this.Vd = /msie/i.test(navigator.userAgent) || /trident/i.test(navigator.userAgent);
        this.height = this.width = 0;
        this.ea = !!b;
        this.di = this.yg = !1;
        this.Hj = 0;
        this.lj = 1;
        this.Al = 1E3;
        this.Lt = (this.Al - this.lj) / 32768;
        this.wj = Wb([0, 0, 100]);
        this.dn = Wb([0, 0, 0]);
        this.Go = Wb([0, 1, 0]);
        this.rh = Wb([1, 1, 1]);
        this.hm = !0;
        this.Bi = Xb();
        this.$b = Xb();
        this.zk = Xb();
        this.Gj = Xb();
        this.q = a;
        this.Hm()
    }

    function l(a, d, b) {
        this.q = a;
        this.hh = d;
        this.name = b;
        this.kc = a.getAttribLocation(d, "aPos");
        this.ee = a.getAttribLocation(d,
            "aTex");
        this.bn = a.getUniformLocation(d, "matP");
        this.wi = a.getUniformLocation(d, "matMV");
        this.Af = a.getUniformLocation(d, "opacity");
        this.Ek = a.getUniformLocation(d, "colorFill");
        this.cn = a.getUniformLocation(d, "samplerFront");
        this.Ng = a.getUniformLocation(d, "samplerBack");
        this.Qe = a.getUniformLocation(d, "destStart");
        this.Pe = a.getUniformLocation(d, "destEnd");
        this.Pg = a.getUniformLocation(d, "seconds");
        this.Gk = a.getUniformLocation(d, "pixelWidth");
        this.Fk = a.getUniformLocation(d, "pixelHeight");
        this.Mg = a.getUniformLocation(d,
            "layerScale");
        this.Lg = a.getUniformLocation(d, "layerAngle");
        this.Qg = a.getUniformLocation(d, "viewOrigin");
        this.Og = a.getUniformLocation(d, "scrollPos");
        this.ds = !!(this.Gk || this.Fk || this.Pg || this.Ng || this.Qe || this.Pe || this.Mg || this.Lg || this.Qg || this.Og);
        this.qn = this.rn = -999;
        this.Ai = 1;
        this.kn = this.jn = 0;
        this.nn = this.hn = this.gn = 1;
        this.un = this.tn = this.sn = this.wn = this.vn = this.ln = 0;
        this.yk = [];
        this.pn = Xb();
        this.Af && a.uniform1f(this.Af, 1);
        this.Ek && a.uniform4f(this.Ek, 1, 1, 1, 1);
        this.cn && a.uniform1i(this.cn, 0);
        this.Ng &&
            a.uniform1i(this.Ng, 1);
        this.Qe && a.uniform2f(this.Qe, 0, 0);
        this.Pe && a.uniform2f(this.Pe, 1, 1);
        this.Mg && a.uniform1f(this.Mg, 1);
        this.Lg && a.uniform1f(this.Lg, 0);
        this.Qg && a.uniform2f(this.Qg, 0, 0);
        this.Og && a.uniform2f(this.Og, 0, 0);
        this.Pg && a.uniform1f(this.Pg, 0);
        this.De = !1
    }

    function h(a, d) {
        return a[0] === d[0] && a[1] === d[1] && a[2] === d[2] && a[3] === d[3] && a[4] === d[4] && a[5] === d[5] && a[6] === d[6] && a[7] === d[7] && a[8] === d[8] && a[9] === d[9] && a[10] === d[10] && a[11] === d[11] && a[12] === d[12] && a[13] === d[13] && a[14] === d[14] && a[15] ===
            d[15]
    }

    function m(a, d) {
        this.type = a;
        this.n = d;
        this.q = d.q;
        this.Wc = this.sb = this.Ln = 0;
        this.J = this.tc = null;
        this.ko = []
    }
    var r = Xb();
    c.prototype.Hm = function() {
        var a = this.q,
            d;
        this.Xm = 1;
        this.Me = this.Zd = null;
        this.Ih = 1;
        a.clearColor(0, 0, 0, 0);
        a.clear(a.COLOR_BUFFER_BIT);
        a.enable(a.BLEND);
        a.blendFunc(a.ONE, a.ONE_MINUS_SRC_ALPHA);
        a.disable(a.CULL_FACE);
        a.disable(a.STENCIL_TEST);
        a.disable(a.DITHER);
        this.ea ? (a.enable(a.DEPTH_TEST), a.depthFunc(a.LEQUAL)) : a.disable(a.DEPTH_TEST);
        this.Ym = a.ONE;
        this.Wm = a.ONE_MINUS_SRC_ALPHA;
        this.jj = new Float32Array(8E3 * (this.ea ? 3 : 2));
        this.Zi = new Float32Array(16E3);
        this.Qn = new Float32Array(32E3);
        this.Nk = a.createBuffer();
        a.bindBuffer(a.ARRAY_BUFFER, this.Nk);
        a.bufferData(a.ARRAY_BUFFER, this.Qn.byteLength, a.DYNAMIC_DRAW);
        this.ph = Array(4);
        this.lh = Array(4);
        for (d = 0; 4 > d; d++) this.ph[d] = a.createBuffer(), a.bindBuffer(a.ARRAY_BUFFER, this.ph[d]), a.bufferData(a.ARRAY_BUFFER, this.jj.byteLength, a.DYNAMIC_DRAW), this.lh[d] = a.createBuffer(), a.bindBuffer(a.ARRAY_BUFFER, this.lh[d]), a.bufferData(a.ARRAY_BUFFER,
            this.Zi.byteLength, a.DYNAMIC_DRAW);
        this.jd = 0;
        this.hs = a.createBuffer();
        a.bindBuffer(a.ELEMENT_ARRAY_BUFFER, this.hs);
        for (var b = new Uint16Array(12E3), e = d = 0; 12E3 > d;) b[d++] = e, b[d++] = e + 1, b[d++] = e + 2, b[d++] = e, b[d++] = e + 2, b[d++] = e + 3, e += 4;
        a.bufferData(a.ELEMENT_ARRAY_BUFFER, b, a.STATIC_DRAW);
        this.Ok = this.Of = this.Ad = 0;
        this.ya = [];
        d = this.ea ? "attribute highp vec3 aPos;\nattribute mediump vec2 aTex;\nvarying mediump vec2 vTex;\nuniform highp mat4 matP;\nuniform highp mat4 matMV;\nvoid main(void) {\n\tgl_Position = matP * matMV * vec4(aPos.x, aPos.y, aPos.z, 1.0);\n\tvTex = aTex;\n}" :
            "attribute highp vec2 aPos;\nattribute mediump vec2 aTex;\nvarying mediump vec2 vTex;\nuniform highp mat4 matP;\nuniform highp mat4 matMV;\nvoid main(void) {\n\tgl_Position = matP * matMV * vec4(aPos.x, aPos.y, 0.0, 1.0);\n\tvTex = aTex;\n}";
        b = this.bg({
            src: "varying mediump vec2 vTex;\nuniform lowp float opacity;\nuniform lowp sampler2D samplerFront;\nvoid main(void) {\n\tgl_FragColor = texture2D(samplerFront, vTex);\n\tgl_FragColor *= opacity;\n}"
        }, d, "<default>");
        this.ya.push(b);
        b = this.bg({
                src: "uniform mediump sampler2D samplerFront;\nvarying lowp float opacity;\nvoid main(void) {\n\tgl_FragColor = texture2D(samplerFront, gl_PointCoord);\n\tgl_FragColor *= opacity;\n}"
            },
            "attribute vec4 aPos;\nvarying float opacity;\nuniform mat4 matP;\nuniform mat4 matMV;\nvoid main(void) {\n\tgl_Position = matP * matMV * vec4(aPos.x, aPos.y, 0.0, 1.0);\n\tgl_PointSize = aPos.z;\n\topacity = aPos.w;\n}", "<point>");
        this.ya.push(b);
        b = this.bg({
            src: "varying mediump vec2 vTex;\nuniform lowp sampler2D samplerFront;\nvoid main(void) {\n\tif (texture2D(samplerFront, vTex).a < 1.0)\n\t\tdiscard;\n}"
        }, d, "<earlyz>");
        this.ya.push(b);
        b = this.bg({
                src: "uniform lowp vec4 colorFill;\nvoid main(void) {\n\tgl_FragColor = colorFill;\n}"
            },
            d, "<fill>");
        this.ya.push(b);
        for (var f in ac) ac.hasOwnProperty(f) && this.ya.push(this.bg(ac[f], d, f));
        a.activeTexture(a.TEXTURE0);
        a.bindTexture(a.TEXTURE_2D, null);
        this.Jd = [];
        this.hd = 0;
        this.hb = !1;
        this.em = this.Gg = -1;
        this.of = null;
        this.Uj = a.createFramebuffer();
        this.Kh = this.Ni = null;
        this.Ql = !1;
        this.ea && (this.Kh = a.createRenderbuffer());
        this.ne = Wb([0, 0, 0]);
        this.xn = a.getParameter(a.ALIASED_POINT_SIZE_RANGE)[1];
        2048 < this.xn && (this.xn = 2048);
        this.Cb(0)
    };
    l.prototype.wl = function(a) {
        h(this.pn, a) || (Yb(a, this.pn), this.q.uniformMatrix4fv(this.wi, !1, a))
    };
    c.prototype.bg = function(a, d, b) {
        var e = this.q,
            f = e.createShader(e.FRAGMENT_SHADER);
        e.shaderSource(f, a.src);
        e.compileShader(f);
        if (!e.getShaderParameter(f, e.COMPILE_STATUS)) return e.deleteShader(f), null;
        var n = e.createShader(e.VERTEX_SHADER);
        e.shaderSource(n, d);
        e.compileShader(n);
        if (!e.getShaderParameter(n, e.COMPILE_STATUS)) return e.deleteShader(f), e.deleteShader(n), null;
        d = e.createProgram();
        e.attachShader(d, f);
        e.attachShader(d, n);
        e.linkProgram(d);
        if (!e.getProgramParameter(d, e.LINK_STATUS)) return e.deleteShader(f),
            e.deleteShader(n), e.deleteProgram(d), null;
        e.useProgram(d);
        e.deleteShader(f);
        e.deleteShader(n);
        b = new l(e, d, b);
        b.Sj = a.Sj || 0;
        b.Tj = a.Tj || 0;
        b.dm = !!a.dm;
        b.Hc = !!a.Hc;
        b.Ol = !!a.Ol;
        b.K = a.K || [];
        a = 0;
        for (f = b.K.length; a < f; a++) b.K[a][1] = e.getUniformLocation(d, b.K[a][0]), b.yk.push(0), e.uniform1f(b.K[a][1], 0);
        return b
    };
    c.prototype.ek = function(a) {
        var d, b;
        d = 0;
        for (b = this.ya.length; d < b; d++)
            if (this.ya[d].name === a) return d;
        return -1
    };
    c.prototype.Sn = function(a, d, b) {
        var e = this.$b,
            f = this.Bi,
            n = [0, 0, 0, 0, 0, 0, 0, 0];
        n[0] = e[0] * a +
            e[4] * d + e[12];
        n[1] = e[1] * a + e[5] * d + e[13];
        n[2] = e[2] * a + e[6] * d + e[14];
        n[3] = e[3] * a + e[7] * d + e[15];
        n[4] = f[0] * n[0] + f[4] * n[1] + f[8] * n[2] + f[12] * n[3];
        n[5] = f[1] * n[0] + f[5] * n[1] + f[9] * n[2] + f[13] * n[3];
        n[6] = f[2] * n[0] + f[6] * n[1] + f[10] * n[2] + f[14] * n[3];
        n[7] = -n[2];
        0 !== n[7] && (n[7] = 1 / n[7], n[4] *= n[7], n[5] *= n[7], n[6] *= n[7], b[0] = (.5 * n[4] + .5) * this.width, b[1] = (.5 * n[5] + .5) * this.height)
    };
    c.prototype.Ze = function(a, d, b) {
        if (this.width !== a || this.height !== d || b) {
            this.Rd();
            b = this.q;
            this.width = a;
            this.height = d;
            b.viewport(0, 0, a, d);
            $b(this.wj,
                this.dn, this.Go, this.$b);
            if (this.ea) {
                var e = -a / 2;
                a = a / 2;
                var f = d / 2;
                d = -d / 2;
                var n = this.lj,
                    g = this.Al,
                    c = this.Bi;
                c || (c = Xb());
                var h = a - e,
                    l = d - f,
                    k = g - n;
                c[0] = 2 / h;
                c[1] = 0;
                c[2] = 0;
                c[3] = 0;
                c[4] = 0;
                c[5] = 2 / l;
                c[6] = 0;
                c[7] = 0;
                c[8] = 0;
                c[9] = 0;
                c[10] = -2 / k;
                c[11] = 0;
                c[12] = -(e + a) / h;
                c[13] = -(d + f) / l;
                c[14] = -(g + n) / k;
                c[15] = 1;
                this.rh[0] = 1;
                this.rh[1] = 1
            } else d = a / d, e = this.lj, a = this.Al, c = this.Bi, g = e * Math.tan(45 * Math.PI / 360), d *= g, f = -d, n = -g, c || (c = Xb()), h = d - f, l = g - n, k = a - e, c[0] = 2 * e / h, c[1] = 0, c[2] = 0, c[3] = 0, c[4] = 0, c[5] = 2 * e / l, c[6] = 0, c[7] = 0, c[8] = (d + f) /
                h, c[9] = (g + n) / l, c[10] = -(a + e) / k, c[11] = -1, c[12] = 0, c[13] = 0, c[14] = -(a * e * 2) / k, c[15] = 0, e = [0, 0], a = [0, 0], this.Sn(0, 0, e), this.Sn(1, 1, a), this.rh[0] = 1 / (a[0] - e[0]), this.rh[1] = -1 / (a[1] - e[1]);
            e = 0;
            for (a = this.ya.length; e < a; e++) f = this.ya[e], f.De = !1, f.bn && (b.useProgram(f.hh), b.uniformMatrix4fv(f.bn, !1, this.Bi));
            b.useProgram(this.ya[this.Gg].hh);
            b.bindTexture(b.TEXTURE_2D, null);
            b.activeTexture(b.TEXTURE1);
            b.bindTexture(b.TEXTURE_2D, null);
            b.activeTexture(b.TEXTURE0);
            this.Me = this.Zd = null;
            this.Kh && (b.bindFramebuffer(b.FRAMEBUFFER,
                this.Uj), b.bindRenderbuffer(b.RENDERBUFFER, this.Kh), b.renderbufferStorage(b.RENDERBUFFER, b.DEPTH_COMPONENT16, this.width, this.height), this.Ql || (b.framebufferRenderbuffer(b.FRAMEBUFFER, b.DEPTH_ATTACHMENT, b.RENDERBUFFER, this.Kh), this.Ql = !0), b.bindRenderbuffer(b.RENDERBUFFER, null), b.bindFramebuffer(b.FRAMEBUFFER, null), this.Ni = null)
        }
    };
    c.prototype.pc = function() {
        $b(this.wj, this.dn, this.Go, this.$b);
        Zb(this.$b, this.rh)
    };
    c.prototype.translate = function(a, d) {
        if (0 !== a || 0 !== d) {
            this.ne[0] = a;
            this.ne[1] = d;
            this.ne[2] =
                0;
            var b = this.$b,
                e = this.ne,
                f = e[0],
                c = e[1],
                e = e[2];
            b[12] = b[0] * f + b[4] * c + b[8] * e + b[12];
            b[13] = b[1] * f + b[5] * c + b[9] * e + b[13];
            b[14] = b[2] * f + b[6] * c + b[10] * e + b[14];
            b[15] = b[3] * f + b[7] * c + b[11] * e + b[15]
        }
    };
    c.prototype.scale = function(a, d) {
        if (1 !== a || 1 !== d) this.ne[0] = a, this.ne[1] = d, this.ne[2] = 1, Zb(this.$b, this.ne)
    };
    c.prototype.Qi = function(a) {
        if (0 !== a) {
            var d = this.$b,
                b, e = Math.sin(a);
            a = Math.cos(a);
            var f = d[0],
                c = d[1],
                g = d[2],
                h = d[3],
                l = d[4],
                m = d[5],
                k = d[6],
                q = d[7];
            b ? d !== b && (b[8] = d[8], b[9] = d[9], b[10] = d[10], b[11] = d[11], b[12] = d[12], b[13] =
                d[13], b[14] = d[14], b[15] = d[15]) : b = d;
            b[0] = f * a + l * e;
            b[1] = c * a + m * e;
            b[2] = g * a + k * e;
            b[3] = h * a + q * e;
            b[4] = f * -e + l * a;
            b[5] = c * -e + m * a;
            b[6] = g * -e + k * a;
            b[7] = h * -e + q * a
        }
    };
    c.prototype.ec = function() {
        if (!h(this.zk, this.$b)) {
            var a = this.Mb();
            a.type = 5;
            a.J ? Yb(this.$b, a.J) : a.J = Xb(this.$b);
            Yb(this.$b, this.zk);
            this.hb = !1
        }
    };
    c.prototype.Si = function(a) {
        this.ea && (32760 < a && (a = 32760), this.Hj = this.wj[2] - this.lj - a * this.Lt)
    };
    m.prototype.wq = function() {
        var a = this.q,
            d = this.n;
        0 !== this.sb ? (a.depthMask(!0), a.colorMask(!1, !1, !1, !1), a.disable(a.BLEND),
            a.bindFramebuffer(a.FRAMEBUFFER, d.Uj), a.framebufferTexture2D(a.FRAMEBUFFER, a.COLOR_ATTACHMENT0, a.TEXTURE_2D, null, 0), a.clear(a.DEPTH_BUFFER_BIT), a.bindFramebuffer(a.FRAMEBUFFER, null), d.di = !0) : (a.depthMask(!1), a.colorMask(!0, !0, !0, !0), a.enable(a.BLEND), d.di = !1)
    };
    m.prototype.Aq = function() {
        this.q.bindTexture(this.q.TEXTURE_2D, this.tc)
    };
    m.prototype.Bq = function() {
        var a = this.q;
        a.activeTexture(a.TEXTURE1);
        a.bindTexture(a.TEXTURE_2D, this.tc);
        a.activeTexture(a.TEXTURE0)
    };
    m.prototype.xq = function() {
        var a = this.Ln,
            d = this.n;
        d.Ih = a;
        d = d.of;
        d.Af && d.Ai !== a && (d.Ai = a, this.q.uniform1f(d.Af, a))
    };
    m.prototype.rq = function() {
        this.q.drawElements(this.q.TRIANGLES, this.Wc, this.q.UNSIGNED_SHORT, this.sb)
    };
    m.prototype.tq = function() {
        this.q.blendFunc(this.sb, this.Wc)
    };
    m.prototype.Cq = function() {
        var a, d, b, e = this.n.ya,
            f = this.n.em;
        a = 0;
        for (d = e.length; a < d; a++) b = e[a], a === f && b.wi ? (b.wl(this.J), b.De = !0) : b.De = !1;
        Yb(this.J, this.n.Gj)
    };
    m.prototype.sq = function() {
        var a = this.q,
            d = this.n;
        this.tc ? (d.Me === this.tc && (a.activeTexture(a.TEXTURE1), a.bindTexture(a.TEXTURE_2D,
            null), d.Me = null, a.activeTexture(a.TEXTURE0)), a.bindFramebuffer(a.FRAMEBUFFER, d.Uj), d.di || a.framebufferTexture2D(a.FRAMEBUFFER, a.COLOR_ATTACHMENT0, a.TEXTURE_2D, this.tc, 0)) : (d.ea || a.framebufferTexture2D(a.FRAMEBUFFER, a.COLOR_ATTACHMENT0, a.TEXTURE_2D, null, 0), a.bindFramebuffer(a.FRAMEBUFFER, null))
    };
    m.prototype.oq = function() {
        var a = this.q,
            d = this.sb;
        0 === d ? (a.clearColor(this.J[0], this.J[1], this.J[2], this.J[3]), a.clear(a.COLOR_BUFFER_BIT)) : 1 === d ? (a.enable(a.SCISSOR_TEST), a.scissor(this.J[0], this.J[1], this.J[2],
            this.J[3]), a.clearColor(0, 0, 0, 0), a.clear(a.COLOR_BUFFER_BIT), a.disable(a.SCISSOR_TEST)) : a.clear(a.DEPTH_BUFFER_BIT)
    };
    m.prototype.vq = function() {
        var a = this.q;
        0 !== this.sb ? a.enable(a.DEPTH_TEST) : a.disable(a.DEPTH_TEST)
    };
    m.prototype.qq = function() {
        var a = this.q,
            d = this.n;
        d.ea && a.disable(a.DEPTH_TEST);
        var b = d.ya[1];
        a.useProgram(b.hh);
        !b.De && b.wi && (b.wl(d.Gj), b.De = !0);
        a.enableVertexAttribArray(b.kc);
        a.bindBuffer(a.ARRAY_BUFFER, d.Nk);
        a.vertexAttribPointer(b.kc, 4, a.FLOAT, !1, 0, 0);
        a.drawArrays(a.POINTS, this.sb /
            4, this.Wc);
        b = d.of;
        a.useProgram(b.hh);
        0 <= b.kc && (a.enableVertexAttribArray(b.kc), a.bindBuffer(a.ARRAY_BUFFER, d.ph[d.jd]), a.vertexAttribPointer(b.kc, d.ea ? 3 : 2, a.FLOAT, !1, 0, 0));
        0 <= b.ee && (a.enableVertexAttribArray(b.ee), a.bindBuffer(a.ARRAY_BUFFER, d.lh[d.jd]), a.vertexAttribPointer(b.ee, 2, a.FLOAT, !1, 0, 0));
        d.ea && a.enable(a.DEPTH_TEST)
    };
    m.prototype.yq = function() {
        var a = this.q,
            d = this.n,
            b = d.ya[this.sb];
        d.em = this.sb;
        d.of = b;
        a.useProgram(b.hh);
        !b.De && b.wi && (b.wl(d.Gj), b.De = !0);
        b.Af && b.Ai !== d.Ih && (b.Ai = d.Ih, a.uniform1f(b.Af,
            d.Ih));
        0 <= b.kc && (a.enableVertexAttribArray(b.kc), a.bindBuffer(a.ARRAY_BUFFER, d.ph[d.jd]), a.vertexAttribPointer(b.kc, d.ea ? 3 : 2, a.FLOAT, !1, 0, 0));
        0 <= b.ee && (a.enableVertexAttribArray(b.ee), a.bindBuffer(a.ARRAY_BUFFER, d.lh[d.jd]), a.vertexAttribPointer(b.ee, 2, a.FLOAT, !1, 0, 0))
    };
    m.prototype.uq = function() {
        var a = this.J;
        this.q.uniform4f(this.n.of.Ek, a[0], a[1], a[2], a[3])
    };
    m.prototype.zq = function() {
        var a, b, c = this.n.of,
            e = this.q;
        a = this.J;
        c.Ng && this.n.Me !== this.tc && (e.activeTexture(e.TEXTURE1), e.bindTexture(e.TEXTURE_2D,
            this.tc), this.n.Me = this.tc, e.activeTexture(e.TEXTURE0));
        var f = a[0];
        c.Gk && f !== c.rn && (c.rn = f, e.uniform1f(c.Gk, f));
        f = a[1];
        c.Fk && f !== c.qn && (c.qn = f, e.uniform1f(c.Fk, f));
        f = a[2];
        b = a[3];
        !c.Qe || f === c.jn && b === c.kn || (c.jn = f, c.kn = b, e.uniform2f(c.Qe, f, b));
        f = a[4];
        b = a[5];
        !c.Pe || f === c.gn && b === c.hn || (c.gn = f, c.hn = b, e.uniform2f(c.Pe, f, b));
        f = a[6];
        c.Mg && f !== c.nn && (c.nn = f, e.uniform1f(c.Mg, f));
        f = a[7];
        c.Lg && f !== c.ln && (c.ln = f, e.uniform1f(c.Lg, f));
        f = a[8];
        b = a[9];
        !c.Qg || f === c.vn && b === c.wn || (c.vn = f, c.wn = b, e.uniform2f(c.Qg, f,
            b));
        f = a[10];
        b = a[11];
        !c.Og || f === c.sn && b === c.tn || (c.sn = f, c.tn = b, e.uniform2f(c.Og, f, b));
        f = a[12];
        c.Pg && f !== c.un && (c.un = f, e.uniform1f(c.Pg, f));
        if (c.K.length)
            for (a = 0, b = c.K.length; a < b; a++) f = this.ko[a], f !== c.yk[a] && (c.yk[a] = f, e.uniform1f(c.K[a][1], f))
    };
    c.prototype.Mb = function() {
        this.hd === this.Jd.length && this.Jd.push(new m(0, this));
        return this.Jd[this.hd++]
    };
    c.prototype.Rd = function() {
        if (0 !== this.hd && !this.q.isContextLost()) {
            var a = this.q;
            0 < this.Ok && (a.bindBuffer(a.ARRAY_BUFFER, this.Nk), a.bufferSubData(a.ARRAY_BUFFER,
                0, this.Qn.subarray(0, this.Ok)), b && 0 <= b.kc && "<point>" === b.name && a.vertexAttribPointer(b.kc, 4, a.FLOAT, !1, 0, 0));
            if (0 < this.Ad) {
                var b = this.of;
                a.bindBuffer(a.ARRAY_BUFFER, this.ph[this.jd]);
                a.bufferSubData(a.ARRAY_BUFFER, 0, this.jj.subarray(0, this.Ad));
                b && 0 <= b.kc && "<point>" !== b.name && a.vertexAttribPointer(b.kc, this.ea ? 3 : 2, a.FLOAT, !1, 0, 0);
                a.bindBuffer(a.ARRAY_BUFFER, this.lh[this.jd]);
                a.bufferSubData(a.ARRAY_BUFFER, 0, this.Zi.subarray(0, this.Of));
                b && 0 <= b.ee && "<point>" !== b.name && a.vertexAttribPointer(b.ee, 2,
                    a.FLOAT, !1, 0, 0)
            }
            for (var c, a = 0, b = this.hd; a < b; a++) switch (c = this.Jd[a], c.type) {
                case 1:
                    c.rq();
                    break;
                case 2:
                    c.Aq();
                    break;
                case 3:
                    c.xq();
                    break;
                case 4:
                    c.tq();
                    break;
                case 5:
                    c.Cq();
                    break;
                case 6:
                    c.sq();
                    break;
                case 7:
                    c.oq();
                    break;
                case 8:
                    c.qq();
                    break;
                case 9:
                    c.yq();
                    break;
                case 10:
                    c.zq();
                    break;
                case 11:
                    c.Bq();
                    break;
                case 12:
                    c.uq();
                    break;
                case 13:
                    c.vq();
                    break;
                case 14:
                    c.wq()
            }
            this.Ok = this.Of = this.Ad = this.hd = 0;
            this.di = this.hb = !1;
            this.jd++;
            4 <= this.jd && (this.jd = 0)
        }
    };
    c.prototype.Ye = function(a) {
        if (a !== this.Xm && !this.yg) {
            var b =
                this.Mb();
            b.type = 3;
            this.Xm = b.Ln = a;
            this.hb = !1
        }
    };
    c.prototype.Ob = function(a) {
        if (a !== this.Zd) {
            var b = this.Mb();
            b.type = 2;
            this.Zd = b.tc = a;
            this.hb = !1
        }
    };
    c.prototype.ke = function(a, b) {
        if ((a !== this.Ym || b !== this.Wm) && !this.yg) {
            var c = this.Mb();
            c.type = 4;
            c.sb = a;
            c.Wc = b;
            this.Ym = a;
            this.Wm = b;
            this.hb = !1
        }
    };
    c.prototype.$n = function() {
        this.ke(this.q.ONE, this.q.ONE_MINUS_SRC_ALPHA)
    };
    c.prototype.ah = function(a, b, c, e, f, n, g, h) {
        15992 <= this.Ad && this.Rd();
        var l = this.Ad,
            m = this.Of,
            k = this.jj,
            q = this.Zi,
            D = this.Hj;
        if (this.hb) this.Jd[this.hd -
            1].Wc += 6;
        else {
            var z = this.Mb();
            z.type = 1;
            z.sb = this.ea ? l : l / 2 * 3;
            z.Wc = 6;
            this.hb = !0
        }
        this.ea ? (k[l++] = a, k[l++] = b, k[l++] = D, k[l++] = c, k[l++] = e, k[l++] = D, k[l++] = f, k[l++] = n, k[l++] = D, k[l++] = g, k[l++] = h, k[l++] = D) : (k[l++] = a, k[l++] = b, k[l++] = c, k[l++] = e, k[l++] = f, k[l++] = n, k[l++] = g, k[l++] = h);
        q[m++] = 0;
        q[m++] = 0;
        q[m++] = 1;
        q[m++] = 0;
        q[m++] = 1;
        q[m++] = 1;
        q[m++] = 0;
        q[m++] = 1;
        this.Ad = l;
        this.Of = m
    };
    c.prototype.$c = function(a, b, c, e, f, n, g, l, h) {
        15992 <= this.Ad && this.Rd();
        var m = this.Ad,
            k = this.Of,
            q = this.jj,
            D = this.Zi,
            z = this.Hj;
        if (this.hb) this.Jd[this.hd -
            1].Wc += 6;
        else {
            var t = this.Mb();
            t.type = 1;
            t.sb = this.ea ? m : m / 2 * 3;
            t.Wc = 6;
            this.hb = !0
        }
        var t = h.left,
            L = h.top,
            p = h.right;
        h = h.bottom;
        this.ea ? (q[m++] = a, q[m++] = b, q[m++] = z, q[m++] = c, q[m++] = e, q[m++] = z, q[m++] = f, q[m++] = n, q[m++] = z, q[m++] = g, q[m++] = l, q[m++] = z) : (q[m++] = a, q[m++] = b, q[m++] = c, q[m++] = e, q[m++] = f, q[m++] = n, q[m++] = g, q[m++] = l);
        D[k++] = t;
        D[k++] = L;
        D[k++] = p;
        D[k++] = L;
        D[k++] = p;
        D[k++] = h;
        D[k++] = t;
        D[k++] = h;
        this.Ad = m;
        this.Of = k
    };
    c.prototype.Cb = function(a) {
        if (this.Gg !== a) {
            if (!this.ya[a]) {
                if (0 === this.Gg) return;
                a = 0
            }
            var b = this.Mb();
            b.type = 9;
            this.Gg = b.sb = a;
            this.hb = !1
        }
    };
    c.prototype.Yg = function(a) {
        a = this.ya[a];
        return !(!a.Qe && !a.Pe)
    };
    c.prototype.Rk = function(a) {
        a = this.ya[a];
        return !!(a.Qe || a.Pe || a.dm)
    };
    c.prototype.Qk = function(a) {
        return this.ya[a].Hc
    };
    c.prototype.Zs = function(a) {
        a = this.ya[a];
        return 0 !== a.Sj || 0 !== a.Tj
    };
    c.prototype.Lr = function(a) {
        return this.ya[a].Sj
    };
    c.prototype.Mr = function(a) {
        return this.ya[a].Tj
    };
    c.prototype.Nr = function(a, b) {
        return this.ya[a].K[b][2]
    };
    c.prototype.Ji = function(a) {
        return this.ya[a].Ol
    };
    c.prototype.Lf =
        function(a, b, c, e, f, n, g, l, h, m, k, q, D, z, t) {
            var L = this.ya[this.Gg],
                p, r;
            if (L.ds || t.length) {
                p = this.Mb();
                p.type = 10;
                p.J ? Yb(this.$b, p.J) : p.J = Xb();
                r = p.J;
                r[0] = b;
                r[1] = c;
                r[2] = e;
                r[3] = f;
                r[4] = n;
                r[5] = g;
                r[6] = l;
                r[7] = h;
                r[8] = m;
                r[9] = k;
                r[10] = q;
                r[11] = D;
                r[12] = z;
                L.Ng ? p.tc = a : p.tc = null;
                if (t.length)
                    for (c = p.ko, c.length = t.length, a = 0, b = t.length; a < b; a++) c[a] = t[a];
                this.hb = !1
            }
        };
    c.prototype.clear = function(a, b, c, e) {
        var f = this.Mb();
        f.type = 7;
        f.sb = 0;
        f.J || (f.J = Xb());
        f.J[0] = a;
        f.J[1] = b;
        f.J[2] = c;
        f.J[3] = e;
        this.hb = !1
    };
    c.prototype.clearRect = function(a,
        b, c, e) {
        if (!(0 > c || 0 > e)) {
            var f = this.Mb();
            f.type = 7;
            f.sb = 1;
            f.J || (f.J = Xb());
            f.J[0] = a;
            f.J[1] = b;
            f.J[2] = c;
            f.J[3] = e;
            this.hb = !1
        }
    };
    c.prototype.co = function(a) {
        if (this.ea && (a = !!a, this.yg !== a)) {
            var b = this.Mb();
            b.type = 14;
            b.sb = a ? 1 : 0;
            this.hb = !1;
            this.yg = a;
            this.Ni = null;
            this.yg ? this.Cb(2) : this.Cb(0)
        }
    };
    c.prototype.bo = function(a) {
        if (this.ea) {
            var b = this.Mb();
            b.type = 13;
            b.sb = a ? 1 : 0;
            this.hb = !1
        }
    };
    c.prototype.qm = function() {
        Yb(this.zk, r);
        this.pc();
        this.ec();
        var a = this.width / 2,
            b = this.height / 2;
        this.ah(-a, b, a, b, a, -b, -a, -b);
        Yb(r,
            this.$b);
        this.ec()
    };
    c.prototype.ao = function(a, b, c) {
        this.Cb(3);
        var e = this.Mb();
        e.type = 12;
        e.J || (e.J = Xb());
        e.J[0] = a;
        e.J[1] = b;
        e.J[2] = c;
        e.J[3] = 1;
        this.hb = !1
    };
    c.prototype.tt = function() {
        this.Cb(0)
    };
    c.prototype.ht = function() {
        this.Cb(2)
    };
    c.prototype.Ys = function() {
        this.Rd();
        this.q.flush()
    };
    var p = [],
        b = {};
    c.prototype.dq = function() {
        S(p);
        b = {}
    };
    c.prototype.ti = function(a, d, c, e) {
        d = !!d;
        c = !!c;
        var f = a.src + "," + d + "," + c + (d ? ",undefined" : ""),
            n = null;
        if ("undefined" !== typeof a.src && b.hasOwnProperty(f)) return n = b[f], n.Bh++, n;
        this.Rd();
        var g = this.q,
            l = ha(a.width) && ha(a.height),
            n = g.createTexture();
        g.bindTexture(g.TEXTURE_2D, n);
        g.pixelStorei(g.UNPACK_PREMULTIPLY_ALPHA_WEBGL, !0);
        var h = g.RGBA,
            m = g.RGBA,
            k = g.UNSIGNED_BYTE;
        if (e && !this.Vd) switch (e) {
            case 1:
                m = h = g.RGB;
                break;
            case 2:
                k = g.UNSIGNED_SHORT_4_4_4_4;
                break;
            case 3:
                k = g.UNSIGNED_SHORT_5_5_5_1;
                break;
            case 4:
                m = h = g.RGB, k = g.UNSIGNED_SHORT_5_6_5
        }
        if (!l && d) {
            e = document.createElement("canvas");
            e.width = ia(a.width);
            e.height = ia(a.height);
            var q = e.getContext("2d");
            q.webkitImageSmoothingEnabled =
                c;
            q.mozImageSmoothingEnabled = c;
            q.msImageSmoothingEnabled = c;
            q.imageSmoothingEnabled = c;
            q.drawImage(a, 0, 0, a.width, a.height, 0, 0, e.width, e.height);
            g.texImage2D(g.TEXTURE_2D, 0, h, m, k, e)
        } else g.texImage2D(g.TEXTURE_2D, 0, h, m, k, a);
        d ? (g.texParameteri(g.TEXTURE_2D, g.TEXTURE_WRAP_S, g.REPEAT), g.texParameteri(g.TEXTURE_2D, g.TEXTURE_WRAP_T, g.REPEAT)) : (g.texParameteri(g.TEXTURE_2D, g.TEXTURE_WRAP_S, g.CLAMP_TO_EDGE), g.texParameteri(g.TEXTURE_2D, g.TEXTURE_WRAP_T, g.CLAMP_TO_EDGE));
        c ? (g.texParameteri(g.TEXTURE_2D, g.TEXTURE_MAG_FILTER,
            g.LINEAR), l && this.hm ? (g.texParameteri(g.TEXTURE_2D, g.TEXTURE_MIN_FILTER, g.LINEAR_MIPMAP_LINEAR), g.generateMipmap(g.TEXTURE_2D)) : g.texParameteri(g.TEXTURE_2D, g.TEXTURE_MIN_FILTER, g.LINEAR)) : (g.texParameteri(g.TEXTURE_2D, g.TEXTURE_MAG_FILTER, g.NEAREST), g.texParameteri(g.TEXTURE_2D, g.TEXTURE_MIN_FILTER, g.NEAREST));
        g.bindTexture(g.TEXTURE_2D, null);
        this.Zd = null;
        n.ue = a.width;
        n.te = a.height;
        n.Bh = 1;
        n.Ul = f;
        p.push(n);
        return b[f] = n
    };
    c.prototype.Vb = function(a, b, c, e) {
        this.Rd();
        var f = this.q;
        this.Vd && (e = !1);
        var n =
            f.createTexture();
        f.bindTexture(f.TEXTURE_2D, n);
        f.texImage2D(f.TEXTURE_2D, 0, f.RGBA, a, b, 0, f.RGBA, e ? f.UNSIGNED_SHORT_4_4_4_4 : f.UNSIGNED_BYTE, null);
        f.texParameteri(f.TEXTURE_2D, f.TEXTURE_WRAP_S, f.CLAMP_TO_EDGE);
        f.texParameteri(f.TEXTURE_2D, f.TEXTURE_WRAP_T, f.CLAMP_TO_EDGE);
        f.texParameteri(f.TEXTURE_2D, f.TEXTURE_MAG_FILTER, c ? f.LINEAR : f.NEAREST);
        f.texParameteri(f.TEXTURE_2D, f.TEXTURE_MIN_FILTER, c ? f.LINEAR : f.NEAREST);
        f.bindTexture(f.TEXTURE_2D, null);
        this.Zd = null;
        n.ue = a;
        n.te = b;
        p.push(n);
        return n
    };
    c.prototype.Jt =
        function(a, b, c) {
            this.Rd();
            var e = this.q;
            this.Vd && (c = !1);
            e.bindTexture(e.TEXTURE_2D, b);
            e.pixelStorei(e.UNPACK_PREMULTIPLY_ALPHA_WEBGL, !0);
            try {
                e.texImage2D(e.TEXTURE_2D, 0, e.RGBA, e.RGBA, c ? e.UNSIGNED_SHORT_4_4_4_4 : e.UNSIGNED_BYTE, a)
            } catch (f) {
                console && console.error && console.error("Error updating WebGL texture: ", f)
            }
            e.bindTexture(e.TEXTURE_2D, null);
            this.Zd = null
        };
    c.prototype.deleteTexture = function(a) {
        a && ("undefined" !== typeof a.Bh && 1 < a.Bh ? a.Bh-- : (this.Rd(), a === this.Zd && (this.q.bindTexture(this.q.TEXTURE_2D,
            null), this.Zd = null), a === this.Me && (this.q.activeTexture(this.q.TEXTURE1), this.q.bindTexture(this.q.TEXTURE_2D, null), this.q.activeTexture(this.q.TEXTURE0), this.Me = null), Aa(p, a), "undefined" !== typeof a.Ul && delete b[a.Ul], this.q.deleteTexture(a)))
    };
    c.prototype.qc = function(a) {
        if (a !== this.Ni) {
            var b = this.Mb();
            b.type = 6;
            this.Ni = b.tc = a;
            this.hb = !1
        }
    };
    eb = c
})();
(function() {
    function c(a) {
        if (a && (a.getContext || a.dc) && !a.c2runtime) {
            a.c2runtime = this;
            var e = this;
            this.od = (this.fi = /crosswalk/i.test(navigator.userAgent) || /xwalk/i.test(navigator.userAgent) || !("undefined" === typeof window.c2isCrosswalk || !window.c2isCrosswalk)) || "undefined" !== typeof window.device && ("undefined" !== typeof window.device.cordova || "undefined" !== typeof window.device.phonegap) || "undefined" !== typeof window.c2iscordova && window.c2iscordova;
            this.Ie = !!a.dc;
            this.ks = "undefined" !== typeof window.AppMobi ||
                this.Ie;
            this.Cc = !!window.c2cocoonjs;
            this.Dc = !!window.c2ejecta;
            this.Cc && (CocoonJS.App.onSuspended.addEventListener(function() {
                e.setSuspended(!0)
            }), CocoonJS.App.onActivated.addEventListener(function() {
                e.setSuspended(!1)
            }));
            this.Dc && (document.addEventListener("pagehide", function() {
                e.setSuspended(!0)
            }), document.addEventListener("pageshow", function() {
                e.setSuspended(!1)
            }), document.addEventListener("resize", function() {
                e.setSize(window.innerWidth, window.innerHeight)
            }));
            this.pb = this.Ie || this.Cc || this.Dc;
            this.gi =
                /edge\//i.test(navigator.userAgent);
            this.Vd = (/msie/i.test(navigator.userAgent) || /trident/i.test(navigator.userAgent) || /iemobile/i.test(navigator.userAgent)) && !this.gi;
            this.Qm = /tizen/i.test(navigator.userAgent);
            this.Om = /android/i.test(navigator.userAgent) && !this.Qm && !this.Vd && !this.gi;
            this.Vm = (/iphone/i.test(navigator.userAgent) || /ipod/i.test(navigator.userAgent)) && !this.Vd && !this.gi;
            this.ts = /ipad/i.test(navigator.userAgent);
            this.xk = this.Vm || this.ts || this.Dc;
            this.pk = (/chrome/i.test(navigator.userAgent) ||
                /chromium/i.test(navigator.userAgent)) && !this.Vd && !this.gi;
            this.Nm = /amazonwebappplatform/i.test(navigator.userAgent);
            this.os = /firefox/i.test(navigator.userAgent);
            this.Xd = "undefined" !== typeof window.c2nodewebkit || "undefined" !== typeof window.c2nwjs || /nodewebkit/i.test(navigator.userAgent) || /nwjs/i.test(navigator.userAgent);
            this.Tm = !("undefined" === typeof window.c2isWindows8 || !window.c2isWindows8);
            this.rs = !("undefined" === typeof window.c2isWindows8Capable || !window.c2isWindows8Capable);
            this.vf = !("undefined" ===
                typeof window.c2isWindowsPhone8 || !window.c2isWindowsPhone8);
            this.vk = !("undefined" === typeof window.c2isWindowsPhone81 || !window.c2isWindowsPhone81);
            this.Sm = !!window.cr_windows10;
            this.Rm = this.Tm || this.rs || this.vk || this.Sm;
            this.ls = !("undefined" === typeof window.c2isBlackberry10 || !window.c2isBlackberry10);
            this.Pm = this.Om && !this.pk && !this.fi && !this.os && !this.Nm && !this.pb;
            this.devicePixelRatio = 1;
            this.Wd = this.od || this.fi || this.ks || this.Cc || this.Om || this.xk || this.vf || this.vk || this.ls || this.Qm || this.Dc;
            this.Wd ||
                (this.Wd = /(blackberry|bb10|playbook|palm|symbian|nokia|windows\s+ce|phone|mobile|tablet|kindle|silk)/i.test(navigator.userAgent));
            this.uk = !!(this.xk && this.od && window.indexedDB);
            this.Fm = null;
            this.es = "";
            this.uk && (this.Fm = cordova && cordova.plugins && cordova.plugins.CorHttpd ? cordova.plugins.CorHttpd : null);
            "undefined" === typeof cr_is_preview || this.Xd || "?nw" !== window.location.search && !/nodewebkit/i.test(navigator.userAgent) && !/nwjs/i.test(navigator.userAgent) || (this.Xd = !0);
            this.canvas = a;
            this.Yl = document.getElementById("c2canvasdiv");
            this.n = this.q = null;
            this.fk = "(unavailable)";
            this.ea = !1;
            this.ye = 0;
            this.X = null;
            this.Xj = "";
            this.Qh = !1;
            this.Jn = this.Kn = 0;
            this.canvas.oncontextmenu = function(a) {
                a.preventDefault && a.preventDefault();
                return !1
            };
            this.canvas.onselectstart = function(a) {
                a.preventDefault && a.preventDefault();
                return !1
            };
            this.Ie && (window.c2runtime = this);
            this.Xd && (window.ondragover = function(a) {
                a.preventDefault();
                return !1
            }, window.ondrop = function(a) {
                a.preventDefault();
                return !1
            }, window.nwgui && window.nwgui.App.clearCache && window.nwgui.App.clearCache());
            this.Pm && "undefined" !== typeof jQuery && jQuery("canvas").parents("*").css("overflow", "visible");
            this.width = a.width;
            this.height = a.height;
            this.B = this.width;
            this.A = this.height;
            this.Gh = this.width;
            this.eg = this.height;
            this.Ig = window.innerWidth;
            this.Hg = window.innerHeight;
            this.xr = !1;
            this.T = !0;
            this.Cg = !1;
            Date.now || (Date.now = function() {
                return +new Date
            });
            this.plugins = [];
            this.types = {};
            this.p = [];
            this.ta = [];
            this.Bk = {};
            this.Fc = [];
            this.Rj = {};
            this.ld = [];
            this.Uf = [];
            this.ej = [];
            this.Sp = [];
            this.Tp = [];
            this.ro = null;
            this.we = {};
            this.rk = this.Ud = !1;
            this.Zb = 0;
            this.qk = this.tk = !1;
            this.Ac = [];
            this.zg = !1;
            this.qd = this.ib = this.ri = this.al = "";
            this.kh = this.oo = !1;
            this.Ph = [];
            this.Qd = this.pf = 0;
            this.An = 30;
            this.Ej = this.Rg = 0;
            this.gf = 1;
            this.Jb = new Ua;
            this.kj = new Ua;
            this.Fi = this.Uh = this.Be = this.ed = this.Oe = this.Wj = this.ki = 0;
            this.$f = null;
            this.Nj = [];
            this.Qj = [];
            this.Mh = -1;
            this.Hk = [
                []
            ];
            this.ql = this.xi = 0;
            this.Li(null);
            this.lg = this.En = this.Tg = 0;
            this.Xi = [];
            this.nl = this.Tk = -1;
            this.Eg = !0;
            this.vi = 0;
            this.Bg = !1;
            this.yt = 0;
            this.Zf = null;
            this.jc = this.Bm = !1;
            this.Hn = new ca;
            this.Jk = new ca;
            this.In = new ca;
            this.eh = [];
            this.Db = new Wa([]);
            this.il = new Wa([]);
            this.Hd = [];
            this.$h = {};
            this.Ld = {};
            this.Gd = {};
            this.Tf = {};
            this.Rl = {};
            this.an = this.qi = this.Da = this.Ia = this.$m = this.pi = this.R = null;
            this.Rf = this.wk = !1;
            this.Yj = [null, null];
            this.Ce = 0;
            this.td = {};
            this.jh = this.ae = null;
            this.po = "";
            this.Ei = [];
            this.gt()
        }
    }

    function l(a, e) {
        return 128 >= e ? a[3] : 256 >= e ? a[2] : 512 >= e ? a[1] : a[0]
    }

    function h() {
        try {
            return !!window.indexedDB
        } catch (a) {
            return !1
        }
    }

    function m(a) {
        a.target.result.createObjectStore("saves", {
            keyPath: "slot"
        })
    }

    function r(a, e, b, f) {
        try {
            var d = indexedDB.open("_C2SaveStates");
            d.onupgradeneeded = m;
            d.onerror = f;
            d.onsuccess = function(d) {
                d = d.target.result;
                d.onerror = f;
                d.transaction(["saves"], "readwrite").objectStore("saves").put({
                    slot: a,
                    data: e
                }).onsuccess = b
            }
        } catch (c) {
            f(c)
        }
    }

    function p(a, e, b) {
        try {
            var f = indexedDB.open("_C2SaveStates");
            f.onupgradeneeded = m;
            f.onerror = b;
            f.onsuccess = function(f) {
                f = f.target.result;
                f.onerror = b;
                var d = f.transaction(["saves"]).objectStore("saves").get(a);
                d.onsuccess = function() {
                    d.result ?
                        e(d.result.data) : e(null)
                }
            }
        } catch (d) {
            b(d)
        }
    }

    function b() {
        x("Reloading for continuous preview");
        window.c2cocoonjs ? CocoonJS.App.reload() : -1 < window.location.search.indexOf("continuous") ? window.location.reload(!0) : window.location = window.location + "?continuous"
    }

    function a(a) {
        var e, b = {};
        for (e in a) !a.hasOwnProperty(e) || a[e] instanceof ca || a[e] && "undefined" !== typeof a[e].Ku || "spriteCreatedDestroyCallback" !== e && (b[e] = a[e]);
        return b
    }
    var d = window.requestAnimationFrame || window.mozRequestAnimationFrame || window.webkitRequestAnimationFrame ||
        window.msRequestAnimationFrame || window.oRequestAnimationFrame;
    c.prototype.gt = function() {
        var a = this;
        if (this.uk) this.Fm.startServer({
            port: 0,
            localhost_only: !0
        }, function(e) {
            a.es = e;
            a.sr(function(e) {
                a.Kg(JSON.parse(e))
            }, function() {
                alert("Error fetching data.js")
            })
        }, function(a) {
            alert("error starting local server: " + a)
        });
        else {
            var e;
            this.vf ? e = new ActiveXObject("Microsoft.XMLHTTP") : e = new XMLHttpRequest;
            var b = "data.js";
            if (this.Tm || this.vf || this.vk || this.Sm) b = "data.json";
            e.open("GET", b, !0);
            var f = !1;
            if (!this.pb &&
                "response" in e && "responseType" in e) try {
                e.responseType = "json", f = "json" === e.responseType
            } catch (d) {
                f = !1
            }
            if (!f && "responseType" in e) try {
                e.responseType = "text"
            } catch (c) {}
            if ("overrideMimeType" in e) try {
                e.overrideMimeType("application/json; charset=utf-8")
            } catch (g) {}
            this.vf ? e.onreadystatechange = function() {
                4 === e.readyState && a.Kg(JSON.parse(e.responseText))
            } : (e.onload = function() {
                    if (f) a.Kg(e.response);
                    else if (a.Dc) {
                        var b = e.responseText,
                            b = b.substr(b.indexOf("{"));
                        a.Kg(JSON.parse(b))
                    } else a.Kg(JSON.parse(e.responseText))
                },
                e.onerror = function(a) {
                    ea("Error requesting " + b + ":");
                    ea(a)
                });
            e.send()
        }
    };
    c.prototype.js = function() {
        var a = this,
            e, b, f, d, c, g, k, n, q;
        this.Je = (!this.pb || this.Dc || this.od) && this.Ht && !this.Pm;
        0 === this.xb && this.xk && (this.Je = !1);
        this.devicePixelRatio = this.Je ? window.devicePixelRatio || window.webkitDevicePixelRatio || window.mozDevicePixelRatio || window.msDevicePixelRatio || 1 : 1;
        this.Ya();
        b = !(!this.xr && (!this.pj || this.Xd || this.Rm || this.vf || this.fi || this.od || this.Nm));
        0 < this.xb && this.setSize(window.innerWidth, window.innerHeight, !0);
        try {
            this.Kq && (this.Cc || this.Dc || !this.pb) && (e = {
                alpha: b,
                depth: !1,
                antialias: !1,
                failIfMajorPerformanceCaveat: !0
            }, this.q = this.canvas.getContext("webgl", e) || this.canvas.getContext("experimental-webgl", e))
        } catch (l) {}
        if (this.q) {
            if (e = this.q.getExtension("WEBGL_debug_renderer_info")) this.fk = this.q.getParameter(e.UNMASKED_RENDERER_WEBGL) + " [" + this.q.getParameter(e.UNMASKED_VENDOR_WEBGL) + "]";
            this.ea && (this.fk += " [front-to-back enabled]");
            this.pb || (this.Wa = document.createElement("canvas"), jQuery(this.Wa).appendTo(this.canvas.parentNode),
                this.Wa.oncontextmenu = function() {
                    return !1
                }, this.Wa.onselectstart = function() {
                    return !1
                }, this.Wa.width = Math.round(this.Gh * this.devicePixelRatio), this.Wa.height = Math.round(this.eg * this.devicePixelRatio), jQuery(this.Wa).css({
                    width: this.Gh + "px",
                    height: this.eg + "px"
                }), this.Rn(), this.Mk = this.Wa.getContext("2d"));
            this.n = new eb(this.q, this.Wd, this.ea);
            this.n.Ze(this.canvas.width, this.canvas.height);
            this.n.hm = 0 !== this.Dq;
            this.X = null;
            this.canvas.addEventListener("webglcontextlost", function(e) {
                e.preventDefault();
                a.Es();
                x("[Construct 2] WebGL context lost");
                window.cr_setSuspended(!0)
            }, !1);
            this.canvas.addEventListener("webglcontextrestored", function() {
                a.n.Hm();
                a.n.Ze(a.n.width, a.n.height, !0);
                a.Ia = null;
                a.Da = null;
                a.Yj[0] = null;
                a.Yj[1] = null;
                a.Fs();
                a.T = !0;
                x("[Construct 2] WebGL context restored");
                window.cr_setSuspended(!1)
            }, !1);
            e = 0;
            for (b = this.p.length; e < b; e++)
                for (c = this.p[e], f = 0, d = c.G.length; f < d; f++) k = c.G[f], k.Fa = this.n.ek(k.id), k.Hc = this.n.Qk(k.Fa), this.Rf = this.Rf || this.n.Yg(k.Fa);
            e = 0;
            for (b = this.Fc.length; e < b; e++) {
                n =
                    this.Fc[e];
                f = 0;
                for (d = n.G.length; f < d; f++) k = n.G[f], k.Fa = this.n.ek(k.id), k.Hc = this.n.Qk(k.Fa);
                n.Mc();
                f = 0;
                for (d = n.I.length; f < d; f++) {
                    q = n.I[f];
                    c = 0;
                    for (g = q.G.length; c < g; c++) k = q.G[c], k.Fa = this.n.ek(k.id), k.Hc = this.n.Qk(k.Fa), this.Rf = this.Rf || this.n.Yg(k.Fa);
                    q.Mc()
                }
            }
        } else {
            if (0 < this.xb && this.Ie) {
                this.canvas = null;
                document.oncontextmenu = function() {
                    return !1
                };
                document.onselectstart = function() {
                    return !1
                };
                this.X = AppMobi.canvas.getContext("2d");
                try {
                    this.X.samplingMode = this.N ? "smooth" : "sharp", this.X.globalScale = 1, this.X.HTML5CompatibilityMode = !0, this.X.imageSmoothingEnabled = this.N
                } catch (h) {}
                0 !== this.width && 0 !== this.height && (this.X.width = this.width, this.X.height = this.height)
            }
            this.X || (this.Cc ? (e = {
                antialias: !!this.N,
                alpha: b
            }, this.X = this.canvas.getContext("2d", e)) : (e = {
                alpha: b
            }, this.X = this.canvas.getContext("2d", e)), this.X.webkitImageSmoothingEnabled = this.N, this.X.mozImageSmoothingEnabled = this.N, this.X.msImageSmoothingEnabled = this.N, this.X.imageSmoothingEnabled = this.N);
            this.Mk = this.Wa = null
        }
        this.xo = function(e) {
            a.cc(!1, e)
        };
        window == window.top ||
            this.pb || this.Rm || this.vf || (document.addEventListener("mousedown", function() {
                window.focus()
            }, !0), document.addEventListener("touchstart", function() {
                window.focus()
            }, !0));
        "undefined" !== typeof cr_is_preview && (this.Cc && console.log("[Construct 2] In preview-over-wifi via CocoonJS mode"), -1 < window.location.search.indexOf("continuous") && (x("Reloading for continuous preview"), this.ri = "__c2_continuouspreview", this.kh = !0), this.Rs && !this.Wd && (jQuery(window).focus(function() {
            a.setSuspended(!1)
        }), jQuery(window).blur(function() {
            a.setSuspended(!0)
        })));
        window.addEventListener("blur", function() {
            a.If()
        });
        this.pb || (e = function(a) {
            if (db(a) && document.activeElement && document.activeElement !== document.getElementsByTagName("body")[0] && document.activeElement.blur) try {
                document.activeElement.blur()
            } catch (e) {}
        }, window.navigator.pointerEnabled ? document.addEventListener("pointerdown", e) : window.navigator.msPointerEnabled ? document.addEventListener("MSPointerDown", e) : document.addEventListener("touchstart", e), document.addEventListener("mousedown", e));
        0 === this.xb &&
            this.Je && 1 < this.devicePixelRatio && this.setSize(this.Ka, this.Ja, !0);
        this.Do();
        this.$r();
        this.go();
        this.u = {}
    };
    c.prototype.setSize = function(a, e, b) {
        var f = 0,
            d = 0,
            c = 0,
            g = 0,
            g = 0;
        if (this.Ig !== a || this.Hg !== e || b) {
            this.Ig = a;
            this.Hg = e;
            var k = this.xb,
                n = (document.mozFullScreen || document.webkitIsFullScreen || !!document.msFullscreenElement || document.fullScreen || this.Bg) && !this.od;
            if (n || 0 !== this.xb || b) n && 0 < this.Ce && (k = this.Ce), b = this.devicePixelRatio, 4 <= k ? (c = this.Ka / this.Ja, a / e > c ? (c *= e, 5 === k ? (g = c * b / this.Ka, 1 < g ? g = Math.floor(g) :
                    1 > g && (g = 1 / Math.ceil(1 / g)), c = this.Ka * g / b, g = this.Ja * g / b, f = (a - c) / 2, d = (e - g) / 2, a = c, e = g) : (f = (a - c) / 2, a = c)) : (g = a / c, 5 === k ? (g = g * b / this.Ja, 1 < g ? g = Math.floor(g) : 1 > g && (g = 1 / Math.ceil(1 / g)), c = this.Ka * g / b, g = this.Ja * g / b, f = (a - c) / 2, d = (e - g) / 2, a = c) : d = (e - g) / 2, e = g), n && !this.Xd && (d = f = 0)) : this.Xd && this.Bg && 0 === this.rm && (f = Math.floor((a - this.Ka) / 2), d = Math.floor((e - this.Ja) / 2), a = this.Ka, e = this.Ja), 2 > k && (this.Yf = b), this.Gh = Math.round(a), this.eg = Math.round(e), this.width = Math.round(a * b), this.height = Math.round(e * b), this.T = !0, this.Oo ?
                (this.B = this.width, this.A = this.height, this.Wb = !0) : this.width < this.Ka && this.height < this.Ja || 1 === k ? (this.B = this.width, this.A = this.height, this.Wb = !0) : (this.B = this.Ka, this.A = this.Ja, this.Wb = !1, 2 === k ? (c = this.Ka / this.Ja, k = this.Ig / this.Hg, k < c ? this.B = this.A * k : k > c && (this.A = this.B / k)) : 3 === k && (c = this.Ka / this.Ja, k = this.Ig / this.Hg, k > c ? this.B = this.A * k : k < c && (this.A = this.B / k))), this.Yl && !this.pb && (jQuery(this.Yl).css({
                    width: Math.round(a) + "px",
                    height: Math.round(e) + "px",
                    "margin-left": Math.floor(f) + "px",
                    "margin-top": Math.floor(d) +
                        "px"
                }), "undefined" !== typeof cr_is_preview && jQuery("#borderwrap").css({
                    width: Math.round(a) + "px",
                    height: Math.round(e) + "px"
                })), this.canvas && (this.canvas.width = Math.round(a * b), this.canvas.height = Math.round(e * b), this.Dc ? (this.canvas.style.left = Math.floor(f) + "px", this.canvas.style.top = Math.floor(d) + "px", this.canvas.style.width = Math.round(a) + "px", this.canvas.style.height = Math.round(e) + "px") : this.Je && !this.pb && (this.canvas.style.width = Math.round(a) + "px", this.canvas.style.height = Math.round(e) + "px")), this.Wa &&
                (this.Wa.width = Math.round(a * b), this.Wa.height = Math.round(e * b), this.Wa.style.width = this.Gh + "px", this.Wa.style.height = this.eg + "px"), this.n && this.n.Ze(Math.round(a * b), Math.round(e * b)), this.Ie && this.X && (this.X.width = Math.round(a), this.X.height = Math.round(e)), this.X && (this.X.webkitImageSmoothingEnabled = this.N, this.X.mozImageSmoothingEnabled = this.N, this.X.msImageSmoothingEnabled = this.N, this.X.imageSmoothingEnabled = this.N), this.Do(), this.Vm && !this.od && window.scrollTo(0, 0)
        }
    };
    c.prototype.Do = function() {
        if (this.Vp &&
            0 !== this.Lk) {
            var a = "portrait";
            2 === this.Lk && (a = "landscape");
            try {
                screen.orientation && screen.orientation.lock ? screen.orientation.lock(a) : screen.lockOrientation ? screen.lockOrientation(a) : screen.webkitLockOrientation ? screen.webkitLockOrientation(a) : screen.mozLockOrientation ? screen.mozLockOrientation(a) : screen.msLockOrientation && screen.msLockOrientation(a)
            } catch (e) {
                console && console.warn && console.warn("Failed to lock orientation: ", e)
            }
        }
    };
    c.prototype.Es = function() {
        this.n.dq();
        this.wk = !0;
        var a, e, b;
        a = 0;
        for (e =
            this.p.length; a < e; a++) b = this.p[a], b.Gi && b.Gi()
    };
    c.prototype.Fs = function() {
        this.wk = !1;
        var a, e, b;
        a = 0;
        for (e = this.p.length; a < e; a++) b = this.p[a], b.Kk && b.Kk()
    };
    c.prototype.Rn = function() {
        if (!this.pb) {
            var a = (document.mozFullScreen || document.webkitIsFullScreen || document.fullScreen || document.msFullscreenElement || this.Bg) && !this.od ? jQuery(this.canvas).offset() : jQuery(this.canvas).position();
            a.position = "absolute";
            jQuery(this.Wa).css(a)
        }
    };
    var u = window.cancelAnimationFrame || window.mozCancelAnimationFrame || window.webkitCancelAnimationFrame ||
        window.msCancelAnimationFrame || window.oCancelAnimationFrame;
    c.prototype.setSuspended = function(a) {
        var e;
        if (a && !this.Cg)
            for (x("[Construct 2] Suspending"), this.Cg = !0, -1 !== this.Tk && u && u(this.Tk), -1 !== this.nl && clearTimeout(this.nl), a = 0, e = this.Xi.length; a < e; a++) this.Xi[a](!0);
        else if (!a && this.Cg) {
            x("[Construct 2] Resuming");
            this.Cg = !1;
            this.ki = Qa();
            this.Oe = Qa();
            a = this.Rg = this.Uh = 0;
            for (e = this.Xi.length; a < e; a++) this.Xi[a](!1);
            this.cc(!1)
        }
    };
    c.prototype.Dd = function(a) {
        return this.Ei[a]
    };
    c.prototype.Kg = function(a) {
        a &&
            a.project || ea("Project model unavailable");
        a = a.project;
        this.name = a[0];
        this.pm = a[1];
        this.xb = a[12];
        this.rm = a[12];
        this.Ka = a[10];
        this.Ja = a[11];
        this.Mn = this.Ka / 2;
        this.Nn = this.Ja / 2;
        this.pb && !this.Dc && (4 <= a[12] || 0 === a[12]) && (x("[Construct 2] Letterbox scale fullscreen modes are not supported on this platform - falling back to 'Scale outer'"), this.rm = this.xb = 3);
        this.yl = a[18];
        this.be = a[19];
        if (0 === this.be) {
            var e = new Image;
            e.crossOrigin = "anonymous";
            this.fo(e, "loading-logo.png");
            this.ae = {
                yi: e
            }
        } else if (4 === this.be) {
            e =
                new Image;
            e.src = "data:image/png;base64,";
            var b = new Image;
            b.src = "data:image/png;base64,";
            var f = new Image;
            f.src = "data:image/png;base64,";
            var d = new Image;
            d.src = "data:image/png;base64,";
            var c = new Image;
            c.src = "data:image/png;base64,";
            var g = new Image;
            g.src = "data:image/png;base64,";
            var k = new Image;
            k.src = "data:image/png;base64,";
            var n = new Image;
            n.src = "data:image/png;base64,";
            var q = new Image;
            q.src = "data:image/png;base64,";
            var l = new Image;
            l.src = "data:image/png;base64,";
            var h = new Image;
            h.src = "data:image/png;base64,";
            var y = new Image;
            y.src = "data:image/png;base64,";
            this.ae = {
                yi: [e, b, f, d],
                Vs: [c, g, k, n],
                Kt: [q, l, h, y]
            }
        }
        this.Tg = a[21];
        this.Ei = bc();
        this.dd = new X(this);
        e = 0;
        for (b = a[2].length; e < b; e++) k = a[2][e], f = this.Dd(k[0]), fb(k, f.prototype), n = new f(this), n.Vi = k[1], n.Yd = k[2], n.Cn = k[9], n.S && n.S(), this.plugins.push(n);
        this.Ei = bc();
        e = 0;
        for (b = a[3].length; e < b; e++) {
            k = a[3][e];
            c = this.Dd(k[1]);
            n = null;
            f = 0;
            for (d = this.plugins.length; f < d; f++)
                if (this.plugins[f] instanceof c) {
                    n = this.plugins[f];
                    break
                }
            q = new n.qa(n);
            q.name = k[0];
            q.D = k[2];
            q.nk = k[3].slice(0);
            q.It = k[3].length;
            q.Wp = k[4];
            q.yr =
                k[5];
            q.L = k[11];
            q.D ? (q.Cf = [], q.md = this.lg++, q.la = null) : (q.Cf = null, q.md = -1, q.la = []);
            q.Oh = null;
            q.rf = null;
            q.jm = null;
            q.qb = !1;
            q.Eb = null;
            k[6] ? (q.kl = k[6][0], q.ll = k[6][1], q.ml = k[6][2]) : (q.kl = null, q.ll = 0, q.ml = 0);
            k[7] ? q.wb = k[7] : q.wb = null;
            q.index = e;
            q.d = [];
            q.Jh = [];
            q.wd = [new gb(q)];
            q.Sc = 0;
            q.ic = null;
            q.jq = 0;
            q.Nf = !0;
            q.fj = hb;
            q.Fr = ib;
            q.Kr = jb;
            q.P = kb;
            q.$g = lb;
            q.Ki = mb;
            q.We = nb;
            q.Wh = ob;
            q.Zj = pb;
            q.ak = qb;
            q.wc = rb;
            q.bk = sb;
            q.Eh = new Xa(this.Ka, this.Ja);
            q.sh = !0;
            q.uh = !1;
            q.u = {};
            q.toString = tb;
            q.ta = [];
            f = 0;
            for (d = k[8].length; f < d; f++) {
                l =
                    k[8][f];
                h = this.Dd(l[1]);
                y = null;
                c = 0;
                for (g = this.ta.length; c < g; c++)
                    if (this.ta[c] instanceof h) {
                        y = this.ta[c];
                        break
                    }
                y || (y = new h(this), y.Ik = [], y.Di = new ca, y.S && y.S(), this.ta.push(y), cc && y instanceof cc && (this.ro = y)); - 1 === y.Ik.indexOf(q) && y.Ik.push(q);
                c = new y.qa(y, q);
                c.name = l[0];
                c.L = l[2];
                c.S();
                q.ta.push(c)
            }
            q.global = k[9];
            q.sk = k[10];
            q.G = [];
            f = 0;
            for (d = k[12].length; f < d; f++) q.G.push({
                id: k[12][f][0],
                name: k[12][f][1],
                Fa: -1,
                Hc: !1,
                Tb: !0,
                index: f
            });
            q.Tu = k[13];
            this.yl && !q.D && !q.sk && n.Yd || q.S();
            q.name && (this.types[q.name] =
                q);
            this.p.push(q);
            n.Vi && (f = new n.ca(q), f.uid = this.Tg++, f.Un = this.En++, f.Ee = 0, f.rg = ub, f.toString = vb, f.H = k[14], f.S(), q.d.push(f), this.td[f.uid.toString()] = f)
        }
        e = 0;
        for (b = a[4].length; e < b; e++)
            for (c = a[4][e], g = this.p[c[0]], f = 1, d = c.length; f < d; f++) k = this.p[c[f]], k.la.push(g), g.Cf.push(k);
        e = 0;
        for (b = a[28].length; e < b; e++) {
            c = a[28][e];
            g = [];
            f = 0;
            for (d = c.length; f < d; f++) g.push(this.p[c[f]]);
            f = 0;
            for (d = g.length; f < d; f++) g[f].qb = !0, g[f].Eb = g
        }
        if (0 < this.lg)
            for (e = 0, b = this.p.length; e < b; e++)
                if (k = this.p[e], !k.D && k.la.length) {
                    k.Oh =
                        Array(this.lg);
                    k.rf = Array(this.lg);
                    k.jm = Array(this.lg);
                    q = [];
                    f = y = h = l = 0;
                    for (d = k.la.length; f < d; f++)
                        for (n = k.la[f], k.Oh[n.md] = l, l += n.It, k.rf[n.md] = h, h += n.Wp, k.jm[n.md] = y, y += n.yr, c = 0, g = n.G.length; c < g; c++) q.push(ra({}, n.G[c]));
                    k.G = q.concat(k.G);
                    f = 0;
                    for (d = k.G.length; f < d; f++) k.G[f].index = f
                }
        e = 0;
        for (b = a[5].length; e < b; e++) k = a[5][e], f = new wb(this, k), this.Bk[f.name] = f, this.Fc.push(f);
        e = 0;
        for (b = a[6].length; e < b; e++) k = a[6][e], f = new xb(this, k), this.Rj[f.name] = f, this.ld.push(f);
        e = 0;
        for (b = this.ld.length; e < b; e++) this.ld[e].za();
        e = 0;
        for (b = this.ld.length; e < b; e++) this.ld[e].ul();
        e = 0;
        for (b = this.ej.length; e < b; e++) this.ej[e].za();
        S(this.ej);
        this.Up = a[7];
        this.Gc = a[9];
        this.Yf = 1;
        this.Kq = a[13];
        this.N = a[14];
        this.pj = a[15];
        this.Ht = a[17];
        this.Lk = a[20];
        this.Vp = 0 < this.Lk;
        this.Rs = a[22];
        this.Wb = this.Oo = a[23];
        this.Dq = a[24];
        this.Ws = a[25];
        this.ea = a[27] && !this.Vd;
        this.Wi = Date.now();
        S(this.Ei);
        this.js()
    };
    var e = !1;
    c.prototype.No = function(a, f) {
        a.cocoonLazyLoad = !0;
        a.onerror = function(f) {
            e = a.Tl = !0;
            console && console.error && console.error("Error loading image '" +
                a.src + "': ", f)
        };
        this.Dc ? a.src = f : a.src || ("undefined" !== typeof XAPKReader ? XAPKReader.get(f, function(e) {
            a.src = e
        }, function(b) {
            e = a.Tl = !0;
            console && console.error && console.error("Error extracting image '" + f + "' from expansion file: ", b)
        }) : (a.crossOrigin = "anonymous", this.fo(a, f)));
        this.Uf.push(a)
    };
    c.prototype.ur = function(a) {
        var e, f;
        e = 0;
        for (f = this.Uf.length; e < f; e++)
            if (this.Uf[e].gq === a) return this.Uf[e];
        return null
    };
    var f = 0,
        n = !1;
    c.prototype.$r = function() {
        this.Zf && (f = this.Zf.Qu(this.Up))
    };
    c.prototype.Pl = function() {
        var a =
            f,
            e = 0,
            b = 0,
            d = !0,
            c, g, b = 0;
        for (c = this.Uf.length; b < c; b++) {
            g = this.Uf[b];
            var k = g.cm;
            if (!k || 0 >= k) k = 5E4;
            a += k;
            g.src && (g.complete || g.loaded) && !g.Tl ? e += k : d = !1
        }
        d && this.Ws && this.Zf && (n || (this.Zf.Ru(), n = !0), b = this.Zf.Mu(), e += b, b < f && (d = !1));
        this.Ic = 0 == a ? 1 : e / a;
        return d
    };
    var g = !1;
    c.prototype.go = function() {
        if (this.X || this.n) {
            var a = this.X || this.Mk;
            this.Wa && this.Rn();
            this.Ic = 0;
            this.Zm = -1;
            var f = this;
            if (this.Pl() && (4 !== this.be || g)) this.as();
            else {
                var b = Date.now() - this.Wi;
                if (a) {
                    var c = this.width,
                        k = this.height,
                        n = this.devicePixelRatio;
                    if (3 > this.be && (this.Cc || 500 <= b && this.Zm != this.Ic)) {
                        a.clearRect(0, 0, c, k);
                        var b = c / 2,
                            k = k / 2,
                            c = 0 === this.be && this.ae.yi.complete,
                            q = 40 * n,
                            l = 0,
                            h = 80 * n,
                            y;
                        if (c) {
                            var t = this.ae.yi,
                                h = t.width * n;
                            y = t.height * n;
                            q = h / 2;
                            l = y / 2;
                            a.drawImage(t, N(b - q), N(k - l), h, y)
                        }
                        1 >= this.be ? (b = N(b - q) + .5, k = N(k + (l + (c ? 12 * n : 0))) + .5, a.fillStyle = e ? "red" : "DodgerBlue", a.fillRect(b, k, Math.floor(h * this.Ic), 6 * n), a.strokeStyle = "black", a.strokeRect(b, k, h, 6 * n), a.strokeStyle = "white", a.strokeRect(b - 1 * n, k - 1 * n, h + 2 * n, 8 * n)) : 2 === this.be && (a.font = this.Dc ? "12pt ArialMT" :
                            "12pt Arial", a.fillStyle = e ? "#f00" : "#999", a.Su = "middle", n = Math.round(100 * this.Ic) + "%", c = a.measureText ? a.measureText(n) : null, a.fillText(n, b - (c ? c.width : 0) / 2, k));
                        this.Zm = this.Ic
                    } else if (4 === this.be) {
                        this.Iq(a);
                        d ? d(function() {
                            f.go()
                        }) : setTimeout(function() {
                            f.go()
                        }, 16);
                        return
                    }
                }
                setTimeout(function() {
                    f.go()
                }, this.Cc ? 10 : 100)
            }
        }
    };
    var y = -1,
        F = "undefined" === typeof cr_is_preview ? 200 : 0,
        I = !0,
        k = !1,
        q = 0,
        D = 0,
        z = "undefined" === typeof cr_is_preview ? 3E3 : 0,
        t = null,
        L = null,
        V = 0;
    c.prototype.Iq = function(a) {
        if (!g) {
            for (var b = Math.ceil(this.width),
                    f = Math.ceil(this.height), d = this.ae.yi, c = this.ae.Vs, n = this.ae.Kt, h = 0; 4 > h; ++h)
                if (!d[h].complete || !c[h].complete || !n[h].complete) return;
            0 === V && (y = Date.now());
            var h = Date.now(),
                u = !1,
                m = a,
                p, r;
            I || k ? (a.clearRect(0, 0, b, f), t && t.width === b && t.height === f || (t = document.createElement("canvas"), t.width = b, t.height = f, L = t.getContext("2d")), m = L, u = !0, I && 1 === V && (y = Date.now())) : a.globalAlpha = 1;
            m.fillStyle = "#333333";
            m.fillRect(0, 0, b, f);
            256 < this.eg ? (p = Ba(.22 * f, 105, .6 * b), r = .25 * p, m.drawImage(l(c, p), .5 * b - p / 2, .2 * f - r / 2, p, r), r = p =
                Math.min(.395 * f, .95 * b), m.drawImage(l(d, p), .5 * b - p / 2, .485 * f - r / 2, p, r), p = Ba(.22 * f, 105, .6 * b), r = .25 * p, m.drawImage(l(n, p), .5 * b - p / 2, .868 * f - r / 2, p, r), m.fillStyle = "#3C3C3C", p = b, r = Math.max(.005 * f, 2), m.fillRect(0, .8 * f - r / 2, p, r), m.fillStyle = e ? "red" : "#E0FF65", p = b * this.Ic, m.fillRect(.5 * b - p / 2, .8 * f - r / 2, p, r)) : (r = p = .55 * f, m.drawImage(l(d, p), .5 * b - p / 2, .45 * f - r / 2, p, r), m.fillStyle = "#3C3C3C", p = b, r = Math.max(.005 * f, 2), m.fillRect(0, .85 * f - r / 2, p, r), m.fillStyle = e ? "red" : "#E0FF65", p = b * this.Ic, m.fillRect(.5 * b - p / 2, .85 * f - r / 2, p, r));
            u &&
                (I ? a.globalAlpha = 0 === V ? 0 : Math.min((h - y) / 300, 1) : k && (a.globalAlpha = Math.max(1 - (h - D) / 300, 0)), a.drawImage(t, 0, 0, b, f));
            I && 300 <= h - y && 2 <= V && (I = !1, q = h);
            !I && h - q >= z && !k && 1 <= this.Ic && (k = !0, D = h);
            if (k && h - D >= 300 + F || "undefined" !== typeof cr_is_preview && 1 <= this.Ic && 500 > Date.now() - y) g = !0, k = I = !1, this.ae = L = t = null;
            ++V
        }
    };
    c.prototype.as = function() {
        this.Wa && (this.canvas.parentNode.removeChild(this.Wa), this.Wa = this.Mk = null);
        this.Wi = Date.now();
        this.Oe = Qa();
        var a, e, b;
        if (this.yl)
            for (a = 0, e = this.p.length; a < e; a++) b = this.p[a], b.D ||
                b.sk || !b.fa.Yd || b.S();
        else this.Eg = !1;
        a = 0;
        for (e = this.Fc.length; a < e; a++) this.Fc[a].hq();
        2 <= this.xb && (a = this.Ka / this.Ja, e = this.width / this.height, this.Yf = 2 !== this.xb && e > a || 2 === this.xb && e < a ? this.height / this.Ja : this.width / this.Ka);
        this.pm ? this.Bk[this.pm].hl() : this.Fc[0].hl();
        this.yl || (this.vi = 1, this.trigger(X.prototype.m.Hl, null));
        navigator.splashscreen && navigator.splashscreen.hide && navigator.splashscreen.hide();
        a = 0;
        for (e = this.p.length; a < e; a++) b = this.p[a], b.Ds && b.Ds();
        document.hidden || document.webkitHidden ||
            document.mozHidden || document.msHidden ? window.cr_setSuspended(!0) : this.cc(!1);
        this.Ie && AppMobi.webview.execute("onGameReady();")
    };
    c.prototype.cc = function(a, e, b) {
        if (this.R) {
            var f = Qa();
            if (b || !this.Cg || a) {
                a || (d ? this.Tk = d(this.xo) : this.nl = setTimeout(this.xo, this.Wd ? 1 : 16));
                e = e || f;
                var c = this.xb;
                ((b = (document.mozFullScreen || document.webkitIsFullScreen || document.fullScreen || !!document.msFullscreenElement) && !this.od) || this.Bg) && 0 < this.Ce && (c = this.Ce);
                if (0 < c) {
                    var c = window.innerWidth,
                        g = window.innerHeight;
                    this.Ig ===
                        c && this.Hg === g || this.setSize(c, g)
                }
                this.pb || (b ? (this.Qh || (this.Xj = jQuery(this.canvas).css("margin") || "0", this.Qh = !0), this.pk || this.Xd || jQuery(this.canvas).css({
                    "margin-left": "" + Math.floor((screen.width - this.width / this.devicePixelRatio) / 2) + "px",
                    "margin-top": "" + Math.floor((screen.height - this.height / this.devicePixelRatio) / 2) + "px"
                })) : this.Qh ? (this.pk || this.Xd || jQuery(this.canvas).css("margin", this.Xj), this.Xj = "", this.Qh = !1, 0 === this.xb && this.setSize(Math.round(this.Kn / this.devicePixelRatio), Math.round(this.Jn /
                    this.devicePixelRatio), !0)) : (this.Kn = this.width, this.Jn = this.height));
                this.Eg && (b = this.Pl(), this.vi = this.Ic, b && (this.Eg = !1, this.Ic = 1, this.trigger(X.prototype.m.Hl, null)));
                this.ys(e);
                !this.T && !this.Cc || this.wk || this.kh || a || (this.T = !1, this.n ? this.Gb() : this.Tc(), this.jh && (this.canvas && this.canvas.toDataURL && (this.po = this.canvas.toDataURL(this.jh[0], this.jh[1]), window.cr_onSnapshot && window.cr_onSnapshot(this.po), this.trigger(X.prototype.m.np, null)), this.jh = null));
                this.Nu || (this.ed++, this.Be++, this.Uh++);
                this.Rg += Qa() - f
            }
        }
    };
    c.prototype.ys = function(a) {
        var e, b, f, d, c, g, k, n;
        1E3 <= a - this.Oe && (this.Oe += 1E3, 1E3 <= a - this.Oe && (this.Oe = a), this.Wj = this.Uh, this.Uh = 0, this.Ej = this.Rg, this.Rg = 0);
        0 !== this.ki && (e = a - this.ki, 0 > e && (e = 0), this.Qd = e / 1E3, .5 < this.Qd ? this.Qd = 0 : this.Qd > 1 / this.An && (this.Qd = 1 / this.An));
        this.ki = a;
        this.pf = this.Qd * this.gf;
        this.Jb.add(this.pf);
        this.kj.add(this.Qd);
        a = (document.mozFullScreen || document.webkitIsFullScreen || document.fullScreen || !!document.msFullscreenElement || this.Bg) && !this.od;
        2 <= this.xb ||
            a && 0 < this.Ce ? (e = this.Ka / this.Ja, b = this.width / this.height, f = this.xb, a && 0 < this.Ce && (f = this.Ce), this.Yf = 2 !== f && b > e || 2 === f && b < e ? this.height / this.Ja : this.width / this.Ka, this.R && (this.R.bl(this.R.scrollX), this.R.cl(this.R.scrollY))) : this.Yf = this.Je ? this.devicePixelRatio : 1;
        this.Ya();
        this.Zb++;
        this.dd.kt();
        this.Zb--;
        this.Ya();
        this.Zb++;
        b = this.Hn.zd();
        a = 0;
        for (e = b.length; a < e; a++) b[a].Ou();
        a = 0;
        for (e = this.p.length; a < e; a++)
            if (g = this.p[a], !g.D && (g.ta.length || g.la.length))
                for (b = 0, f = g.d.length; b < f; b++)
                    for (k = g.d[b],
                        d = 0, c = k.C.length; d < c; d++) k.C[d].cc();
        a = 0;
        for (e = this.p.length; a < e; a++)
            if (g = this.p[a], !g.D && (g.ta.length || g.la.length))
                for (b = 0, f = g.d.length; b < f; b++)
                    for (k = g.d[b], d = 0, c = k.C.length; d < c; d++) n = k.C[d], n.Us && n.Us();
        b = this.Jk.zd();
        a = 0;
        for (e = b.length; a < e; a++) b[a].cc();
        this.Zb--;
        this.bs();
        for (a = 0; this.$f && 10 > a++;) this.gm(this.$f);
        a = 0;
        for (e = this.ld.length; a < e; a++) this.ld[a].jk = !1;
        this.R.ze && this.R.ze.Ea();
        S(this.eh);
        this.Zb++;
        a = 0;
        for (e = this.p.length; a < e; a++)
            if (g = this.p[a], !g.D && (g.ta.length || g.la.length))
                for (b =
                    0, f = g.d.length; b < f; b++)
                    for (k = g.d[b], d = 0, c = k.C.length; d < c; d++) n = k.C[d], n.aj && n.aj();
        b = this.In.zd();
        a = 0;
        for (e = b.length; a < e; a++) b[a].aj();
        this.Zb--
    };
    c.prototype.If = function() {
        var a, e, b, f, d, c, g, k, n;
        a = 0;
        for (e = this.p.length; a < e; a++)
            if (g = this.p[a], !g.D)
                for (b = 0, f = g.d.length; b < f; b++)
                    if (k = g.d[b], k.If && k.If(), k.C)
                        for (d = 0, c = k.C.length; d < c; d++) n = k.C[d], n.If && n.If()
    };
    c.prototype.gm = function(a) {
        var e = this.R;
        this.R.zt();
        var b, f, d;
        if (this.n)
            for (b = 0, f = this.p.length; b < f; b++) d = this.p[b], d.D || !d.tl || d.global && 0 !== d.d.length ||
                -1 !== a.xg.indexOf(d) || d.tl();
        e == a && S(this.dd.Rb);
        S(this.eh);
        this.Xn(!0);
        a.hl();
        this.Xn(!1);
        this.T = !0;
        this.Ya()
    };
    c.prototype.Xn = function(a) {
        var e, b, f, d, c, g, k, n, q;
        e = 0;
        for (b = this.ta.length; e < b; e++) f = this.ta[e], a ? f.Gf && f.Gf() : f.Vg && f.Vg();
        e = 0;
        for (b = this.p.length; e < b; e++)
            if (f = this.p[e], f.global || f.fa.Vi)
                for (d = 0, c = f.d.length; d < c; d++)
                    if (g = f.d[d], a ? g.Gf && g.Gf() : g.Vg && g.Vg(), g.C)
                        for (k = 0, n = g.C.length; k < n; k++) q = g.C[k], a ? q.Gf && q.Gf() : q.Vg && q.Vg()
    };
    c.prototype.yo = function(a) {
        this.Jk.add(a)
    };
    c.prototype.mg = function(a) {
        return a &&
            -1 !== a.Sg ? this.Qd * a.Sg : this.pf
    };
    c.prototype.Tc = function() {
        this.R.Tc(this.X);
        this.Ie && this.X.present()
    };
    c.prototype.Gb = function() {
        this.ea && (this.ye = 1, this.R.xe(this.n));
        this.R.Gb(this.n);
        this.n.Ys()
    };
    c.prototype.Qp = function(a) {
        a && this.Nj.push(a)
    };
    c.prototype.ym = function(a) {
        a = a.toString();
        return this.td.hasOwnProperty(a) ? this.td[a] : null
    };
    var Z = [];
    c.prototype.Cd = function(a) {
        var e, b;
        e = a.type.name;
        var f = null;
        if (this.we.hasOwnProperty(e)) {
            if (f = this.we[e], f.contains(a)) return
        } else f = Z.length ? Z.pop() : new ca,
            this.we[e] = f;
        f.add(a);
        this.Ud = !0;
        if (a.qb)
            for (e = 0, b = a.siblings.length; e < b; e++) this.Cd(a.siblings[e]);
        this.rk && f.Sf.push(a);
        this.qk || (this.Zb++, this.trigger(Object.getPrototypeOf(a.type.fa).m.pp, a), this.Zb--)
    };
    c.prototype.Ya = function() {
        if (this.Ud) {
            var a, e, b, f, d, c;
            this.rk = !0;
            b = 0;
            for (d = this.Ac.length; b < d; ++b)
                for (a = this.Ac[b], e = a.type, e.d.push(a), f = 0, c = e.la.length; f < c; ++f) e.la[f].d.push(a), e.la[f].Nf = !0;
            S(this.Ac);
            this.ep();
            Pa(this.we);
            this.Ud = this.rk = !1
        }
    };
    c.prototype.ep = function() {
        for (var a in this.we) this.we.hasOwnProperty(a) &&
            this.To(this.we[a])
    };
    c.prototype.To = function(a) {
        var e = a.zd(),
            b = e[0].type,
            f, d, c, g, k, n;
        Ra(b.d, a);
        b.Nf = !0;
        0 === b.d.length && (b.uh = !1);
        f = 0;
        for (d = b.la.length; f < d; ++f) n = b.la[f], Ra(n.d, a), n.Nf = !0;
        f = 0;
        for (d = this.dd.Rb.length; f < d; ++f)
            if (k = this.dd.Rb[f], k.Qb.hasOwnProperty(b.index) && Ra(k.Qb[b.index].Ge, a), !b.D)
                for (c = 0, g = b.la.length; c < g; ++c) n = b.la[c], k.Qb.hasOwnProperty(n.index) && Ra(k.Qb[n.index].Ge, a);
        if (k = e[0].j) {
            if (k.fc)
                for (c = k.d, f = 0, d = c.length; f < d; ++f) g = c[f], a.contains(g) && (g.Z(), k.Xa.update(g, g.Bb, null),
                    g.Bb.set(0, 0, -1, -1));
            Ra(k.d, a);
            k.fh(0)
        }
        for (f = 0; f < e.length; ++f) this.So(e[f], b);
        a.clear();
        Z.push(a);
        this.T = !0
    };
    c.prototype.So = function(a, e) {
        var b, f, d;
        b = 0;
        for (f = this.Nj.length; b < f; ++b) this.Nj[b](a);
        a.Md && e.Eh.update(a, a.Md, null);
        (b = a.j) && b.Jf(a, !0);
        if (a.C)
            for (b = 0, f = a.C.length; b < f; ++b) d = a.C[b], d.Hf && d.Hf(), d.behavior.Di.remove(a);
        this.Hn.remove(a);
        this.Jk.remove(a);
        this.In.remove(a);
        a.Hf && a.Hf();
        this.td.hasOwnProperty(a.uid.toString()) && delete this.td[a.uid.toString()];
        this.Fi--;
        100 > e.Jh.length && e.Jh.push(a)
    };
    c.prototype.Fj = function(a, e, b, f) {
        if (a.D) {
            var d = N(Math.random() * a.Cf.length);
            return this.Fj(a.Cf[d], e, b, f)
        }
        return a.ic ? this.Od(a.ic, e, !1, b, f, !1) : null
    };
    var E = [];
    c.prototype.Od = function(a, e, b, f, d, c) {
        var g, k, n, q;
        if (!a) return null;
        var h = this.p[a[1]],
            l = h.fa.Yd;
        if (this.Eg && l && !h.sk || l && !this.n && 11 === a[0][11]) return null;
        var y = e;
        l || (e = null);
        var t;
        h.Jh.length ? (t = h.Jh.pop(), t.Nb = !0, h.fa.ca.call(t, h)) : (t = new h.fa.ca(h), t.Nb = !1);
        !b || c || this.td.hasOwnProperty(a[2].toString()) ? t.uid = this.Tg++ : t.uid = a[2];
        this.td[t.uid.toString()] =
            t;
        t.Un = this.En++;
        t.Ee = h.d.length;
        g = 0;
        for (k = this.Ac.length; g < k; ++g) this.Ac[g].type === h && t.Ee++;
        t.rg = ub;
        t.toString = vb;
        n = a[3];
        if (t.Nb) Pa(t.u);
        else {
            t.u = {};
            if ("undefined" !== typeof cr_is_preview)
                for (t.Km = [], t.Km.length = n.length, g = 0, k = n.length; g < k; g++) t.Km[g] = n[g][1];
            t.Ha = [];
            t.Ha.length = n.length
        }
        g = 0;
        for (k = n.length; g < k; g++) t.Ha[g] = n[g][0];
        if (l) {
            var z = a[0];
            t.x = fa(f) ? z[0] : f;
            t.y = fa(d) ? z[1] : d;
            t.z = z[2];
            t.width = z[3];
            t.height = z[4];
            t.depth = z[5];
            t.i = z[6];
            t.opacity = z[7];
            t.yb = z[8];
            t.zb = z[9];
            t.Za = z[10];
            g = z[11];
            !this.n &&
                h.G.length && (t.Za = g);
            t.ag = $a(t.Za);
            this.q && ab(t, t.Za, this.q);
            if (t.Nb) {
                g = 0;
                for (k = z[12].length; g < k; g++)
                    for (n = 0, q = z[12][g].length; n < q; n++) t.ua[g][n] = z[12][g][n];
                t.da.set(0, 0, 0, 0);
                t.Md.set(0, 0, -1, -1);
                t.Bb.set(0, 0, -1, -1);
                t.bb.cf(t.da);
                S(t.tj)
            } else {
                t.ua = z[12].slice(0);
                g = 0;
                for (k = t.ua.length; g < k; g++) t.ua[g] = z[12][g].slice(0);
                t.U = [];
                t.Fd = [];
                t.Fd.length = h.G.length;
                t.da = new Q(0, 0, 0, 0);
                t.Md = new Q(0, 0, -1, -1);
                t.Bb = new Q(0, 0, -1, -1);
                t.bb = new qa;
                t.tj = [];
                t.Q = yb;
                t.Ju = zb;
                t.nb = Ab;
                t.Z = Bb;
                t.Gt = Cb;
                t.Ko = Db;
                t.Vc = Eb
            }
            t.Pf = !1;
            t.Et = 0;
            t.Dt = 0;
            t.Ct = null;
            14 === z.length && (t.Pf = !0, t.Et = z[13][0], t.Dt = z[13][1], t.Ct = z[13][2]);
            g = 0;
            for (k = h.G.length; g < k; g++) t.Fd[g] = !0;
            t.ud = !0;
            t.Mc = Fb;
            t.Mc();
            t.Lo = !!t.U.length;
            t.sj = !0;
            t.xj = !0;
            h.sh = !0;
            t.visible = !0;
            t.Sg = -1;
            t.j = e;
            t.Nc = e.d.length;
            t.ye = 0;
            "undefined" === typeof t.O && (t.O = null);
            this.T = t.Rc = !0
        }
        var m;
        S(E);
        g = 0;
        for (k = h.la.length; g < k; g++) E.push.apply(E, h.la[g].ta);
        E.push.apply(E, h.ta);
        if (t.Nb)
            for (g = 0, k = E.length; g < k; g++) {
                var u = E[g];
                m = t.C[g];
                m.Nb = !0;
                u.behavior.ca.call(m, u, t);
                z = a[4][g];
                n = 0;
                for (q = z.length; n <
                    q; n++) m.H[n] = z[n];
                m.S();
                u.behavior.Di.add(t)
            } else
                for (t.C = [], g = 0, k = E.length; g < k; g++) u = E[g], m = new u.behavior.ca(u, t), m.Nb = !1, m.H = a[4][g].slice(0), m.S(), t.C.push(m), u.behavior.Di.add(t);
        z = a[5];
        if (t.Nb)
            for (g = 0, k = z.length; g < k; g++) t.H[g] = z[g];
        else t.H = z.slice(0);
        this.Ac.push(t);
        this.Ud = !0;
        e && (e.Xf(t, !0), 1 !== e.nc || 1 !== e.oc) && (h.uh = !0);
        this.Fi++;
        if (h.qb) {
            if (t.qb = !0, t.Nb ? S(t.siblings) : t.siblings = [], !b && !c) {
                g = 0;
                for (k = h.Eb.length; g < k; g++)
                    if (h.Eb[g] !== h) {
                        if (!h.Eb[g].ic) return null;
                        t.siblings.push(this.Od(h.Eb[g].ic,
                            y, !1, l ? t.x : f, l ? t.y : d, !0))
                    }
                g = 0;
                for (k = t.siblings.length; g < k; g++)
                    for (t.siblings[g].siblings.push(t), n = 0; n < k; n++) g !== n && t.siblings[g].siblings.push(t.siblings[n])
            }
        } else t.qb = !1, t.siblings = null;
        t.S();
        g = 0;
        for (k = t.C.length; g < k; g++) t.C[g].Ts && t.C[g].Ts();
        return t
    };
    c.prototype.ck = function(a) {
        var e, b;
        e = 0;
        for (b = this.R.I.length; e < b; e++) {
            var f = this.R.I[e];
            if (cb(f.name, a)) return f
        }
        return null
    };
    c.prototype.og = function(a) {
        a = N(a);
        0 > a && (a = 0);
        a >= this.R.I.length && (a = this.R.I.length - 1);
        return this.R.I[a]
    };
    c.prototype.Cj =
        function(a) {
            var e, b;
            e = 0;
            for (b = a.length; e < b; e++) a[e].P().W = !0
        };
    c.prototype.$g = function(a) {
        var e, b;
        e = 0;
        for (b = a.length; e < b; e++) a[e].$g()
    };
    c.prototype.Ki = function(a) {
        var e, b;
        e = 0;
        for (b = a.length; e < b; e++) a[e].Ki()
    };
    c.prototype.We = function(a) {
        var e, b;
        e = 0;
        for (b = a.length; e < b; e++) a[e].We()
    };
    c.prototype.Io = function(a) {
        if (a.sh) {
            var e, b, f = a.d;
            e = 0;
            for (b = f.length; e < b; ++e) f[e].Ko();
            f = this.Ac;
            e = 0;
            for (b = f.length; e < b; ++e) f[e].type === a && f[e].Ko();
            a.sh = !1
        }
    };
    c.prototype.Xh = function(a, e, b, f) {
        var d, g, c = a ? 1 !== a.nc || 1 !== a.oc :
            !1;
        if (e.D)
            for (a = 0, d = e.Cf.length; a < d; ++a) g = e.Cf[a], c || g.uh ? ya(f, g.d) : (this.Io(g), g.Eh.Mi(b, f));
        else c || e.uh ? ya(f, e.d) : (this.Io(e), e.Eh.Mi(b, f))
    };
    c.prototype.Am = function(a, e, b, f) {
        var d, g;
        d = 0;
        for (g = e.length; d < g; ++d) this.Xh(a, e[d], b, f)
    };
    c.prototype.zm = function(a, e, b) {
        var f = this.ro;
        f && this.Am(a, f.Ik, e, b)
    };
    c.prototype.At = function(a, e, b) {
        var f = a.P(),
            d, g, c, k, n, q;
        if (f.W)
            for (f.W = !1, S(f.d), d = 0, k = a.d.length; d < k; d++) c = a.d[d], c.Z(), n = c.j.mb(e, b, !0), q = c.j.mb(e, b, !1), c.nb(n, q) && f.d.push(c);
        else {
            d = g = 0;
            for (k = f.d.length; d <
                k; d++) c = f.d[d], c.Z(), n = c.j.mb(e, b, !0), q = c.j.mb(e, b, !1), c.nb(n, q) && (f.d[g] = f.d[d], g++);
            f.d.length = g
        }
        a.wc();
        return f.ik()
    };
    c.prototype.me = function(a, e) {
        if (!(a && e && a !== e && a.Rc && e.Rc)) return !1;
        a.Z();
        e.Z();
        var b = a.j,
            f = e.j,
            d, g, c, k, n, q, t, h;
        if (b === f || b.nc === f.nc && f.oc === f.oc && b.scale === f.scale && b.i === f.i && b.Oc === f.Oc) {
            if (!a.da.Lm(e.da) || !a.bb.ok(e.bb) || a.Pf && e.Pf) return !1;
            if (a.Pf) return this.uo(a, e);
            if (e.Pf) return this.uo(e, a);
            t = a.O && !a.O.Ke();
            d = e.O && !e.O.Ke();
            if (!t && !d) return !0;
            t ? (a.O.ve(a.width, a.height,
                a.i), t = a.O) : (this.Db.bf(a.bb, a.x, a.y, a.width, a.height), t = this.Db);
            d ? (e.O.ve(e.width, e.height, e.i), h = e.O) : (this.Db.bf(e.bb, e.x, e.y, e.width, e.height), h = this.Db);
            return t.He(h, e.x - a.x, e.y - a.y)
        }
        t = a.O && !a.O.Ke();
        d = e.O && !e.O.Ke();
        t ? (a.O.ve(a.width, a.height, a.i), this.Db.io(a.O)) : this.Db.bf(a.bb, a.x, a.y, a.width, a.height);
        t = this.Db;
        d ? (e.O.ve(e.width, e.height, e.i), this.il.io(e.O)) : this.il.bf(e.bb, e.x, e.y, e.width, e.height);
        h = this.il;
        d = 0;
        for (g = t.Jc; d < g; d++) c = 2 * d, k = c + 1, n = t.La[c], q = t.La[k], t.La[c] = b.ma(n + a.x, q +
            a.y, !0), t.La[k] = b.ma(n + a.x, q + a.y, !1);
        t.Z();
        d = 0;
        for (g = h.Jc; d < g; d++) c = 2 * d, k = c + 1, n = h.La[c], q = h.La[k], h.La[c] = f.ma(n + e.x, q + e.y, !0), h.La[k] = f.ma(n + e.x, q + e.y, !1);
        h.Z();
        return t.He(h, 0, 0)
    };
    var G = new qa;
    new Q(0, 0, 0, 0);
    var U = [];
    c.prototype.uo = function(a, e) {
        var b, f, d, g, c = e.da,
            k = a.x,
            n = a.y;
        a.Dr(c, U);
        var q = e.O && !e.O.Ke();
        b = 0;
        for (f = U.length; b < f; ++b)
            if (d = U[b], g = d.et, c.Mm(g, k, n) && (G.cf(g), G.offset(k, n), G.ok(e.bb)))
                if (q)
                    if (e.O.ve(e.width, e.height, e.i), d.Xg) {
                        if (d.Xg.He(e.O, e.x - (k + g.left), e.y - (n + g.top))) return S(U), !0
                    } else {
                        if (this.Db.bf(G, 0, 0, g.right - g.left, g.bottom - g.top), this.Db.He(e.O, e.x, e.y)) return S(U), !0
                    }
        else if (d.Xg) {
            if (this.Db.bf(e.bb, 0, 0, e.width, e.height), d.Xg.He(this.Db, -(k + g.left), -(n + g.top))) return S(U), !0
        } else return S(U), !0;
        S(U);
        return !1
    };
    c.prototype.jl = function(a, e) {
        if (!e || !e.Rc) return !1;
        e.Z();
        if (!e.da.Lm(a)) return !1;
        if (e.Pf) {
            e.Dr(a, U);
            var b, f, d, g, c = e.x,
                k = e.y;
            b = 0;
            for (f = U.length; b < f; ++b)
                if (d = U[b], g = d.et, a.Mm(g, c, k))
                    if (d.Xg) {
                        if (this.Db.cf(a, 0, 0), d.Xg.He(this.Db, -(c + g.left), -(k + g.top))) return S(U), !0
                    } else return S(U), !0;
            S(U);
            return !1
        }
        G.cf(a);
        if (!e.bb.ok(G)) return !1;
        if (!e.O || e.O.Ke()) return !0;
        e.O.ve(e.width, e.height, e.i);
        G.offset(-a.left, -a.top);
        this.Db.bf(G, 0, 0, 1, 1);
        return e.O.He(this.Db, a.left - e.x, a.top - e.y)
    };
    c.prototype.Eo = function(a, e) {
        if (!e) return !1;
        var b, f, d, g, c;
        b = 0;
        for (f = a.ta.length; b < f; b++)
            if (a.ta[b].behavior instanceof e) return !0;
        if (!a.D)
            for (b = 0, f = a.la.length; b < f; b++)
                for (c = a.la[b], d = 0, g = c.ta.length; d < g; d++)
                    if (c.ta[d].behavior instanceof e) return !0;
        return !1
    };
    c.prototype.rl = function(a) {
        return this.Eo(a,
            Ub.ku)
    };
    c.prototype.sl = function(a) {
        return this.Eo(a, Ub.lu)
    };
    var R = [];
    c.prototype.ef = function(a) {
        var e, b, f;
        a.Z();
        this.zm(a.j, a.da, R);
        e = 0;
        for (b = R.length; e < b; ++e)
            if (f = R[e], f.u.solidEnabled && this.me(a, f)) return S(R), f;
        S(R);
        return null
    };
    c.prototype.Bt = function(a) {
        var e, b, f;
        this.zm(null, a, R);
        e = 0;
        for (b = R.length; e < b; ++e)
            if (f = R[e], f.u.solidEnabled && this.jl(a, f)) return S(R), f;
        S(R);
        return null
    };
    c.prototype.ct = function(a, e, b, f) {
        f = f || 50;
        var d = a.x,
            g = a.y,
            c, k = null,
            n = null;
        for (c = 0; c < f; c++)
            if (a.x = d + e * c, a.y = g + b * c, a.Q(), !this.me(a, k) && ((k = this.ef(a)) && (n = k), !k && !k)) return n && this.$s(a, e, b, n), !0;
        a.x = d;
        a.y = g;
        a.Q();
        return !1
    };
    c.prototype.$s = function(a, e, b, f) {
        var d = 2,
            g, c = !1;
        g = !1;
        for (var k = a.x, n = a.y; 16 >= d;) g = 1 / d, d *= 2, a.x += e * g * (c ? 1 : -1), a.y += b * g * (c ? 1 : -1), a.Q(), this.me(a, f) ? g = c = !0 : (g = c = !1, k = a.x, n = a.y);
        g && (a.x = k, a.y = n, a.Q())
    };
    c.prototype.dt = function(a) {
        var e = 0,
            b = a.x,
            f = a.y,
            d = 0,
            g = 0,
            c = 0,
            k = this.ef(a);
        if (k) {
            for (; 100 >= e;) {
                switch (d) {
                    case 0:
                        g = 0;
                        c = -1;
                        e++;
                        break;
                    case 1:
                        g = 1;
                        c = -1;
                        break;
                    case 2:
                        g = 1;
                        c = 0;
                        break;
                    case 3:
                        c = g = 1;
                        break;
                    case 4:
                        g =
                            0;
                        c = 1;
                        break;
                    case 5:
                        g = -1;
                        c = 1;
                        break;
                    case 6:
                        g = -1;
                        c = 0;
                        break;
                    case 7:
                        c = g = -1
                }
                d = (d + 1) % 8;
                a.x = N(b + g * e);
                a.y = N(f + c * e);
                a.Q();
                if (!this.me(a, k) && (k = this.ef(a), !k)) return
            }
            a.x = b;
            a.y = f;
            a.Q()
        }
    };
    c.prototype.ft = function(a, e) {
        a.Rc && e.Rc && this.eh.push([a, e])
    };
    c.prototype.bq = function(a, e) {
        var b, f, d;
        b = 0;
        for (f = this.eh.length; b < f; b++)
            if (d = this.eh[b], d[0] == a && d[1] == e || d[0] == e && d[1] == a) return !0;
        return !1
    };
    c.prototype.$p = function(a, e, b) {
        var f = a.x,
            d = a.y,
            g = ka(10, Ma(e, b, f, d)),
            c = Ia(e, b, f, d),
            k = this.ef(a);
        if (!k) return Ea(c + na);
        var n =
            k,
            q, t, h, l, y = T(5);
        for (q = 1; 36 > q; q++)
            if (t = c - q * y, a.x = e + Math.cos(t) * g, a.y = b + Math.sin(t) * g, a.Q(), !this.me(a, n) && (n = this.ef(a), !n)) {
                h = t;
                break
            }
        36 === q && (h = Ea(c + na));
        n = k;
        for (q = 1; 36 > q; q++)
            if (t = c + q * y, a.x = e + Math.cos(t) * g, a.y = b + Math.sin(t) * g, a.Q(), !this.me(a, n) && (n = this.ef(a), !n)) {
                l = t;
                break
            }
        36 === q && (l = Ea(c + na));
        a.x = f;
        a.y = d;
        a.Q();
        if (l === h) return l;
        a = Ja(l, h) / 2;
        a = La(l, h) ? Ea(h + a + na) : Ea(l + a);
        h = Math.cos(c);
        c = Math.sin(c);
        l = Math.cos(a);
        a = Math.sin(a);
        e = h * l + c * a;
        return Ia(0, 0, h - 2 * e * l, c - 2 * e * a)
    };
    var O = -1;
    c.prototype.trigger =
        function(a, e, b) {
            if (!this.R) return !1;
            var f = this.R.ze;
            if (!f) return !1;
            var d = !1,
                g, c, k;
            O++;
            var n = f.Kj;
            c = 0;
            for (k = n.length; c < k; ++c) g = this.Bo(a, e, n[c], b), d = d || g;
            g = this.Bo(a, e, f, b);
            O--;
            return d || g
        };
    c.prototype.Bo = function(a, e, b, f) {
        var d = !1,
            g, c, k, n;
        if (e)
            for (k = this.pl(a, e, e.type.name, b, f), d = d || k, n = e.type.la, g = 0, c = n.length; g < c; ++g) k = this.pl(a, e, n[g].name, b, f), d = d || k;
        else k = this.pl(a, e, "system", b, f), d = d || k;
        return d
    };
    c.prototype.pl = function(a, e, b, f, d) {
        var g, c = !1,
            k = !1,
            k = "undefined" !== typeof d,
            n = (k ? f.lm : f.Co)[b];
        if (!n) return c;
        var q = null;
        f = 0;
        for (g = n.length; f < g; ++f)
            if (n[f].method == a) {
                q = n[f].jg;
                break
            }
        if (!q) return c;
        var t;
        k ? t = q[d] : t = q;
        if (!t) return null;
        f = 0;
        for (g = t.length; f < g; f++) a = t[f][0], d = t[f][1], k = this.qr(e, b, a, d), c = c || k;
        return c
    };
    c.prototype.qr = function(a, e, b, f) {
        var d, g, c = !1;
        this.ql++;
        var k = this.Qa().Fb;
        k && this.$g(k.le);
        var n = 1 < this.ql;
        this.$g(b.le);
        n && this.bt();
        var q = this.Li(b);
        q.Fb = b;
        a && (d = this.types[e].P(), d.W = !1, S(d.d), d.d[0] = a, this.types[e].wc());
        a = !0;
        if (b.parent) {
            e = q.to;
            for (d = b.parent; d;) e.push(d),
                d = d.parent;
            e.reverse();
            d = 0;
            for (g = e.length; d < g; d++)
                if (!e[d].mt()) {
                    a = !1;
                    break
                }
        }
        a && (this.Be++, b.lc ? b.lt(f) : b.Ea(), c = c || q.Ne);
        this.Ii();
        n && this.Ss();
        this.We(b.le);
        k && this.We(k.le);
        this.Ud && 0 === this.Zb && 0 === O && !this.tk && this.Ya();
        this.ql--;
        return c
    };
    c.prototype.Yh = function() {
        var a = this.Qa();
        return a.Fb.Oa[a.Ga]
    };
    c.prototype.ns = function() {
        return 0 === this.Qa().Ga
    };
    c.prototype.Er = function() {
        var a = this.Qa();
        return a.Fb.gc[a.Sb]
    };
    c.prototype.bt = function() {
        this.xi++;
        this.xi >= this.Hk.length && this.Hk.push([])
    };
    c.prototype.Ss = function() {
        this.xi--
    };
    c.prototype.um = function() {
        return this.Hk[this.xi]
    };
    c.prototype.Li = function(a) {
        this.Mh++;
        this.Mh >= this.Qj.length && this.Qj.push(new Gb);
        var e = this.Qa();
        e.reset(a);
        return e
    };
    c.prototype.Ii = function() {
        this.Mh--
    };
    c.prototype.Qa = function() {
        return this.Qj[this.Mh]
    };
    c.prototype.vm = function(a, e) {
        for (var b, f, d, g, c, k; e;) {
            b = 0;
            for (f = e.sc.length; b < f; b++)
                if (k = e.sc[b], k instanceof Hb && cb(a, k.name)) return k;
            e = e.parent
        }
        b = 0;
        for (f = this.ld.length; b < f; b++)
            for (c = this.ld[b], d = 0, g = c.Sd.length; d <
                g; d++)
                if (k = c.Sd[d], k instanceof Hb && cb(a, k.name)) return k;
        return null
    };
    c.prototype.wm = function(a) {
        var e, b;
        e = 0;
        for (b = this.Fc.length; e < b; e++)
            if (this.Fc[e].L === a) return this.Fc[e];
        return null
    };
    c.prototype.uf = function(a) {
        var e, b;
        e = 0;
        for (b = this.p.length; e < b; e++)
            if (this.p[e].L === a) return this.p[e];
        return null
    };
    c.prototype.Gr = function(a) {
        var e, b;
        e = 0;
        for (b = this.Hd.length; e < b; e++)
            if (this.Hd[e].L === a) return this.Hd[e];
        return null
    };
    c.prototype.lq = function(a, e) {
        this.jh = [a, e];
        this.T = !0
    };
    c.prototype.bs = function() {
        var a =
            this,
            e = this.al,
            f = this.qd,
            d = this.ri,
            g = !1;
        this.oo && (g = !0, e = "__c2_continuouspreview", this.oo = !1);
        if (e.length) {
            this.Ya();
            f = this.qt();
            if (h() && !this.Cc) r(e, f, function() {
                x("Saved state to IndexedDB storage (" + f.length + " bytes)");
                a.qd = f;
                a.trigger(X.prototype.m.oj, null);
                a.qd = "";
                g && b()
            }, function(d) {
                try {
                    localStorage.setItem("__c2save_" + e, f), x("Saved state to WebStorage (" + f.length + " bytes)"), a.qd = f, a.trigger(X.prototype.m.oj, null), a.qd = "", g && b()
                } catch (c) {
                    x("Failed to save game state: " + d + "; " + c)
                }
            });
            else try {
                localStorage.setItem("__c2save_" +
                    e, f), x("Saved state to WebStorage (" + f.length + " bytes)"), a.qd = f, this.trigger(X.prototype.m.oj, null), a.qd = "", g && b()
            } catch (c) {
                x("Error saving to WebStorage: " + c)
            }
            this.ib = this.ri = this.al = ""
        }
        if (d.length) {
            if (h() && !this.Cc) p(d, function(e) {
                e ? (a.ib = e, x("Loaded state from IndexedDB storage (" + a.ib.length + " bytes)")) : (a.ib = localStorage.getItem("__c2save_" + d) || "", x("Loaded state from WebStorage (" + a.ib.length + " bytes)"));
                a.kh = !1;
                a.ib.length || a.trigger(X.prototype.m.mj, null)
            }, function() {
                a.ib = localStorage.getItem("__c2save_" +
                    d) || "";
                x("Loaded state from WebStorage (" + a.ib.length + " bytes)");
                a.kh = !1;
                a.ib.length || a.trigger(X.prototype.m.mj, null)
            });
            else {
                try {
                    this.ib = localStorage.getItem("__c2save_" + d) || "", x("Loaded state from WebStorage (" + this.ib.length + " bytes)")
                } catch (k) {
                    this.ib = ""
                }
                this.kh = !1;
                a.ib.length || a.trigger(X.prototype.m.mj, null)
            }
            this.al = this.ri = ""
        }
        this.ib.length && (this.Ya(), this.xs(this.ib), this.qd = this.ib, this.trigger(X.prototype.m.wp, null), this.ib = this.qd = "")
    };
    c.prototype.qt = function() {
        var e, b, f, d, g, c, k, n = {
            c2save: !0,
            version: 1,
            rt: {
                time: this.Jb.Y,
                walltime: this.kj.Y,
                timescale: this.gf,
                tickcount: this.ed,
                execcount: this.Be,
                next_uid: this.Tg,
                running_layout: this.R.L,
                start_time_offset: Date.now() - this.Wi
            },
            types: {},
            layouts: {},
            events: {
                groups: {},
                cnds: {},
                acts: {},
                vars: {}
            }
        };
        e = 0;
        for (b = this.p.length; e < b; e++)
            if (g = this.p[e], !g.D && !this.rl(g)) {
                c = {
                    instances: []
                };
                Oa(g.u) && (c.ex = a(g.u));
                f = 0;
                for (d = g.d.length; f < d; f++) c.instances.push(this.$k(g.d[f]));
                n.types[g.L.toString()] = c
            }
        e = 0;
        for (b = this.Fc.length; e < b; e++) f = this.Fc[e], n.layouts[f.L.toString()] =
            f.rb();
        d = n.events.groups;
        e = 0;
        for (b = this.Hd.length; e < b; e++) f = this.Hd[e], d[f.L.toString()] = this.$h[f.tg].sg;
        b = n.events.cnds;
        for (k in this.Ld) this.Ld.hasOwnProperty(k) && (e = this.Ld[k], Oa(e.u) && (b[k] = {
            ex: a(e.u)
        }));
        b = n.events.acts;
        for (k in this.Gd) this.Gd.hasOwnProperty(k) && (e = this.Gd[k], Oa(e.u) && (b[k] = {
            ex: a(e.u)
        }));
        b = n.events.vars;
        for (k in this.Tf) this.Tf.hasOwnProperty(k) && (e = this.Tf[k], e.hi || e.parent && !e.Dg || (b[k] = e.data));
        n.system = this.dd.rb();
        return JSON.stringify(n)
    };
    c.prototype.Wn = function() {
        var a,
            e, b, f, d, g;
        this.td = {};
        a = 0;
        for (e = this.p.length; a < e; a++)
            if (b = this.p[a], !b.D)
                for (f = 0, d = b.d.length; f < d; f++) g = b.d[f], this.td[g.uid.toString()] = g
    };
    c.prototype.xs = function(a) {
        a = JSON.parse(a);
        if (a.c2save && !(1 < a.version)) {
            this.zg = !0;
            var e = a.rt;
            this.Jb.reset();
            this.Jb.Y = e.time;
            this.kj.reset();
            this.kj.Y = e.walltime || 0;
            this.gf = e.timescale;
            this.ed = e.tickcount;
            this.Be = e.execcount;
            this.Wi = Date.now() - e.start_time_offset;
            var b = e.running_layout;
            if (b !== this.R.L)
                if (b = this.wm(b)) this.gm(b);
                else return;
            var f, d, g, c, k, n, q;
            n = a.types;
            for (d in n)
                if (n.hasOwnProperty(d) && (c = this.uf(parseInt(d, 10))) && !c.D && !this.rl(c)) {
                    n[d].ex ? c.u = n[d].ex : Pa(c.u);
                    k = c.d;
                    g = n[d].instances;
                    b = 0;
                    for (f = ma(k.length, g.length); b < f; b++) this.si(k[b], g[b]);
                    b = g.length;
                    for (f = k.length; b < f; b++) this.Cd(k[b]);
                    b = k.length;
                    for (f = g.length; b < f; b++) {
                        k = null;
                        if (c.fa.Yd && (k = this.R.Zh(g[b].w.l), !k)) continue;
                        k = this.Od(c.ic, k, !1, 0, 0, !0);
                        this.si(k, g[b])
                    }
                    c.Nf = !0
                }
            this.Ya();
            this.Wn();
            f = a.layouts;
            for (d in f) f.hasOwnProperty(d) && (b = this.wm(parseInt(d, 10))) && b.Kb(f[d]);
            f = a.events.groups;
            for (d in f) f.hasOwnProperty(d) && (b = this.Gr(parseInt(d, 10))) && this.$h[b.tg] && this.$h[b.tg].eo(f[d]);
            b = a.events.cnds;
            for (d in this.Ld) this.Ld.hasOwnProperty(d) && (b.hasOwnProperty(d) ? this.Ld[d].u = b[d].ex : this.Ld[d].u = {});
            b = a.events.acts;
            for (d in this.Gd) this.Gd.hasOwnProperty(d) && (b.hasOwnProperty(d) ? this.Gd[d].u = b[d].ex : this.Gd[d].u = {});
            b = a.events.vars;
            for (d in b) b.hasOwnProperty(d) && this.Tf.hasOwnProperty(d) && (this.Tf[d].data = b[d]);
            this.Tg = e.next_uid;
            this.zg = !1;
            b = 0;
            for (f = this.Ph.length; b < f; ++b) k = this.Ph[b],
                this.trigger(Object.getPrototypeOf(k.type.fa).m.lf, k);
            S(this.Ph);
            this.dd.Kb(a.system);
            b = 0;
            for (f = this.p.length; b < f; b++)
                if (c = this.p[b], !c.D && !this.rl(c))
                    for (d = 0, a = c.d.length; d < a; d++) {
                        k = c.d[d];
                        if (c.qb)
                            for (n = k.rg(), S(k.siblings), e = 0, g = c.Eb.length; e < g; e++) q = c.Eb[e], c !== q && k.siblings.push(q.d[n]);
                        k.fd && k.fd();
                        if (k.C)
                            for (e = 0, g = k.C.length; e < g; e++) n = k.C[e], n.fd && n.fd()
                    }
                this.T = !0
        }
    };
    c.prototype.$k = function(e, b) {
        var f, d, g, c, k;
        c = e.type;
        g = c.fa;
        var n = {};
        b ? n.c2 = !0 : n.uid = e.uid;
        Oa(e.u) && (n.ex = a(e.u));
        if (e.Ha && e.Ha.length)
            for (n.ivs = {}, f = 0, d = e.Ha.length; f < d; f++) n.ivs[e.type.nk[f].toString()] = e.Ha[f];
        if (g.Yd) {
            g = {
                x: e.x,
                y: e.y,
                w: e.width,
                h: e.height,
                l: e.j.L,
                zi: e.Vc()
            };
            0 !== e.i && (g.a = e.i);
            1 !== e.opacity && (g.o = e.opacity);
            .5 !== e.yb && (g.hX = e.yb);
            .5 !== e.zb && (g.hY = e.zb);
            0 !== e.Za && (g.bm = e.Za);
            e.visible || (g.v = e.visible);
            e.Rc || (g.ce = e.Rc); - 1 !== e.Sg && (g.mts = e.Sg);
            if (c.G.length)
                for (g.fx = [], f = 0, d = c.G.length; f < d; f++) k = c.G[f], g.fx.push({
                    name: k.name,
                    active: e.Fd[k.index],
                    params: e.ua[k.index]
                });
            n.w = g
        }
        if (e.C && e.C.length)
            for (n.behs = {}, f = 0, d = e.C.length; f <
                d; f++) c = e.C[f], c.rb && (n.behs[c.type.L.toString()] = c.rb());
        e.rb && (n.data = e.rb());
        return n
    };
    c.prototype.Ir = function(a, e) {
        var b, f;
        b = 0;
        for (f = a.nk.length; b < f; b++)
            if (a.nk[b] === e) return b;
        return -1
    };
    c.prototype.Cr = function(a, e) {
        var b, f;
        b = 0;
        for (f = a.C.length; b < f; b++)
            if (a.C[b].type.L === e) return b;
        return -1
    };
    c.prototype.si = function(a, e, b) {
        var f, d, g, c, k;
        k = a.type;
        c = k.fa;
        if (b) {
            if (!e.c2) return
        } else a.uid = e.uid;
        e.ex ? a.u = e.ex : Pa(a.u);
        if (d = e.ivs)
            for (f in d) d.hasOwnProperty(f) && (g = this.Ir(k, parseInt(f, 10)), 0 > g || g >= a.Ha.length ||
                (a.Ha[g] = d[f]));
        if (c.Yd) {
            g = e.w;
            a.j.L !== g.l && (d = a.j, a.j = this.R.Zh(g.l), a.j ? (d.Jf(a, !0), a.j.Xf(a, !0), a.Q(), a.j.fh(0)) : (a.j = d, b || this.Cd(a)));
            a.x = g.x;
            a.y = g.y;
            a.width = g.w;
            a.height = g.h;
            a.Nc = g.zi;
            a.i = g.hasOwnProperty("a") ? g.a : 0;
            a.opacity = g.hasOwnProperty("o") ? g.o : 1;
            a.yb = g.hasOwnProperty("hX") ? g.hX : .5;
            a.zb = g.hasOwnProperty("hY") ? g.hY : .5;
            a.visible = g.hasOwnProperty("v") ? g.v : !0;
            a.Rc = g.hasOwnProperty("ce") ? g.ce : !0;
            a.Sg = g.hasOwnProperty("mts") ? g.mts : -1;
            a.Za = g.hasOwnProperty("bm") ? g.bm : 0;
            a.ag = $a(a.Za);
            this.q &&
                ab(a, a.Za, this.q);
            a.Q();
            if (g.hasOwnProperty("fx"))
                for (b = 0, d = g.fx.length; b < d; b++) c = k.ak(g.fx[b].name), 0 > c || (a.Fd[c] = g.fx[b].active, a.ua[c] = g.fx[b].params);
            a.Mc()
        }
        if (k = e.behs)
            for (f in k) k.hasOwnProperty(f) && (b = this.Cr(a, parseInt(f, 10)), 0 > b || a.C[b].Kb(k[f]));
        e.data && a.Kb(e.data)
    };
    c.prototype.mm = function(a, e, b) {
        window.resolveLocalFileSystemURL(cordova.file.applicationDirectory + "www/" + a, function(a) {
            a.file(e, b)
        }, b)
    };
    c.prototype.sr = function(a, e) {
        this.mm("data.js", function(b) {
            var f = new FileReader;
            f.onload =
                function(e) {
                    a(e.target.result)
                };
            f.onerror = e;
            f.readAsText(b)
        }, e)
    };
    c.prototype.rr = function(a, e, b) {
        this.mm(a, function(a) {
            var b = new FileReader;
            b.onload = function(a) {
                e(a.target.result)
            };
            b.readAsArrayBuffer(a)
        }, b)
    };
    c.prototype.tr = function(a, e, b) {
        this.rr(a, function(a) {
            a = URL.createObjectURL(new Blob([a]));
            e(a)
        }, b)
    };
    c.prototype.fo = function(a, e) {
        this.uk ? this.tr(e, function(e) {
            a.src = e
        }, function(a) {
            alert("Failed to load image: " + a)
        }) : a.src = e
    };
    Ib = function(a) {
        return new c(document.getElementById(a))
    };
    Jb = function(a,
        e) {
        return new c({
            dc: !0,
            width: a,
            height: e
        })
    };
    window.cr_createRuntime = Ib;
    window.cr_createDCRuntime = Jb;
    window.createCocoonJSRuntime = function() {
        window.c2cocoonjs = !0;
        var a = document.createElement("screencanvas") || document.createElement("canvas");
        a.wh = !0;
        document.body.appendChild(a);
        a = new c(a);
        window.c2runtime = a;
        window.addEventListener("orientationchange", function() {
            window.c2runtime.setSize(window.innerWidth, window.innerHeight)
        });
        window.c2runtime.setSize(window.innerWidth, window.innerHeight);
        return a
    };
    window.createEjectaRuntime =
        function() {
            var a = new c(document.getElementById("canvas"));
            window.c2runtime = a;
            window.c2runtime.setSize(window.innerWidth, window.innerHeight);
            return a
        }
})();
window.cr_getC2Runtime = function() {
    var c = document.getElementById("c2canvas");
    return c ? c.c2runtime : window.c2runtime ? window.c2runtime : null
};
window.cr_getSnapshot = function(c, l) {
    var h = window.cr_getC2Runtime();
    h && h.lq(c, l)
};
window.cr_sizeCanvas = function(c, l) {
    if (0 !== c && 0 !== l) {
        var h = window.cr_getC2Runtime();
        h && h.setSize(c, l)
    }
};
window.cr_setSuspended = function(c) {
    var l = window.cr_getC2Runtime();
    l && l.setSuspended(c)
};
(function() {
    function c(a, b) {
        this.b = a;
        this.ze = null;
        this.scrollX = this.b.Ka / 2;
        this.scrollY = this.b.Ja / 2;
        this.scale = 1;
        this.i = 0;
        this.sf = !0;
        this.name = b[0];
        this.width = b[1];
        this.height = b[2];
        this.Fo = b[3];
        this.lo = b[4];
        this.L = b[5];
        var d = b[6],
            g, c;
        this.I = [];
        this.xg = [];
        g = 0;
        for (c = d.length; g < c; g++) {
            var h = new Kb(this, d[g]);
            h.Fn = g;
            this.I.push(h)
        }
        d = b[7];
        this.Fe = [];
        g = 0;
        for (c = d.length; g < c; g++) {
            var h = d[g],
                l = this.b.p[h[1]];
            l.ic || (l.ic = h);
            this.Fe.push(h); - 1 === this.xg.indexOf(l) && this.xg.push(l)
        }
        this.G = [];
        this.U = [];
        this.ud = !0;
        this.ua = [];
        g = 0;
        for (c = b[8].length; g < c; g++) this.G.push({
            id: b[8][g][0],
            name: b[8][g][1],
            Fa: -1,
            Hc: !1,
            Tb: !0,
            index: g
        }), this.ua.push(b[8][g][2].slice(0));
        this.Mc();
        this.je = new Q(0, 0, 1, 1);
        this.Uk = new Q(0, 0, 1, 1);
        this.ie = {}
    }

    function l(a, b) {
        return a.Nc - b.Nc
    }

    function h(a, b) {
        this.Ca = a;
        this.b = a.b;
        this.d = [];
        this.scale = 1;
        this.i = 0;
        this.kd = !1;
        this.xd = new Q(0, 0, 0, 0);
        this.zo = new qa;
        this.oa = this.ia = this.pa = this.ga = 0;
        this.hf = !1;
        this.Bd = -1;
        this.Dj = 0;
        this.name = b[0];
        this.index = b[1];
        this.L = b[2];
        this.visible = b[3];
        this.xc =
            b[4];
        this.Lc = b[5];
        this.nc = b[6];
        this.oc = b[7];
        this.opacity = b[8];
        this.Th = b[9];
        this.fc = b[10];
        this.Oc = b[11];
        this.Za = b[12];
        this.Jq = b[13];
        this.ag = "source-over";
        this.gb = this.jb = 0;
        this.Xa = null;
        this.rd = m();
        this.Kc = !0;
        this.zf = new Q(0, 0, -1, -1);
        this.Ta = new Q(0, 0, -1, -1);
        this.fc && (this.Xa = new Za(this.b.Ka, this.b.Ja));
        this.cd = !1;
        var d = b[14],
            g, c;
        this.so = [];
        this.Yb = [];
        this.dg = [];
        g = 0;
        for (c = d.length; g < c; g++) {
            var h = d[g],
                l = this.b.p[h[1]];
            l.ic || (l.ic = h, l.jq = this.index);
            this.Yb.push(h); - 1 === this.Ca.xg.indexOf(l) && this.Ca.xg.push(l)
        }
        wa(this.so,
            this.Yb);
        this.G = [];
        this.U = [];
        this.ud = !0;
        this.ua = [];
        g = 0;
        for (c = b[15].length; g < c; g++) this.G.push({
            id: b[15][g][0],
            name: b[15][g][1],
            Fa: -1,
            Hc: !1,
            Tb: !0,
            index: g
        }), this.ua.push(b[15][g][2].slice(0));
        this.Mc();
        this.je = new Q(0, 0, 1, 1);
        this.Uk = new Q(0, 0, 1, 1)
    }

    function m() {
        return a.length ? a.pop() : []
    }

    function r(e) {
        S(e);
        a.push(e)
    }
    c.prototype.pt = function(a) {
        var b = a.type.L.toString();
        this.ie.hasOwnProperty(b) || (this.ie[b] = []);
        this.ie[b].push(this.b.$k(a))
    };
    c.prototype.Cm = function() {
        var a = this.I[0];
        return !a.Lc && 1 ===
            a.opacity && !a.Th && a.visible
    };
    c.prototype.Mc = function() {
        S(this.U);
        this.ud = !0;
        var a, b, d;
        a = 0;
        for (b = this.G.length; a < b; a++) d = this.G[a], d.Tb && (this.U.push(d), d.Hc || (this.ud = !1))
    };
    c.prototype.$j = function(a) {
        var b, d, g;
        b = 0;
        for (d = this.G.length; b < d; b++)
            if (g = this.G[b], g.name === a) return g;
        return null
    };
    var p = [],
        b = !0;
    c.prototype.hl = function() {
        this.lo && (this.ze = this.b.Rj[this.lo], this.ze.ul());
        this.b.R = this;
        this.scrollX = this.b.Ka / 2;
        this.scrollY = this.b.Ja / 2;
        var a, f, d, g, c, h, m;
        a = 0;
        for (d = this.b.p.length; a < d; a++)
            if (f =
                this.b.p[a], !f.D)
                for (c = f.d, f = 0, g = c.length; f < g; f++)
                    if (h = c[f], h.j) {
                        var k = h.j.Fn;
                        k >= this.I.length && (k = this.I.length - 1);
                        h.j = this.I[k]; - 1 === h.j.d.indexOf(h) && h.j.d.push(h);
                        h.j.hf = !0
                    }
        if (!b)
            for (a = 0, d = this.I.length; a < d; ++a) this.I[a].d.sort(l);
        S(p);
        this.Yp();
        a = 0;
        for (d = this.I.length; a < d; a++) h = this.I[a], h.iq(), h.gj();
        c = !1;
        if (!this.sf) {
            for (m in this.ie)
                if (this.ie.hasOwnProperty(m) && (f = this.b.uf(parseInt(m, 10))) && !f.D && this.b.sl(f)) {
                    g = this.ie[m];
                    a = 0;
                    for (d = g.length; a < d; a++) {
                        h = null;
                        if (f.fa.Yd && (h = this.Zh(g[a].w.l), !h)) continue;
                        h = this.b.Od(f.ic, h, !1, 0, 0, !0);
                        this.b.si(h, g[a]);
                        c = !0;
                        p.push(h)
                    }
                    S(g)
                }
            a = 0;
            for (d = this.I.length; a < d; a++) this.I[a].d.sort(l), this.I[a].hf = !0
        }
        c && (this.b.Ya(), this.b.Wn());
        for (a = 0; a < p.length; a++)
            if (h = p[a], h.type.qb)
                for (d = h.rg(), f = 0, g = h.type.Eb.length; f < g; f++) m = h.type.Eb[f], h.type !== m && (m.d.length > d ? h.siblings.push(m.d[d]) : m.ic && (c = this.b.Od(m.ic, h.j, !0, h.x, h.y, !0), this.b.Ya(), m.fj(), h.siblings.push(c), p.push(c)));
        a = 0;
        for (d = this.Fe.length; a < d; a++) this.b.Od(this.Fe[a], null, !0);
        this.b.$f = null;
        this.b.Ya();
        if (this.b.X && !this.b.pb)
            for (a = 0, d = this.b.p.length; a < d; a++) m = this.b.p[a], !m.D && m.d.length && m.Pk && m.Pk(this.b.X);
        if (this.b.zg) wa(this.b.Ph, p);
        else
            for (a = 0, d = p.length; a < d; a++) h = p[a], this.b.trigger(Object.getPrototypeOf(h.type.fa).m.lf, h);
        S(p);
        this.b.zg || this.b.trigger(X.prototype.m.vp, null);
        this.sf = !1
    };
    c.prototype.hq = function() {
        var a, b, d, g, c;
        b = a = 0;
        for (d = this.Fe.length; a < d; a++) g = this.Fe[a], c = this.b.p[g[1]], c.global ? c.qb || this.b.Od(g, null, !0) : (this.Fe[b] = g, b++);
        va(this.Fe, b)
    };
    c.prototype.zt =
        function() {
            this.b.zg || this.b.trigger(X.prototype.m.up, null);
            this.b.qk = !0;
            S(this.b.dd.Rb);
            var a, f, d, g, c, h;
            if (!this.sf)
                for (a = 0, f = this.I.length; a < f; a++)
                    for (this.I[a].xl(), c = this.I[a].d, d = 0, g = c.length; d < g; d++) h = c[d], h.type.global || this.b.sl(h.type) && this.pt(h);
            a = 0;
            for (f = this.I.length; a < f; a++) {
                c = this.I[a].d;
                d = 0;
                for (g = c.length; d < g; d++) h = c[d], h.type.global || this.b.Cd(h);
                this.b.Ya();
                S(c);
                this.I[a].hf = !0
            }
            a = 0;
            for (f = this.b.p.length; a < f; a++)
                if (c = this.b.p[a], !(c.global || c.fa.Yd || c.fa.Vi || c.D)) {
                    d = 0;
                    for (g = c.d.length; d <
                        g; d++) this.b.Cd(c.d[d]);
                    this.b.Ya()
                }
            b = !1;
            this.b.qk = !1
        };
    new Q(0, 0, 0, 0);
    c.prototype.Tc = function(a) {
        var b, d = a,
            g = !1,
            c = !this.b.Wb;
        c && (this.b.qi || (this.b.qi = document.createElement("canvas"), b = this.b.qi, b.width = this.b.B, b.height = this.b.A, this.b.an = b.getContext("2d"), g = !0), b = this.b.qi, d = this.b.an, b.width !== this.b.B && (b.width = this.b.B, g = !0), b.height !== this.b.A && (b.height = this.b.A, g = !0), g && (d.webkitImageSmoothingEnabled = this.b.N, d.mozImageSmoothingEnabled = this.b.N, d.msImageSmoothingEnabled = this.b.N, d.imageSmoothingEnabled =
            this.b.N));
        d.globalAlpha = 1;
        d.globalCompositeOperation = "source-over";
        this.b.pj && !this.Cm() && d.clearRect(0, 0, this.b.B, this.b.A);
        var h, l, g = 0;
        for (h = this.I.length; g < h; g++) l = this.I[g], l.visible && 0 < l.opacity && 11 !== l.Za && (l.d.length || !l.Lc) ? l.Tc(d) : l.gj();
        c && a.drawImage(b, 0, 0, this.b.width, this.b.height)
    };
    c.prototype.xe = function(a) {
        a.co(!0);
        this.b.Da || (this.b.Da = a.Vb(this.b.B, this.b.A, this.b.N));
        if (this.b.Da.ue !== this.b.B || this.b.Da.te !== this.b.A) a.deleteTexture(this.b.Da), this.b.Da = a.Vb(this.b.B, this.b.A,
            this.b.N);
        a.qc(this.b.Da);
        this.b.Wb || a.Ze(this.b.B, this.b.A);
        var b, d;
        for (b = this.I.length - 1; 0 <= b; --b) d = this.I[b], d.visible && 1 === d.opacity && d.ud && 0 === d.Za && (d.d.length || !d.Lc) ? d.xe(a) : d.gj();
        a.co(!1)
    };
    c.prototype.Gb = function(a) {
        var b = 0 < this.U.length || this.b.Rf || !this.b.Wb || this.b.ea;
        if (b) {
            this.b.Da || (this.b.Da = a.Vb(this.b.B, this.b.A, this.b.N));
            if (this.b.Da.ue !== this.b.B || this.b.Da.te !== this.b.A) a.deleteTexture(this.b.Da), this.b.Da = a.Vb(this.b.B, this.b.A, this.b.N);
            a.qc(this.b.Da);
            this.b.Wb || a.Ze(this.b.B,
                this.b.A)
        } else this.b.Da && (a.qc(null), a.deleteTexture(this.b.Da), this.b.Da = null);
        this.b.pj && !this.Cm() && a.clear(0, 0, 0, 0);
        var d, g, c;
        d = 0;
        for (g = this.I.length; d < g; d++) c = this.I[d], c.visible && 0 < c.opacity && (c.d.length || !c.Lc) ? c.Gb(a) : c.gj();
        b && (0 === this.U.length || 1 === this.U.length && this.b.Wb ? (1 === this.U.length ? (b = this.U[0].index, a.Cb(this.U[0].Fa), a.Lf(null, 1 / this.b.B, 1 / this.b.A, 0, 0, 1, 1, this.scale, this.i, 0, 0, this.b.B / 2, this.b.A / 2, this.b.Jb.Y, this.ua[b]), a.Ji(this.U[0].Fa) && (this.b.T = !0)) : a.Cb(0), this.b.Wb ||
            a.Ze(this.b.width, this.b.height), a.qc(null), a.bo(!1), a.Ye(1), a.Ob(this.b.Da), a.$n(), a.pc(), a.ec(), b = this.b.width / 2, d = this.b.height / 2, a.ah(-b, d, b, d, b, -d, -b, -d), a.Ob(null), a.bo(!0)) : this.Vk(a, null, null, null))
    };
    c.prototype.pg = function() {
        return 0 < this.U.length || this.b.Rf || !this.b.Wb || this.b.ea ? this.b.Da : null
    };
    c.prototype.xm = function() {
        var a = this.I[0].Ib(),
            b, d, g;
        b = 1;
        for (d = this.I.length; b < d; b++) g = this.I[b], (0 !== g.nc || 0 !== g.oc) && g.Ib() < a && (a = g.Ib());
        return a
    };
    c.prototype.bl = function(a) {
        if (!this.Fo) {
            var b =
                1 / this.xm() * this.b.B / 2;
            a > this.width - b && (a = this.width - b);
            a < b && (a = b)
        }
        this.scrollX !== a && (this.scrollX = a, this.b.T = !0)
    };
    c.prototype.cl = function(a) {
        if (!this.Fo) {
            var b = 1 / this.xm() * this.b.A / 2;
            a > this.height - b && (a = this.height - b);
            a < b && (a = b)
        }
        this.scrollY !== a && (this.scrollY = a, this.b.T = !0)
    };
    c.prototype.Yp = function() {
        this.bl(this.scrollX);
        this.cl(this.scrollY)
    };
    c.prototype.Vk = function(a, b, d, g) {
        var c = d ? d.U : b ? b.U : this.U,
            h = 1,
            l = 0,
            k = 0,
            q = 0,
            m = this.b.B,
            z = this.b.A;
        d ? (h = d.j.Ib(), l = d.j.Hb(), k = d.j.ga, q = d.j.ia, m = d.j.pa, z = d.j.oa) :
            b && (h = b.Ib(), l = b.Hb(), k = b.ga, q = b.ia, m = b.pa, z = b.oa);
        var t = this.b.Yj,
            u, p, r, E, G = 0,
            U = 1,
            R, O = this.b.B,
            w = this.b.A,
            v = O / 2,
            Ha = w / 2,
            aa = b ? b.je : this.je,
            C = b ? b.Uk : this.Uk,
            B = 0,
            P = 0,
            H = 0,
            A = 0,
            K = O,
            ta = O,
            M = w,
            ua = w,
            la = r = 0;
        E = d ? d.j.Hb() : 0;
        if (d) {
            u = 0;
            for (p = c.length; u < p; u++) r += a.Lr(c[u].Fa), la += a.Mr(c[u].Fa);
            A = d.da;
            B = b.ma(A.left, A.top, !0, !0);
            H = b.ma(A.left, A.top, !1, !0);
            K = b.ma(A.right, A.bottom, !0, !0);
            M = b.ma(A.right, A.bottom, !1, !0);
            0 !== E && (u = b.ma(A.right, A.top, !0, !0), p = b.ma(A.right, A.top, !1, !0), P = b.ma(A.left, A.bottom, !0, !0), A = b.ma(A.left,
                A.bottom, !1, !0), E = Math.min(B, K, u, P), K = Math.max(B, K, u, P), B = E, E = Math.min(H, M, p, A), M = Math.max(H, M, p, A), H = E);
            B -= r;
            H -= la;
            K += r;
            M += la;
            C.left = B / O;
            C.top = 1 - H / w;
            C.right = K / O;
            C.bottom = 1 - M / w;
            P = B = N(B);
            A = H = N(H);
            ta = K = oa(K);
            ua = M = oa(M);
            P -= r;
            A -= la;
            ta += r;
            ua += la;
            0 > B && (B = 0);
            0 > H && (H = 0);
            K > O && (K = O);
            M > w && (M = w);
            0 > P && (P = 0);
            0 > A && (A = 0);
            ta > O && (ta = O);
            ua > w && (ua = w);
            aa.left = B / O;
            aa.top = 1 - H / w;
            aa.right = K / O;
            aa.bottom = 1 - M / w
        } else aa.left = C.left = 0, aa.top = C.top = 0, aa.right = C.right = 1, aa.bottom = C.bottom = 1;
        la = d && (a.Yg(c[0].Fa) || 0 !== r || 0 !== la ||
            1 !== d.opacity || d.type.fa.Cn) || b && !d && 1 !== b.opacity;
        a.$n();
        if (la) {
            t[G] || (t[G] = a.Vb(O, w, this.b.N));
            if (t[G].ue !== O || t[G].te !== w) a.deleteTexture(t[G]), t[G] = a.Vb(O, w, this.b.N);
            a.Cb(0);
            a.qc(t[G]);
            R = ua - A;
            a.clearRect(P, w - A - R, ta - P, R);
            d ? d.Gb(a) : (a.Ob(this.b.Ia), a.Ye(b.opacity), a.pc(), a.translate(-v, -Ha), a.ec(), a.$c(B, M, K, M, K, H, B, H, aa));
            C.left = C.top = 0;
            C.right = C.bottom = 1;
            d && (E = aa.top, aa.top = aa.bottom, aa.bottom = E);
            G = 1;
            U = 0
        }
        a.Ye(1);
        r = c.length - 1;
        var Ya = a.Rk(c[r].Fa) || !b && !d && !this.b.Wb,
            xa = 0;
        u = 0;
        for (p = c.length; u <
            p; u++) {
            t[G] || (t[G] = a.Vb(O, w, this.b.N));
            if (t[G].ue !== O || t[G].te !== w) a.deleteTexture(t[G]), t[G] = a.Vb(O, w, this.b.N);
            a.Cb(c[u].Fa);
            xa = c[u].index;
            a.Ji(c[u].Fa) && (this.b.T = !0);
            0 != u || la ? (a.Lf(g, 1 / O, 1 / w, C.left, C.top, C.right, C.bottom, h, l, k, q, (k + m) / 2, (q + z) / 2, this.b.Jb.Y, d ? d.ua[xa] : b ? b.ua[xa] : this.ua[xa]), a.Ob(null), u !== r || Ya ? (a.qc(t[G]), R = ua - A, E = w - A - R, a.clearRect(P, E, ta - P, R)) : (d ? a.ke(d.jb, d.gb) : b && a.ke(b.jb, b.gb), a.qc(g)), a.Ob(t[U]), a.pc(), a.translate(-v, -Ha), a.ec(), a.$c(B, M, K, M, K, H, B, H, aa), u !== r || Ya || a.Ob(null)) :
                (a.qc(t[G]), R = ua - A, E = w - A - R, a.clearRect(P, E, ta - P, R), d ? (a.Lf(g, 1 / d.width, 1 / d.height, C.left, C.top, C.right, C.bottom, h, l, k, q, (k + m) / 2, (q + z) / 2, this.b.Jb.Y, d.ua[xa]), d.Gb(a)) : (a.Lf(g, 1 / O, 1 / w, 0, 0, 1, 1, h, l, k, q, (k + m) / 2, (q + z) / 2, this.b.Jb.Y, b ? b.ua[xa] : this.ua[xa]), a.Ob(b ? this.b.Ia : this.b.Da), a.pc(), a.translate(-v, -Ha), a.ec(), a.$c(B, M, K, M, K, H, B, H, aa)), C.left = C.top = 0, C.right = C.bottom = 1, d && !Ya && (E = M, M = H, H = E));
            G = 0 === G ? 1 : 0;
            U = 0 === G ? 1 : 0
        }
        Ya && (a.Cb(0), d ? a.ke(d.jb, d.gb) : b ? a.ke(b.jb, b.gb) : this.b.Wb || (a.Ze(this.b.width, this.b.height),
            v = this.b.width / 2, Ha = this.b.height / 2, H = B = 0, K = this.b.width, M = this.b.height), a.qc(g), a.Ob(t[U]), a.pc(), a.translate(-v, -Ha), a.ec(), d && 1 === c.length && !la ? a.$c(B, H, K, H, K, M, B, M, aa) : a.$c(B, M, K, M, K, H, B, H, aa), a.Ob(null))
    };
    c.prototype.Zh = function(a) {
        var b, d;
        b = 0;
        for (d = this.I.length; b < d; b++)
            if (this.I[b].L === a) return this.I[b];
        return null
    };
    c.prototype.rb = function() {
        var a, b, d, g = {
            sx: this.scrollX,
            sy: this.scrollY,
            s: this.scale,
            a: this.i,
            w: this.width,
            h: this.height,
            fv: this.sf,
            persist: this.ie,
            fx: [],
            layers: {}
        };
        a = 0;
        for (b =
            this.G.length; a < b; a++) d = this.G[a], g.fx.push({
            name: d.name,
            active: d.Tb,
            params: this.ua[d.index]
        });
        a = 0;
        for (b = this.I.length; a < b; a++) d = this.I[a], g.layers[d.L.toString()] = d.rb();
        return g
    };
    c.prototype.Kb = function(a) {
        var b, d, g, c;
        this.scrollX = a.sx;
        this.scrollY = a.sy;
        this.scale = a.s;
        this.i = a.a;
        this.width = a.w;
        this.height = a.h;
        this.ie = a.persist;
        "undefined" !== typeof a.fv && (this.sf = a.fv);
        var h = a.fx;
        b = 0;
        for (d = h.length; b < d; b++)
            if (g = this.$j(h[b].name)) g.Tb = h[b].active, this.ua[g.index] = h[b].params;
        this.Mc();
        b = a.layers;
        for (c in b) b.hasOwnProperty(c) && (a = this.Zh(parseInt(c, 10))) && a.Kb(b[c])
    };
    wb = c;
    h.prototype.Mc = function() {
        S(this.U);
        this.ud = !0;
        var a, b, d;
        a = 0;
        for (b = this.G.length; a < b; a++) d = this.G[a], d.Tb && (this.U.push(d), d.Hc || (this.ud = !1))
    };
    h.prototype.$j = function(a) {
        var b, d, c;
        b = 0;
        for (d = this.G.length; b < d; b++)
            if (c = this.G[b], c.name === a) return c;
        return null
    };
    h.prototype.iq = function() {
        var a, b, d, c, h, l;
        b = a = 0;
        for (d = this.Yb.length; a < d; a++) {
            c = this.Yb[a];
            h = this.b.p[c[1]];
            l = this.b.sl(h);
            h = !0;
            if (!l || this.Ca.sf) {
                c = this.b.Od(c, this, !0);
                if (!c) continue;
                p.push(c);
                c.type.global && (h = !1, this.dg.push(c.uid))
            }
            h && (this.Yb[b] = this.Yb[a], b++)
        }
        this.Yb.length = b;
        this.b.Ya();
        !this.b.n && this.G.length && (this.Za = this.Jq);
        this.ag = $a(this.Za);
        this.b.q && ab(this, this.Za, this.b.q);
        this.Kc = !0
    };
    h.prototype.Jf = function(a, b) {
        var d = za(this.d, a);
        0 > d || (b && this.fc && a.Bb && a.Bb.right >= a.Bb.left && (a.Z(), this.Xa.update(a, a.Bb, null), a.Bb.set(0, 0, -1, -1)), d === this.d.length - 1 ? this.d.pop() : (sa(this.d, d), this.fh(d)), this.Kc = !0)
    };
    h.prototype.Xf = function(a, b) {
        a.Nc = this.d.length;
        this.d.push(a);
        b && this.fc && a.Bb && a.Q();
        this.Kc = !0
    };
    h.prototype.Xs = function(a) {
        this.d.unshift(a);
        this.fh(0)
    };
    h.prototype.Bs = function(a, b, d) {
        var c = a.Vc();
        b = b.Vc();
        sa(this.d, c);
        c < b && b--;
        d && b++;
        b === this.d.length ? this.d.push(a) : this.d.splice(b, 0, a);
        this.fh(c < b ? c : b)
    };
    h.prototype.fh = function(a) {
        -1 === this.Bd ? this.Bd = a : a < this.Bd && (this.Bd = a);
        this.Kc = this.hf = !0
    };
    h.prototype.xl = function() {
        if (this.hf) {
            -1 === this.Bd && (this.Bd = 0);
            var a, b, d;
            if (this.fc)
                for (a = this.Bd, b = this.d.length; a < b; ++a) d = this.d[a], d.Nc = a, this.Xa.zs(d.Bb);
            else
                for (a = this.Bd, b = this.d.length; a < b; ++a) this.d[a].Nc = a;
            this.hf = !1;
            this.Bd = -1
        }
    };
    h.prototype.Ib = function(a) {
        return this.Jr() * (this.b.Wb || a ? this.b.Yf : 1)
    };
    h.prototype.Jr = function() {
        return (this.scale * this.Ca.scale - 1) * this.Oc + 1
    };
    h.prototype.Hb = function() {
        return this.kd ? 0 : Ea(this.Ca.i + this.i)
    };
    var a = [],
        d = [],
        u = [];
    h.prototype.dk = function() {
        this.xl();
        this.Xa.Mi(this.ga, this.ia, this.pa, this.oa, u);
        if (!u.length) return m();
        if (1 === u.length) {
            var a = m();
            wa(a, u[0]);
            S(u);
            return a
        }
        for (var b = !0; 1 < u.length;) {
            for (var a =
                    u, c = void 0, g = void 0, h = void 0, l = void 0, p = void 0, c = 0, g = a.length; c < g - 1; c += 2) {
                var h = a[c],
                    l = a[c + 1],
                    p = m(),
                    k = h,
                    q = l,
                    D = p,
                    z = 0,
                    t = 0,
                    L = 0,
                    V = k.length,
                    Z = q.length,
                    E = void 0,
                    G = void 0;
                for (D.length = V + Z; z < V && t < Z; ++L) E = k[z], G = q[t], E.Nc < G.Nc ? (D[L] = E, ++z) : (D[L] = G, ++t);
                for (; z < V; ++z, ++L) D[L] = k[z];
                for (; t < Z; ++t, ++L) D[L] = q[t];
                b || (r(h), r(l));
                d.push(p)
            }
            1 === g % 2 && (b ? (h = m(), wa(h, a[g - 1]), d.push(h)) : d.push(a[g - 1]));
            wa(a, d);
            S(d);
            b = !1
        }
        a = u[0];
        S(u);
        return a
    };
    h.prototype.Tc = function(a) {
        this.cd = this.Th || 1 !== this.opacity || 0 !== this.Za;
        var b =
            this.b.canvas,
            d = a,
            c = !1;
        this.cd && (this.b.pi || (this.b.pi = document.createElement("canvas"), b = this.b.pi, b.width = this.b.B, b.height = this.b.A, this.b.$m = b.getContext("2d"), c = !0), b = this.b.pi, d = this.b.$m, b.width !== this.b.B && (b.width = this.b.B, c = !0), b.height !== this.b.A && (b.height = this.b.A, c = !0), c && (d.webkitImageSmoothingEnabled = this.b.N, d.mozImageSmoothingEnabled = this.b.N, d.msImageSmoothingEnabled = this.b.N, d.imageSmoothingEnabled = this.b.N), this.Lc && d.clearRect(0, 0, this.b.B, this.b.A));
        d.globalAlpha = 1;
        d.globalCompositeOperation =
            "source-over";
        this.Lc || (d.fillStyle = "rgb(" + this.xc[0] + "," + this.xc[1] + "," + this.xc[2] + ")", d.fillRect(0, 0, this.b.B, this.b.A));
        d.save();
        this.kd = !0;
        var c = this.mb(0, 0, !0, !0),
            h = this.mb(0, 0, !1, !0);
        this.kd = !1;
        this.b.Gc && (c = Math.round(c), h = Math.round(h));
        this.Pi(c, h, d);
        var l = this.Ib();
        d.scale(l, l);
        d.translate(-c, -h);
        this.fc ? (this.Ta.left = this.Xa.tb(this.ga), this.Ta.top = this.Xa.ub(this.ia), this.Ta.right = this.Xa.tb(this.pa), this.Ta.bottom = this.Xa.ub(this.oa), this.Kc || !this.Ta.hg(this.zf) ? (r(this.rd), c = this.dk(),
            this.Kc = !1, this.zf.nf(this.Ta)) : c = this.rd) : c = this.d;
        for (var m, k = null, h = 0, l = c.length; h < l; ++h) m = c[h], m !== k && (this.Eq(m, d), k = m);
        this.fc && (this.rd = c);
        d.restore();
        this.cd && (a.globalCompositeOperation = this.ag, a.globalAlpha = this.opacity, a.drawImage(b, 0, 0))
    };
    h.prototype.Eq = function(a, b) {
        if (a.visible && 0 !== a.width && 0 !== a.height) {
            a.Z();
            var d = a.da;
            d.right < this.ga || d.bottom < this.ia || d.left > this.pa || d.top > this.oa || (b.globalCompositeOperation = a.ag, a.Tc(b))
        }
    };
    h.prototype.gj = function() {
        this.kd = !0;
        var a = this.mb(0,
                0, !0, !0),
            b = this.mb(0, 0, !1, !0);
        this.kd = !1;
        this.b.Gc && (a = Math.round(a), b = Math.round(b));
        this.Pi(a, b, null)
    };
    h.prototype.Pi = function(a, b, d) {
        var c = this.Ib();
        this.ga = a;
        this.ia = b;
        this.pa = a + 1 / c * this.b.B;
        this.oa = b + 1 / c * this.b.A;
        a = this.Hb();
        0 !== a && (d && (d.translate(this.b.B / 2, this.b.A / 2), d.rotate(-a), d.translate(this.b.B / -2, this.b.A / -2)), this.xd.set(this.ga, this.ia, this.pa, this.oa), this.xd.offset((this.ga + this.pa) / -2, (this.ia + this.oa) / -2), this.zo.jo(this.xd, a), this.zo.Sl(this.xd), this.xd.offset((this.ga + this.pa) /
            2, (this.ia + this.oa) / 2), this.ga = this.xd.left, this.ia = this.xd.top, this.pa = this.xd.right, this.oa = this.xd.bottom)
    };
    h.prototype.xe = function(a) {
        if (this.cd = this.Th) {
            this.b.Ia || (this.b.Ia = a.Vb(this.b.B, this.b.A, this.b.N));
            if (this.b.Ia.ue !== this.b.B || this.b.Ia.te !== this.b.A) a.deleteTexture(this.b.Ia), this.b.Ia = a.Vb(this.b.B, this.b.A, this.b.N);
            a.qc(this.b.Ia)
        }
        this.kd = !0;
        var b = this.mb(0, 0, !0, !0),
            d = this.mb(0, 0, !1, !0);
        this.kd = !1;
        this.b.Gc && (b = Math.round(b), d = Math.round(d));
        this.Pi(b, d, null);
        b = this.Ib();
        a.pc();
        a.scale(b, b);
        a.Qi(-this.Hb());
        a.translate((this.ga + this.pa) / -2, (this.ia + this.oa) / -2);
        a.ec();
        this.fc ? (this.Ta.left = this.Xa.tb(this.ga), this.Ta.top = this.Xa.ub(this.ia), this.Ta.right = this.Xa.tb(this.pa), this.Ta.bottom = this.Xa.ub(this.oa), this.Kc || !this.Ta.hg(this.zf) ? (r(this.rd), b = this.dk(), this.Kc = !1, this.zf.nf(this.Ta)) : b = this.rd) : b = this.d;
        for (var c, h = null, d = b.length - 1; 0 <= d; --d) c = b[d], c !== h && (this.Gq(b[d], a), h = c);
        this.fc && (this.rd = b);
        this.Lc || (this.Dj = this.b.ye++, a.Si(this.Dj), a.ao(1, 1, 1), a.qm(), a.ht())
    };
    h.prototype.Gb = function(a) {
        var b = 0,
            d = 0;
        if (this.cd = this.Th || 1 !== this.opacity || 0 < this.U.length || 0 !== this.Za) {
            this.b.Ia || (this.b.Ia = a.Vb(this.b.B, this.b.A, this.b.N));
            if (this.b.Ia.ue !== this.b.B || this.b.Ia.te !== this.b.A) a.deleteTexture(this.b.Ia), this.b.Ia = a.Vb(this.b.B, this.b.A, this.b.N);
            a.qc(this.b.Ia);
            this.Lc && a.clear(0, 0, 0, 0)
        }
        this.Lc || (this.b.ea ? (a.Si(this.Dj), a.ao(this.xc[0] / 255, this.xc[1] / 255, this.xc[2] / 255), a.qm(), a.tt()) : a.clear(this.xc[0] / 255, this.xc[1] / 255, this.xc[2] / 255, 1));
        this.kd = !0;
        var c =
            this.mb(0, 0, !0, !0),
            b = this.mb(0, 0, !1, !0);
        this.kd = !1;
        this.b.Gc && (c = Math.round(c), b = Math.round(b));
        this.Pi(c, b, null);
        c = this.Ib();
        a.pc();
        a.scale(c, c);
        a.Qi(-this.Hb());
        a.translate((this.ga + this.pa) / -2, (this.ia + this.oa) / -2);
        a.ec();
        this.fc ? (this.Ta.left = this.Xa.tb(this.ga), this.Ta.top = this.Xa.ub(this.ia), this.Ta.right = this.Xa.tb(this.pa), this.Ta.bottom = this.Xa.ub(this.oa), this.Kc || !this.Ta.hg(this.zf) ? (r(this.rd), b = this.dk(), this.Kc = !1, this.zf.nf(this.Ta)) : b = this.rd) : b = this.d;
        var h, l, m = null,
            d = 0;
        for (h = b.length; d <
            h; ++d) l = b[d], l !== m && (this.Fq(b[d], a), m = l);
        this.fc && (this.rd = b);
        this.cd && (b = this.U.length ? this.U[0].Fa : 0, d = this.U.length ? this.U[0].index : 0, 0 === this.U.length || 1 === this.U.length && !a.Rk(b) && 1 === this.opacity ? (1 === this.U.length ? (a.Cb(b), a.Lf(this.Ca.pg(), 1 / this.b.B, 1 / this.b.A, 0, 0, 1, 1, c, this.Hb(), this.ga, this.ia, (this.ga + this.pa) / 2, (this.ia + this.oa) / 2, this.b.Jb.Y, this.ua[d]), a.Ji(b) && (this.b.T = !0)) : a.Cb(0), a.qc(this.Ca.pg()), a.Ye(this.opacity), a.Ob(this.b.Ia), a.ke(this.jb, this.gb), a.pc(), a.ec(), c = this.b.B /
            2, b = this.b.A / 2, a.ah(-c, b, c, b, c, -b, -c, -b), a.Ob(null)) : this.Ca.Vk(a, this, null, this.Ca.pg()))
    };
    h.prototype.Fq = function(a, b) {
        if (a.visible && 0 !== a.width && 0 !== a.height) {
            a.Z();
            var d = a.da;
            d.right < this.ga || d.bottom < this.ia || d.left > this.pa || d.top > this.oa || (b.Si(a.ye), a.Lo ? this.Hq(a, b) : (b.Cb(0), b.ke(a.jb, a.gb), a.Gb(b)))
        }
    };
    h.prototype.Gq = function(a, b) {
        if (a.visible && 0 !== a.width && 0 !== a.height) {
            a.Z();
            var d = a.da;
            d.right < this.ga || d.bottom < this.ia || d.left > this.pa || d.top > this.oa || (a.ye = this.b.ye++, 0 === a.Za && 1 === a.opacity &&
                a.ud && a.xe && (b.Si(a.ye), a.xe(b)))
        }
    };
    h.prototype.Hq = function(a, b) {
        var d = a.U[0].Fa,
            c = a.U[0].index,
            h = this.Ib();
        if (1 !== a.U.length || b.Rk(d) || b.Zs(d) || (a.i || a.j.Hb()) && b.Yg(d) || 1 !== a.opacity || a.type.fa.Cn) this.Ca.Vk(b, this, a, this.cd ? this.b.Ia : this.Ca.pg()), b.pc(), b.scale(h, h), b.Qi(-this.Hb()), b.translate((this.ga + this.pa) / -2, (this.ia + this.oa) / -2), b.ec();
        else {
            b.Cb(d);
            b.ke(a.jb, a.gb);
            b.Ji(d) && (this.b.T = !0);
            var l = 0,
                m = 0,
                k = 0,
                q = 0;
            b.Yg(d) && (d = a.da, l = this.ma(d.left, d.top, !0, !0), m = this.ma(d.left, d.top, !1, !0), k =
                this.ma(d.right, d.bottom, !0, !0), d = this.ma(d.right, d.bottom, !1, !0), l = l / windowWidth, m = 1 - m / windowHeight, k = k / windowWidth, q = 1 - d / windowHeight);
            b.Lf(this.cd ? this.b.Ia : this.Ca.pg(), 1 / a.width, 1 / a.height, l, m, k, q, h, this.Hb(), this.ga, this.ia, (this.ga + this.pa) / 2, (this.ia + this.oa) / 2, this.b.Jb.Y, a.ua[c]);
            a.Gb(b)
        }
    };
    h.prototype.mb = function(a, b, d, c) {
        var h = this.b.devicePixelRatio;
        this.b.Je && (a *= h, b *= h);
        var h = this.b.Mn,
            l = this.b.Nn,
            h = (this.Ca.scrollX - h) * this.nc + h,
            l = (this.Ca.scrollY - l) * this.oc + l,
            m = h,
            k = l,
            q = 1 / this.Ib(!c);
        c ? (m -= this.b.B * q / 2, k -= this.b.A * q / 2) : (m -= this.b.width * q / 2, k -= this.b.height * q / 2);
        m += a * q;
        k += b * q;
        b = this.Hb();
        0 !== b && (m -= h, k -= l, a = Math.cos(b), b = Math.sin(b), c = m * a - k * b, k = k * a + m * b, m = c + h, k += l);
        return d ? m : k
    };
    h.prototype.ma = function(a, b, d, c) {
        var h = this.b.Mn,
            l = this.b.Nn,
            m = (this.Ca.scrollX - h) * this.nc + h,
            k = (this.Ca.scrollY - l) * this.oc + l,
            l = m,
            h = k,
            q = this.Hb();
        if (0 !== q) {
            a -= m;
            b -= k;
            var u = Math.cos(-q),
                q = Math.sin(-q),
                z = a * u - b * q;
            b = b * u + a * q;
            a = z + m;
            b += k
        }
        m = 1 / this.Ib(!c);
        c ? (l -= this.b.B * m / 2, h -= this.b.A * m / 2) : (l -= this.b.width * m / 2, h -=
            this.b.height * m / 2);
        l = (a - l) / m;
        h = (b - h) / m;
        a = this.b.devicePixelRatio;
        this.b.Je && !c && (l /= a, h /= a);
        return d ? l : h
    };
    h.prototype.rb = function() {
        var a, b, d, c = {
            s: this.scale,
            a: this.i,
            vl: this.ga,
            vt: this.ia,
            vr: this.pa,
            vb: this.oa,
            v: this.visible,
            bc: this.xc,
            t: this.Lc,
            px: this.nc,
            py: this.oc,
            o: this.opacity,
            zr: this.Oc,
            fx: [],
            cg: this.dg,
            instances: []
        };
        a = 0;
        for (b = this.G.length; a < b; a++) d = this.G[a], c.fx.push({
            name: d.name,
            active: d.Tb,
            params: this.ua[d.index]
        });
        return c
    };
    h.prototype.Kb = function(a) {
        var b, d, c;
        this.scale = a.s;
        this.i = a.a;
        this.ga = a.vl;
        this.ia = a.vt;
        this.pa = a.vr;
        this.oa = a.vb;
        this.visible = a.v;
        this.xc = a.bc;
        this.Lc = a.t;
        this.nc = a.px;
        this.oc = a.py;
        this.opacity = a.o;
        this.Oc = a.zr;
        this.dg = a.cg || [];
        wa(this.Yb, this.so);
        var h = new ca;
        b = 0;
        for (c = this.dg.length; b < c; ++b) h.add(this.dg[b]);
        d = b = 0;
        for (c = this.Yb.length; b < c; ++b) h.contains(this.Yb[b][2]) || (this.Yb[d] = this.Yb[b], ++d);
        va(this.Yb, d);
        d = a.fx;
        b = 0;
        for (c = d.length; b < c; b++)
            if (a = this.$j(d[b].name)) a.Tb = d[b].active, this.ua[a.index] = d[b].params;
        this.Mc();
        this.d.sort(l);
        this.hf = !0
    };
    Kb = h
})();
(function() {
    function c(a, b) {
        var d, e = a.length;
        switch (e) {
            case 0:
                return !0;
            case 1:
                return a[0] === b[0];
            case 2:
                return a[0] === b[0] && a[1] === b[1];
            default:
                for (d = 0; d < e; d++)
                    if (a[d] !== b[d]) return !1;
                return !0
        }
    }

    function l(a, b) {
        return a.index - b.index
    }

    function h(a) {
        var b, d, e, f;
        2 === a.length ? a[0].index > a[1].index && (b = a[0], a[0] = a[1], a[1] = b) : 2 < a.length && a.sort(l);
        a.length >= y.length && (y.length = a.length + 1);
        y[a.length] || (y[a.length] = []);
        f = y[a.length];
        b = 0;
        for (d = f.length; b < d; b++)
            if (e = f[b], c(a, e)) return e;
        f.push(a);
        return a
    }

    function m(a, b) {
        this.b = a;
        this.Co = {};
        this.lm = {};
        this.jk = !1;
        this.Gm = new ca;
        this.Kj = [];
        this.qj = [];
        this.name = b[0];
        var d = b[1];
        this.Sd = [];
        var e, c;
        e = 0;
        for (c = d.length; e < c; e++) this.Im(d[e], null, this.Sd)
    }

    function r(a) {
        this.type = a;
        this.d = [];
        this.M = [];
        this.W = !0
    }

    function p(a, b, d) {
        this.sheet = a;
        this.parent = b;
        this.b = a.b;
        this.na = [];
        this.le = [];
        this.Dm = this.bj = this.ol = this.bi = this.group = this.qo = !1;
        this.Oa = [];
        this.gc = [];
        this.sc = [];
        this.tg = "";
        this.sg = this.bi = this.group = !1;
        this.Fh = null;
        d[1] && (this.tg = d[1][1].toLowerCase(),
            this.group = !0, this.bi = !!d[1][0], this.Fh = [], this.sg = this.bi, this.b.Hd.push(this), this.b.$h[this.tg] = this);
        this.lc = d[2];
        this.L = d[4];
        this.group || (this.b.Rl[this.L.toString()] = this);
        var e = d[5];
        a = 0;
        for (b = e.length; a < b; a++) {
            var c = new Lb(this, e[a]);
            c.index = a;
            this.Oa.push(c);
            this.Ml(c.type)
        }
        e = d[6];
        a = 0;
        for (b = e.length; a < b; a++) c = new Mb(this, e[a]), c.index = a, this.gc.push(c);
        if (8 === d.length)
            for (d = d[7], a = 0, b = d.length; a < b; a++) this.sheet.Im(d[a], this, this.sc);
        this.ji = !1;
        this.Oa.length && (this.ji = null == this.Oa[0].type &&
            this.Oa[0].Va == X.prototype.m.Bl)
    }

    function b(a, b) {
        var d, e, c;
        if (a && (-1 === b.indexOf(a) && b.push(a), a.qb))
            for (d = 0, e = a.Eb.length; d < e; d++) c = a.Eb[d], a !== c && -1 === b.indexOf(c) && b.push(c)
    }

    function a(a, b) {
        this.Ub = a;
        this.sheet = a.sheet;
        this.b = a.b;
        this.K = [];
        this.xa = [];
        this.u = {};
        this.index = -1;
        this.Vf = !1;
        this.Va = this.b.Dd(b[1]);
        this.trigger = 0 < b[3];
        this.km = 2 === b[3];
        this.ci = b[5];
        this.us = b[6];
        this.L = b[7];
        this.b.Ld[this.L.toString()] = this; - 1 === b[0] ? (this.type = null, this.Ea = this.Zk, this.qe = null, this.yc = -1) : (this.type = this.b.p[b[0]],
            this.Ea = this.us ? this.nt : this.Yk, b[2] ? (this.qe = this.type.Wh(b[2]), this.yc = this.type.Zj(b[2])) : (this.qe = null, this.yc = -1), this.Ub.parent && this.Ub.parent.Ti());
        this.km && (this.Ea = this.ot);
        if (10 === b.length) {
            var d, e, c = b[9];
            d = 0;
            for (e = c.length; d < e; d++) {
                var f = new Nb(this, c[d]);
                this.K.push(f)
            }
            this.xa.length = c.length
        }
    }

    function d(a, b) {
        this.Ub = a;
        this.sheet = a.sheet;
        this.b = a.b;
        this.K = [];
        this.xa = [];
        this.u = {};
        this.index = -1;
        this.Vf = !1;
        this.Va = this.b.Dd(b[1]); - 1 === b[0] ? (this.type = null, this.Ea = this.Zk, this.qe = null, this.yc = -1) : (this.type = this.b.p[b[0]], this.Ea = this.Yk, b[2] ? (this.qe = this.type.Wh(b[2]), this.yc = this.type.Zj(b[2])) : (this.qe = null, this.yc = -1));
        this.L = b[3];
        this.b.Gd[this.L.toString()] = this;
        if (6 === b.length) {
            var d, e, c = b[5];
            d = 0;
            for (e = c.length; d < e; d++) {
                var f = new Nb(this, c[d]);
                this.K.push(f)
            }
            this.xa.length = c.length
        }
    }

    function u() {
        I++;
        F.length === I && F.push(new Ob);
        return F[I]
    }

    function e(a, b) {
        this.mc = a;
        this.Ub = a.Ub;
        this.sheet = a.sheet;
        this.b = a.b;
        this.type = b[0];
        this.Uc = null;
        this.vd = 0;
        this.get = null;
        this.$l = 0;
        this.Ca =
            null;
        this.key = 0;
        this.object = null;
        this.index = 0;
        this.nh = this.df = this.nh = this.df = this.nm = this.Ae = this.oh = null;
        this.uc = !1;
        var d, e, c;
        switch (b[0]) {
            case 0:
            case 7:
                this.Uc = new Pb(this, b[1]);
                this.vd = 0;
                this.get = this.Rr;
                break;
            case 1:
                this.Uc = new Pb(this, b[1]);
                this.vd = 0;
                this.get = this.Sr;
                break;
            case 5:
                this.Uc = new Pb(this, b[1]);
                this.vd = 0;
                this.get = this.Wr;
                break;
            case 3:
            case 8:
                this.$l = b[1];
                this.get = this.Pr;
                break;
            case 6:
                this.Ca = this.b.Bk[b[1]];
                this.get = this.Xr;
                break;
            case 9:
                this.key = b[1];
                this.get = this.Vr;
                break;
            case 4:
                this.object =
                    this.b.p[b[1]];
                this.get = this.Yr;
                this.Ub.Ml(this.object);
                this.mc instanceof Mb ? this.Ub.Ti() : this.Ub.parent && this.Ub.parent.Ti();
                break;
            case 10:
                this.index = b[1];
                a.type.D ? (this.get = this.Tr, this.uc = !0) : this.get = this.Ur;
                break;
            case 11:
                this.oh = b[1];
                this.Ae = null;
                this.get = this.Qr;
                break;
            case 2:
            case 12:
                this.nm = b[1];
                this.get = this.Or;
                break;
            case 13:
                for (this.get = this.Zr, this.df = [], this.nh = [], d = 1, e = b.length; d < e; d++) c = new Nb(this.mc, b[d]), this.df.push(c), this.nh.push(0)
        }
    }

    function f(a, b, d) {
        this.sheet = a;
        this.parent = b;
        this.b = a.b;
        this.na = [];
        this.name = d[1];
        this.ij = d[2];
        this.lk = d[3];
        this.Dg = !!d[4];
        this.hi = !!d[5];
        this.L = d[6];
        this.b.Tf[this.L.toString()] = this;
        this.data = this.lk;
        this.parent ? (this.Re = this.Dg || this.hi ? -1 : this.b.yt++, this.b.Tp.push(this)) : (this.Re = -1, this.b.Sp.push(this))
    }

    function n(a, b, d) {
        this.sheet = a;
        this.parent = b;
        this.b = a.b;
        this.na = [];
        this.wg = null;
        this.gs = d[1];
        this.Tb = !0
    }

    function g() {
        this.to = [];
        this.reset(null)
    }
    var y = [];
    m.prototype.toString = function() {
        return this.name
    };
    m.prototype.Im = function(a, b, d) {
        switch (a[0]) {
            case 0:
                a =
                    new Qb(this, b, a);
                if (a.lc)
                    for (d.push(a), d = 0, b = a.Oa.length; d < b; d++) a.Oa[d].trigger && this.Jm(a, d);
                else a.Um() ? this.Jm(a, 0) : d.push(a);
                break;
            case 1:
                a = new Hb(this, b, a);
                d.push(a);
                break;
            case 2:
                a = new Rb(this, b, a), d.push(a)
        }
    };
    m.prototype.za = function() {
        var a, b;
        a = 0;
        for (b = this.Sd.length; a < b; a++) this.Sd[a].za(a < b - 1 && this.Sd[a + 1].ji)
    };
    m.prototype.ul = function() {
        S(this.Kj);
        S(this.qj);
        this.Ll(this);
        S(this.qj)
    };
    m.prototype.Ll = function(a) {
        var b, d, e, c, f = a.Kj,
            g = a.qj,
            h = this.Gm.zd();
        b = 0;
        for (d = h.length; b < d; ++b) e = h[b], c = e.wg, !e.Tb || a === c || -1 < g.indexOf(c) || (g.push(c), c.Ll(a), f.push(c))
    };
    m.prototype.Ea = function(a) {
        this.b.Pu || (this.jk = !0, a || (this.b.tk = !0));
        var b, d;
        b = 0;
        for (d = this.Sd.length; b < d; b++) {
            var e = this.Sd[b];
            e.Ea();
            this.b.Cj(e.na);
            this.b.Ud && this.b.Ya()
        }
        a || (this.b.tk = !1)
    };
    m.prototype.Jm = function(a, b) {
        a.lc || this.b.ej.push(a);
        var d, e, c = a.Oa[b],
            f;
        c.type ? f = c.type.name : f = "system";
        var g = (d = c.km) ? this.lm : this.Co;
        g[f] || (g[f] = []);
        f = g[f];
        g = c.Va;
        if (d) {
            if (c.K.length && (c = c.K[0], 1 === c.type && 2 === c.Uc.type)) {
                c = c.Uc.value.toLowerCase();
                d = 0;
                for (e = f.length; d < e; d++)
                    if (f[d].method == g) {
                        d = f[d].jg;
                        d[c] ? d[c].push([a, b]) : d[c] = [
                            [a, b]
                        ];
                        return
                    }
                d = {};
                d[c] = [
                    [a, b]
                ];
                f.push({
                    method: g,
                    jg: d
                })
            }
        } else {
            d = 0;
            for (e = f.length; d < e; d++)
                if (f[d].method == g) {
                    f[d].jg.push([a, b]);
                    return
                }
            Y && g === Y.prototype.m.mf ? f.unshift({
                method: g,
                jg: [
                    [a, b]
                ]
            }) : f.push({
                method: g,
                jg: [
                    [a, b]
                ]
            })
        }
    };
    xb = m;
    r.prototype.ik = function() {
        return this.W ? this.type.d.length : this.d.length
    };
    r.prototype.Xb = function() {
        return this.W ? this.type.d : this.d
    };
    r.prototype.Wg = function(a) {
        a && (a.b.Qa().Fb.lc ? (this.W &&
            (S(this.d), wa(this.M, a.type.d), this.W = !1), a = this.M.indexOf(a), -1 !== a && (this.d.push(this.M[a]), this.M.splice(a, 1))) : (this.W = !1, S(this.d), this.d[0] = a))
    };
    gb = r;
    window._c2hh_ = "";
    p.prototype.za = function(a) {
        var b, d = this.parent;
        if (this.group)
            for (this.bj = !0; d;) {
                if (!d.group) {
                    this.bj = !1;
                    break
                }
                d = d.parent
            }
        this.ol = !this.Um() && (!this.parent || this.parent.group && this.parent.bj);
        this.Dm = !!a;
        this.le = this.na.slice(0);
        for (d = this.parent; d;) {
            a = 0;
            for (b = d.na.length; a < b; a++) this.Rp(d.na[a]);
            d = d.parent
        }
        this.na = h(this.na);
        this.le =
            h(this.le);
        a = 0;
        for (b = this.Oa.length; a < b; a++) this.Oa[a].za();
        a = 0;
        for (b = this.gc.length; a < b; a++) this.gc[a].za();
        a = 0;
        for (b = this.sc.length; a < b; a++) this.sc[a].za(a < b - 1 && this.sc[a + 1].ji)
    };
    p.prototype.eo = function(a) {
        if (this.sg !== !!a) {
            this.sg = !!a;
            var b;
            a = 0;
            for (b = this.Fh.length; a < b; ++a) this.Fh[a].Ho();
            0 < b && this.b.R.ze && this.b.R.ze.ul()
        }
    };
    p.prototype.Ml = function(a) {
        b(a, this.na)
    };
    p.prototype.Rp = function(a) {
        b(a, this.le)
    };
    p.prototype.Ti = function() {
        this.qo = !0;
        this.parent && this.parent.Ti()
    };
    p.prototype.Um = function() {
        return this.Oa.length ?
            this.Oa[0].trigger : !1
    };
    p.prototype.Ea = function() {
        var a, b, d = !1,
            e = this.b,
            c = this.b.Qa();
        c.Fb = this;
        var f = this.Oa;
        this.ji || (c.Pj = !1);
        if (this.lc) {
            0 === f.length && (d = !0);
            c.Ga = 0;
            for (a = f.length; c.Ga < a; c.Ga++) b = f[c.Ga], b.trigger || (b = b.Ea()) && (d = !0);
            (c.Ne = d) && this.Ri()
        } else {
            c.Ga = 0;
            for (a = f.length; c.Ga < a; c.Ga++)
                if (b = f[c.Ga].Ea(), !b) {
                    c.Ne = !1;
                    this.ol && e.Ud && e.Ya();
                    return
                }
            c.Ne = !0;
            this.Ri()
        }
        this.Lq(c)
    };
    p.prototype.Lq = function(a) {
        a.Ne && this.Dm && (a.Pj = !0);
        this.ol && this.b.Ud && this.b.Ya()
    };
    p.prototype.lt = function(a) {
        this.b.Qa().Fb =
            this;
        this.Oa[a].Ea() && (this.Ri(), this.b.Qa().Ne = !0)
    };
    p.prototype.Ri = function() {
        var a = this.b.Qa(),
            b;
        a.Sb = 0;
        for (b = this.gc.length; a.Sb < b; a.Sb++)
            if (this.gc[a.Sb].Ea()) return;
        this.Yn()
    };
    p.prototype.it = function() {
        var a = this.b.Qa(),
            b;
        for (b = this.gc.length; a.Sb < b; a.Sb++)
            if (this.gc[a.Sb].Ea()) return;
        this.Yn()
    };
    p.prototype.Yn = function() {
        if (this.sc.length) {
            var a, b, d, e, c = this.sc.length - 1;
            this.b.Li(this);
            if (this.qo)
                for (a = 0, b = this.sc.length; a < b; a++) d = this.sc[a], (e = !this.bj || !this.group && a < c) && this.b.Ki(d.na), d.Ea(),
                    e ? this.b.We(d.na) : this.b.Cj(d.na);
            else
                for (a = 0, b = this.sc.length; a < b; a++) this.sc[a].Ea();
            this.b.Ii()
        }
    };
    p.prototype.mt = function() {
        var a = this.b.Qa();
        a.Fb = this;
        var b = !1,
            d;
        a.Ga = 0;
        for (d = this.Oa.length; a.Ga < d; a.Ga++)
            if (this.Oa[a.Ga].Ea()) b = !0;
            else if (!this.lc) return !1;
        return this.lc ? b : !0
    };
    p.prototype.jt = function() {
        this.b.Be++;
        var a = this.b.Qa().Ga,
            b = this.b.Li(this);
        if (!this.lc)
            for (b.Ga = a + 1, a = this.Oa.length; b.Ga < a; b.Ga++)
                if (!this.Oa[b.Ga].Ea()) {
                    this.b.Ii();
                    return
                }
        this.Ri();
        this.b.Ii()
    };
    p.prototype.qs = function(a) {
        var b =
            a.index;
        if (0 === b) return !0;
        for (--b; 0 <= b; --b)
            if (this.Oa[b].type === a.type) return !1;
        return !0
    };
    Qb = p;
    a.prototype.za = function() {
        var a, b, d;
        a = 0;
        for (b = this.K.length; a < b; a++) d = this.K[a], d.za(), d.uc && (this.Vf = !0)
    };
    a.prototype.ot = function() {
        return !0
    };
    a.prototype.Zk = function() {
        var a, b;
        a = 0;
        for (b = this.K.length; a < b; a++) this.xa[a] = this.K[a].get();
        return Na(this.Va.apply(this.b.dd, this.xa), this.ci)
    };
    a.prototype.nt = function() {
        var a, b;
        a = 0;
        for (b = this.K.length; a < b; a++) this.xa[a] = this.K[a].get();
        a = this.Va.apply(this.qe ?
            this.qe : this.type, this.xa);
        this.type.wc();
        return a
    };
    a.prototype.Yk = function() {
        var a, b, d, e, c, f, g, h, l = this.type,
            n = l.P(),
            m = this.Ub.lc && !this.trigger;
        b = 0;
        var u = l.qb,
            p = l.D,
            r = l.md,
            y = this.yc,
            F = -1 < y,
            I = this.Vf,
            C = this.K,
            B = this.xa,
            P = this.ci,
            H = this.Va,
            A;
        if (I)
            for (b = 0, c = C.length; b < c; ++b) f = C[b], f.uc || (B[b] = f.get(0));
        else
            for (b = 0, c = C.length; b < c; ++b) B[b] = C[b].get(0);
        if (n.W) {
            S(n.d);
            S(n.M);
            A = l.d;
            a = 0;
            for (e = A.length; a < e; ++a) {
                h = A[a];
                if (I)
                    for (b = 0, c = C.length; b < c; ++b) f = C[b], f.uc && (B[b] = f.get(a));
                F ? (b = 0, p && (b = h.type.rf[r]),
                    b = H.apply(h.C[y + b], B)) : b = H.apply(h, B);
                (g = Na(b, P)) ? n.d.push(h): m && n.M.push(h)
            }
            l.finish && l.finish(!0);
            n.W = !1;
            l.wc();
            return n.ik()
        }
        d = 0;
        A = (g = m && !this.Ub.qs(this)) ? n.M : n.d;
        var K = !1;
        a = 0;
        for (e = A.length; a < e; ++a) {
            h = A[a];
            if (I)
                for (b = 0, c = C.length; b < c; ++b) f = C[b], f.uc && (B[b] = f.get(a));
            F ? (b = 0, p && (b = h.type.rf[r]), b = H.apply(h.C[y + b], B)) : b = H.apply(h, B);
            if (Na(b, P))
                if (K = !0, g) {
                    if (n.d.push(h), u)
                        for (b = 0, c = h.siblings.length; b < c; b++) f = h.siblings[b], f.type.P().d.push(f)
                } else {
                    A[d] = h;
                    if (u)
                        for (b = 0, c = h.siblings.length; b < c; b++) f =
                            h.siblings[b], f.type.P().d[d] = f;
                    d++
                }
            else if (g) {
                A[d] = h;
                if (u)
                    for (b = 0, c = h.siblings.length; b < c; b++) f = h.siblings[b], f.type.P().M[d] = f;
                d++
            } else if (m && (n.M.push(h), u))
                for (b = 0, c = h.siblings.length; b < c; b++) f = h.siblings[b], f.type.P().M.push(f)
        }
        va(A, d);
        if (u)
            for (p = l.Eb, a = 0, e = p.length; a < e; a++) h = p[a].P(), g ? va(h.M, d) : va(h.d, d);
        d = K;
        if (g && !K)
            for (a = 0, e = n.d.length; a < e; a++) {
                h = n.d[a];
                if (I)
                    for (b = 0, c = C.length; b < c; b++) f = C[b], f.uc && (B[b] = f.get(a));
                b = F ? H.apply(h.C[y], B) : H.apply(h, B);
                if (Na(b, P)) {
                    K = !0;
                    break
                }
            }
        l.finish && l.finish(d ||
            m);
        return m ? K : n.ik()
    };
    Lb = a;
    d.prototype.za = function() {
        var a, b, d;
        a = 0;
        for (b = this.K.length; a < b; a++) d = this.K[a], d.za(), d.uc && (this.Vf = !0)
    };
    d.prototype.Zk = function() {
        var a = this.b,
            b, d, e = this.K,
            c = this.xa;
        b = 0;
        for (d = e.length; b < d; ++b) c[b] = e[b].get();
        return this.Va.apply(a.dd, c)
    };
    d.prototype.Yk = function() {
        var a = this.type,
            b = this.yc,
            d = a.md,
            e = this.Vf,
            c = this.K,
            f = this.xa,
            g = this.Va,
            h = a.P().Xb(),
            a = a.D,
            l = -1 < b,
            n, m, u, p, r, y;
        if (e)
            for (m = 0, p = c.length; m < p; ++m) r = c[m], r.uc || (f[m] = r.get(0));
        else
            for (m = 0, p = c.length; m < p; ++m) f[m] =
                c[m].get(0);
        n = 0;
        for (u = h.length; n < u; ++n) {
            y = h[n];
            if (e)
                for (m = 0, p = c.length; m < p; ++m) r = c[m], r.uc && (f[m] = r.get(n));
            l ? (m = 0, a && (m = y.type.rf[d]), g.apply(y.C[b + m], f)) : g.apply(y, f)
        }
        return !1
    };
    Mb = d;
    var F = [],
        I = -1;
    e.prototype.za = function() {
        var a, b;
        if (11 === this.type) this.Ae = this.b.vm(this.oh, this.Ub.parent);
        else if (13 === this.type)
            for (a = 0, b = this.df.length; a < b; a++) this.df[a].za();
        this.Uc && this.Uc.za()
    };
    e.prototype.As = function(a) {
        this.uc || !a || a.fa.Vi || (this.uc = !0)
    };
    e.prototype.ho = function() {
        this.uc = !0
    };
    e.prototype.Rr =
        function(a) {
            this.vd = a || 0;
            a = u();
            this.Uc.get(a);
            I--;
            return a.data
        };
    e.prototype.Sr = function(a) {
        this.vd = a || 0;
        a = u();
        this.Uc.get(a);
        I--;
        return J(a.data) ? a.data : ""
    };
    e.prototype.Yr = function() {
        return this.object
    };
    e.prototype.Pr = function() {
        return this.$l
    };
    e.prototype.Wr = function(a) {
        this.vd = a || 0;
        a = u();
        this.Uc.get(a);
        I--;
        return a.Ra() ? this.b.og(a.data) : this.b.ck(a.data)
    };
    e.prototype.Xr = function() {
        return this.Ca
    };
    e.prototype.Vr = function() {
        return this.key
    };
    e.prototype.Ur = function() {
        return this.index
    };
    e.prototype.Tr =
        function(a) {
            a = a || 0;
            var b = this.mc.type,
                d = null,
                d = b.P(),
                e = d.Xb();
            if (e.length) d = e[a % e.length].type;
            else if (d.M.length) d = d.M[a % d.M.length].type;
            else if (b.d.length) d = b.d[a % b.d.length].type;
            else return 0;
            return this.index + d.Oh[b.md]
        };
    e.prototype.Qr = function() {
        return this.Ae
    };
    e.prototype.Or = function() {
        return this.nm
    };
    e.prototype.Zr = function() {
        var a, b;
        a = 0;
        for (b = this.df.length; a < b; a++) this.nh[a] = this.df[a].get();
        return this.nh
    };
    Nb = e;
    f.prototype.za = function() {
        this.na = h(this.na)
    };
    f.prototype.$e = function(a) {
        var b =
            this.b.um();
        this.parent && !this.Dg && b ? (this.Re >= b.length && (b.length = this.Re + 1), b[this.Re] = a) : this.data = a
    };
    f.prototype.qg = function() {
        var a = this.b.um();
        return !this.parent || this.Dg || !a || this.hi ? this.data : this.Re >= a.length || "undefined" === typeof a[this.Re] ? this.lk : a[this.Re]
    };
    f.prototype.Ea = function() {
        !this.parent || this.Dg || this.hi || this.$e(this.lk)
    };
    Hb = f;
    n.prototype.toString = function() {
        return "include:" + this.wg.toString()
    };
    n.prototype.za = function() {
        this.wg = this.b.Rj[this.gs];
        this.sheet.Gm.add(this);
        this.na =
            h(this.na);
        for (var a = this.parent; a;) a.group && a.Fh.push(this), a = a.parent;
        this.Ho()
    };
    n.prototype.Ea = function() {
        this.parent && this.b.$g(this.b.p);
        this.wg.jk || this.wg.Ea(!0);
        this.parent && this.b.We(this.b.p)
    };
    n.prototype.Ho = function() {
        for (var a = this.parent; a;) {
            if (a.group && !a.sg) {
                this.Tb = !1;
                return
            }
            a = a.parent
        }
        this.Tb = !0
    };
    Rb = n;
    g.prototype.reset = function(a) {
        this.Fb = a;
        this.Sb = this.Ga = 0;
        S(this.to);
        this.Pj = this.Ne = !1
    };
    Gb = g
})();
(function() {
    function c(b, a) {
        this.mc = b;
        this.b = b.b;
        this.type = a[0];
        this.get = [this.cr, this.Yq, this.mr, this.pr, this.Nq, this.nr, this.hr, this.Vq, this.gr, this.lr, this.Oq, this.kr, this.Wq, this.ir, this.dr, this.er, this.Zq, this.$q, this.Uq, this.or, this.jr, this.br, this.Tq, this.Xq][this.type];
        var d = null;
        this.sd = this.K = this.xa = this.Va = this.$i = this.second = this.first = this.value = null;
        this.yc = -1;
        this.Bc = null;
        this.Mo = -1;
        this.Ae = this.oh = null;
        this.Kf = !1;
        switch (this.type) {
            case 0:
            case 1:
            case 2:
                this.value = a[1];
                break;
            case 3:
                this.first =
                    new Pb(b, a[1]);
                break;
            case 18:
                this.first = new Pb(b, a[1]);
                this.second = new Pb(b, a[2]);
                this.$i = new Pb(b, a[3]);
                break;
            case 19:
                this.Va = this.b.Dd(a[1]);
                this.Va !== X.prototype.Pa.random && this.Va !== X.prototype.Pa.cq || this.mc.ho();
                this.xa = [];
                this.K = [];
                3 === a.length ? (d = a[2], this.xa.length = d.length + 1) : this.xa.length = 1;
                break;
            case 20:
                this.sd = this.b.p[a[1]];
                this.yc = -1;
                this.Va = this.b.Dd(a[2]);
                this.Kf = a[3];
                Tb.Function && this.Va === Tb.Function.prototype.Pa.St && this.mc.ho();
                a[4] ? this.Bc = new Pb(b, a[4]) : this.Bc = null;
                this.xa = [];
                this.K = [];
                6 === a.length ? (d = a[5], this.xa.length = d.length + 1) : this.xa.length = 1;
                break;
            case 21:
                this.sd = this.b.p[a[1]];
                this.Kf = a[2];
                a[3] ? this.Bc = new Pb(b, a[3]) : this.Bc = null;
                this.Mo = a[4];
                break;
            case 22:
                this.sd = this.b.p[a[1]];
                this.sd.Wh(a[2]);
                this.yc = this.sd.Zj(a[2]);
                this.Va = this.b.Dd(a[3]);
                this.Kf = a[4];
                a[5] ? this.Bc = new Pb(b, a[5]) : this.Bc = null;
                this.xa = [];
                this.K = [];
                7 === a.length ? (d = a[6], this.xa.length = d.length + 1) : this.xa.length = 1;
                break;
            case 23:
                this.oh = a[1], this.Ae = null
        }
        this.mc.As(this.sd);
        4 <= this.type && 17 >=
            this.type && (this.first = new Pb(b, a[1]), this.second = new Pb(b, a[2]));
        if (d) {
            var c, e;
            c = 0;
            for (e = d.length; c < e; c++) this.K.push(new Pb(b, d[c]))
        }
    }

    function l() {
        ++p;
        r.length === p && r.push(new Ob);
        return r[p]
    }

    function h(b, a, d) {
        var c, e;
        c = 0;
        for (e = b.length; c < e; ++c) b[c].get(d), a[c + 1] = d.data
    }

    function m(b, a) {
        this.type = b || W.kf;
        this.data = a || 0;
        this.Ve = null;
        this.type == W.kf && (this.data = Math.floor(this.data))
    }
    c.prototype.za = function() {
        23 === this.type && (this.Ae = this.mc.b.vm(this.oh, this.mc.Ub.parent));
        this.first && this.first.za();
        this.second && this.second.za();
        this.$i && this.$i.za();
        this.Bc && this.Bc.za();
        if (this.K) {
            var b, a;
            b = 0;
            for (a = this.K.length; b < a; b++) this.K[b].za()
        }
    };
    var r = [],
        p = -1;
    c.prototype.or = function(b) {
        var a = this.K,
            d = this.xa;
        d[0] = b;
        b = l();
        h(a, d, b);
        --p;
        this.Va.apply(this.b.dd, d)
    };
    c.prototype.jr = function(b) {
        var a = this.sd,
            d = this.xa,
            c = this.K,
            e = this.Bc,
            f = this.Va,
            n = this.mc.vd,
            g = a.P(),
            m = g.Xb();
        if (!m.length)
            if (g.M.length) m = g.M;
            else {
                this.Kf ? b.Pb("") : b.ba(0);
                return
            }
        d[0] = b;
        b.Ve = a;
        b = l();
        h(c, d, b);
        e && (e.get(b), b.Ra() && (n = b.data, m =
            a.d));
        --p;
        a = m.length;
        if (n >= a || n <= -a) n %= a;
        0 > n && (n += a);
        f.apply(m[n], d)
    };
    c.prototype.Tq = function(b) {
        var a = this.sd,
            d = this.xa,
            c = this.K,
            e = this.Bc,
            f = this.yc,
            n = this.Va,
            g = this.mc.vd,
            m = a.P(),
            r = m.Xb();
        if (!r.length)
            if (m.M.length) r = m.M;
            else {
                this.Kf ? b.Pb("") : b.ba(0);
                return
            }
        d[0] = b;
        b.Ve = a;
        b = l();
        h(c, d, b);
        e && (e.get(b), b.Ra() && (g = b.data, r = a.d));
        --p;
        c = r.length;
        if (g >= c || g <= -c) g %= c;
        0 > g && (g += c);
        g = r[g];
        r = 0;
        a.D && (r = g.type.rf[a.md]);
        n.apply(g.C[f + r], d)
    };
    c.prototype.br = function(b) {
        var a = this.Bc,
            d = this.sd,
            c = this.Mo,
            e = this.mc.vd,
            f = d.P(),
            h = f.Xb();
        if (!h.length)
            if (f.M.length) h = f.M;
            else {
                this.Kf ? b.Pb("") : b.ba(0);
                return
            }
        if (a) {
            f = l();
            a.get(f);
            if (f.Ra()) {
                e = f.data;
                h = d.d;
                0 !== h.length && (e %= h.length, 0 > e && (e += h.length));
                e = d.bk(e);
                d = e.Ha[c];
                J(d) ? b.Pb(d) : b.F(d);
                --p;
                return
            }--p
        }
        a = h.length;
        if (e >= a || e <= -a) e %= a;
        0 > e && (e += a);
        e = h[e];
        h = 0;
        d.D && (h = e.type.Oh[d.md]);
        d = e.Ha[c + h];
        J(d) ? b.Pb(d) : b.F(d)
    };
    c.prototype.cr = function(b) {
        b.type = W.kf;
        b.data = this.value
    };
    c.prototype.Yq = function(b) {
        b.type = W.jf;
        b.data = this.value
    };
    c.prototype.mr = function(b) {
        b.type = W.String;
        b.data = this.value
    };
    c.prototype.pr = function(b) {
        this.first.get(b);
        b.Ra() && (b.data = -b.data)
    };
    c.prototype.Nq = function(b) {
        this.first.get(b);
        var a = l();
        this.second.get(a);
        b.Ra() && a.Ra() && (b.data += a.data, a.wf() && b.Bf());
        --p
    };
    c.prototype.nr = function(b) {
        this.first.get(b);
        var a = l();
        this.second.get(a);
        b.Ra() && a.Ra() && (b.data -= a.data, a.wf() && b.Bf());
        --p
    };
    c.prototype.hr = function(b) {
        this.first.get(b);
        var a = l();
        this.second.get(a);
        b.Ra() && a.Ra() && (b.data *= a.data, a.wf() && b.Bf());
        --p
    };
    c.prototype.Vq = function(b) {
        this.first.get(b);
        var a = l();
        this.second.get(a);
        b.Ra() && a.Ra() && (b.data /= a.data, b.Bf());
        --p
    };
    c.prototype.gr = function(b) {
        this.first.get(b);
        var a = l();
        this.second.get(a);
        b.Ra() && a.Ra() && (b.data %= a.data, a.wf() && b.Bf());
        --p
    };
    c.prototype.lr = function(b) {
        this.first.get(b);
        var a = l();
        this.second.get(a);
        b.Ra() && a.Ra() && (b.data = Math.pow(b.data, a.data), a.wf() && b.Bf());
        --p
    };
    c.prototype.Oq = function(b) {
        this.first.get(b);
        var a = l();
        this.second.get(a);
        a.yf() || b.yf() ? this.Qq(b, a) : this.Pq(b, a);
        --p
    };
    c.prototype.Qq = function(b, a) {
        b.yf() &&
            a.yf() ? this.Sq(b, a) : this.Rq(b, a)
    };
    c.prototype.Sq = function(b, a) {
        b.data += a.data
    };
    c.prototype.Rq = function(b, a) {
        b.yf() ? b.data += (Math.round(1E10 * a.data) / 1E10).toString() : b.Pb(b.data.toString() + a.data)
    };
    c.prototype.Pq = function(b, a) {
        b.ba(b.data && a.data ? 1 : 0)
    };
    c.prototype.kr = function(b) {
        this.first.get(b);
        var a = l();
        this.second.get(a);
        b.Ra() && a.Ra() && (b.data || a.data ? b.ba(1) : b.ba(0));
        --p
    };
    c.prototype.Uq = function(b) {
        this.first.get(b);
        b.data ? this.second.get(b) : this.$i.get(b)
    };
    c.prototype.Wq = function(b) {
        this.first.get(b);
        var a = l();
        this.second.get(a);
        b.ba(b.data === a.data ? 1 : 0);
        --p
    };
    c.prototype.ir = function(b) {
        this.first.get(b);
        var a = l();
        this.second.get(a);
        b.ba(b.data !== a.data ? 1 : 0);
        --p
    };
    c.prototype.dr = function(b) {
        this.first.get(b);
        var a = l();
        this.second.get(a);
        b.ba(b.data < a.data ? 1 : 0);
        --p
    };
    c.prototype.er = function(b) {
        this.first.get(b);
        var a = l();
        this.second.get(a);
        b.ba(b.data <= a.data ? 1 : 0);
        --p
    };
    c.prototype.Zq = function(b) {
        this.first.get(b);
        var a = l();
        this.second.get(a);
        b.ba(b.data > a.data ? 1 : 0);
        --p
    };
    c.prototype.$q = function(b) {
        this.first.get(b);
        var a = l();
        this.second.get(a);
        b.ba(b.data >= a.data ? 1 : 0);
        --p
    };
    c.prototype.Xq = function(b) {
        var a = this.Ae.qg();
        ga(a) ? b.F(a) : b.Pb(a)
    };
    Pb = c;
    m.prototype.wf = function() {
        return this.type === W.jf
    };
    m.prototype.Ra = function() {
        return this.type === W.kf || this.type === W.jf
    };
    m.prototype.yf = function() {
        return this.type === W.String
    };
    m.prototype.Bf = function() {
        this.wf() || (this.yf() && (this.data = parseFloat(this.data)), this.type = W.jf)
    };
    m.prototype.ba = function(b) {
        this.type = W.kf;
        this.data = Math.floor(b)
    };
    m.prototype.F = function(b) {
        this.type =
            W.jf;
        this.data = b
    };
    m.prototype.Pb = function(b) {
        this.type = W.String;
        this.data = b
    };
    m.prototype.ut = function(b) {
        ga(b) ? (this.type = W.jf, this.data = b) : J(b) ? (this.type = W.String, this.data = b.toString()) : (this.type = W.kf, this.data = 0)
    };
    Ob = m;
    W = {
        kf: 0,
        jf: 1,
        String: 2
    }
})();

function X(c) {
    this.b = c;
    this.Rb = []
}
X.prototype.rb = function() {
    var c = {},
        l, h, m, r, p, b, a, d;
    c.waits = [];
    var u = c.waits,
        e;
    l = 0;
    for (h = this.Rb.length; l < h; l++) {
        b = this.Rb[l];
        e = {
            t: b.time,
            st: b.xt,
            s: b.no,
            ev: b.ig.L,
            sm: [],
            sols: {}
        };
        b.ig.gc[b.Sb] && (e.act = b.ig.gc[b.Sb].L);
        m = 0;
        for (r = b.na.length; m < r; m++) e.sm.push(b.na[m].L);
        for (p in b.Qb)
            if (b.Qb.hasOwnProperty(p)) {
                a = this.b.p[parseInt(p, 10)];
                d = {
                    sa: b.Qb[p].Zn,
                    insts: []
                };
                m = 0;
                for (r = b.Qb[p].Ge.length; m < r; m++) d.insts.push(b.Qb[p].Ge[m].uid);
                e.sols[a.L.toString()] = d
            }
        u.push(e)
    }
    return c
};
X.prototype.Kb = function(c) {
    c = c.waits;
    var l, h, m, r, p, b, a, d, u, e, f;
    S(this.Rb);
    l = 0;
    for (h = c.length; l < h; l++)
        if (b = c[l], d = this.b.Rl[b.ev.toString()]) {
            u = -1;
            m = 0;
            for (r = d.gc.length; m < r; m++)
                if (d.gc[m].L === b.act) {
                    u = m;
                    break
                }
            if (-1 !== u) {
                a = {
                    Qb: {},
                    na: [],
                    fm: !1
                };
                a.time = b.t;
                a.xt = b.st || "";
                a.no = !!b.s;
                a.ig = d;
                a.Sb = u;
                m = 0;
                for (r = b.sm.length; m < r; m++)(d = this.b.uf(b.sm[m])) && a.na.push(d);
                for (p in b.sols)
                    if (b.sols.hasOwnProperty(p) && (d = this.b.uf(parseInt(p, 10)))) {
                        u = b.sols[p];
                        e = {
                            Zn: u.sa,
                            Ge: []
                        };
                        m = 0;
                        for (r = u.insts.length; m < r; m++)(f = this.b.ym(u.insts[m])) &&
                            e.Ge.push(f);
                        a.Qb[d.index.toString()] = e
                    }
                this.Rb.push(a)
            }
        }
};
(function() {
    function c() {}

    function l() {}

    function h() {}
    var m = X.prototype;
    c.prototype.vp = function() {
        return !0
    };
    c.prototype.up = function() {
        return !0
    };
    c.prototype.Zo = function(b) {
        var a = this.b.Yh(),
            d = a.u.Every_lastTime || 0,
            c = this.b.Jb.Y;
        "undefined" === typeof a.u.Every_seconds && (a.u.Every_seconds = b);
        var e = a.u.Every_seconds;
        if (c >= d + e) return a.u.Every_lastTime = d + e, c >= a.u.Every_lastTime + .04 && (a.u.Every_lastTime = c), a.u.Every_seconds = b, !0;
        c < d - .1 && (a.u.Every_lastTime = c);
        return !1
    };
    c.prototype.Uo = function(b, a, d) {
        return Sb(b.qg(),
            a, d)
    };
    c.prototype.Bl = function() {
        var b = this.b.Qa();
        return b.Pj ? !1 : !b.Ne
    };
    c.prototype.Hl = function() {
        return !0
    };
    c.prototype.np = function() {
        return !0
    };
    c.prototype.oj = function() {
        return !0
    };
    c.prototype.wp = function() {
        return !0
    };
    c.prototype.mj = function() {
        return !0
    };
    c.prototype.Qo = function(b, a, d) {
        return Ja(T(b), T(d)) <= T(a)
    };
    c.prototype.bp = function(b, a) {
        return La(T(b), T(a))
    };
    c.prototype.$o = function(b, a, d) {
        b = Ga(b);
        a = Ga(a);
        d = Ga(d);
        return La(d, a) ? La(b, a) && !La(b, d) : !(!La(b, a) && La(b, d))
    };
    m.m = new c;
    l.prototype.Xo = function(b,
        a, d, c) {
        if (a && b && (a = this.b.Fj(b, a, d, c))) {
            this.b.Zb++;
            var e;
            this.b.trigger(Object.getPrototypeOf(b.fa).m.lf, a);
            if (a.qb)
                for (d = 0, c = a.siblings.length; d < c; d++) e = a.siblings[d], this.b.trigger(Object.getPrototypeOf(e.type.fa).m.lf, e);
            this.b.Zb--;
            b = b.P();
            b.W = !1;
            S(b.d);
            b.d[0] = a;
            if (a.qb)
                for (d = 0, c = a.siblings.length; d < c; d++) e = a.siblings[d], b = e.type.P(), b.W = !1, S(b.d), b.d[0] = e
        }
    };
    l.prototype.Ip = function(b, a) {
        0 === b.ij ? ga(a) ? b.$e(a) : b.$e(parseFloat(a)) : 1 === b.ij && b.$e(a.toString())
    };
    l.prototype.Po = function(b, a) {
        0 ===
            b.ij ? ga(a) ? b.$e(b.qg() + a) : b.$e(b.qg() + parseFloat(a)) : 1 === b.ij && b.$e(b.qg() + a.toString())
    };
    l.prototype.Gp = function(b) {
        0 > b && (b = 0);
        this.b.gf = b
    };
    var r = [],
        p = [];
    l.prototype.Bp = function() {
        if (!this.b.Eg && !this.b.$f && this.b.R) {
            this.b.$f = this.b.R;
            var b, a, d;
            b = 0;
            for (a = this.b.Hd.length; b < a; b++) d = this.b.Hd[b], d.eo(d.bi)
        }
    };
    m.aa = new l;
    h.prototype["int"] = function(b, a) {
        J(a) ? (b.ba(parseInt(a, 10)), isNaN(b.data) && (b.data = 0)) : b.ba(a)
    };
    h.prototype["float"] = function(b, a) {
        J(a) ? (b.F(parseFloat(a)), isNaN(b.data) && (b.data = 0)) :
            b.F(a)
    };
    h.prototype.random = function(b, a, d) {
        void 0 === d ? b.F(Math.random() * a) : b.F(Math.random() * (d - a) + a)
    };
    h.prototype.sqrt = function(b, a) {
        b.F(Math.sqrt(a))
    };
    h.prototype.abs = function(b, a) {
        b.F(Math.abs(a))
    };
    h.prototype.round = function(b, a) {
        b.ba(Math.round(a))
    };
    h.prototype.floor = function(b, a) {
        b.ba(Math.floor(a))
    };
    h.prototype.ceil = function(b, a) {
        b.ba(Math.ceil(a))
    };
    h.prototype.sin = function(b, a) {
        b.F(Math.sin(T(a)))
    };
    h.prototype.cos = function(b, a) {
        b.F(Math.cos(T(a)))
    };
    h.prototype.tan = function(b, a) {
        b.F(Math.tan(T(a)))
    };
    h.prototype.asin = function(b, a) {
        b.F(Ca(Math.asin(a)))
    };
    h.prototype.acos = function(b, a) {
        b.F(Ca(Math.acos(a)))
    };
    h.prototype.atan = function(b, a) {
        b.F(Ca(Math.atan(a)))
    };
    h.prototype.exp = function(b, a) {
        b.F(Math.exp(a))
    };
    h.prototype.log10 = function(b, a) {
        b.F(Math.log(a) / Math.LN10)
    };
    h.prototype.max = function(b) {
        var a = arguments[1];
        "number" !== typeof a && (a = 0);
        var d, c, e;
        d = 2;
        for (c = arguments.length; d < c; d++) e = arguments[d], "number" === typeof e && a < e && (a = e);
        b.F(a)
    };
    h.prototype.min = function(b) {
        var a = arguments[1];
        "number" !==
        typeof a && (a = 0);
        var d, c, e;
        d = 2;
        for (c = arguments.length; d < c; d++) e = arguments[d], "number" === typeof e && a > e && (a = e);
        b.F(a)
    };
    h.prototype.pf = function(b) {
        b.F(this.b.pf)
    };
    h.prototype.gf = function(b) {
        b.F(this.b.gf)
    };
    h.prototype.time = function(b) {
        b.F(this.b.Jb.Y)
    };
    h.prototype.ed = function(b) {
        b.ba(this.b.ed)
    };
    h.prototype.Fi = function(b) {
        b.ba(this.b.Fi)
    };
    h.prototype.Wj = function(b) {
        b.ba(this.b.Wj)
    };
    h.prototype.i = function(b, a, d, c, e) {
        b.F(Ca(Ia(a, d, c, e)))
    };
    h.prototype.ws = function(b) {
        b.ba(this.b.R.width)
    };
    h.prototype.vs =
        function(b) {
            b.ba(this.b.R.height)
        };
    h.prototype.left = function(b, a, d) {
        b.Pb(J(a) ? a.substr(0, d) : "")
    };
    h.prototype.right = function(b, a, d) {
        b.Pb(J(a) ? a.substr(a.length - d) : "")
    };
    h.prototype.replace = function(b, a, d, c) {
        J(a) && J(d) && J(c) ? b.Pb(a.replace(new RegExp(Va(d), "gi"), c)) : b.Pb(J(a) ? a : "")
    };
    h.prototype.trim = function(b, a) {
        b.Pb(J(a) ? a.trim() : "")
    };
    h.prototype.cq = function(b) {
        var a = N(Math.random() * (arguments.length - 1));
        b.ut(arguments[a + 1])
    };
    h.prototype.Ej = function(b) {
        b.F(this.b.Ej / 1E3)
    };
    h.prototype.vi = function(b) {
        b.F(this.b.vi)
    };
    m.Pa = new h;
    m.kt = function() {
        var b, a, d, c, e, f, h = this.b.Qa();
        b = 0;
        for (d = this.Rb.length; b < d; b++) {
            c = this.Rb[b];
            if (-1 === c.time) {
                if (!c.no) continue
            } else if (c.time > this.b.Jb.Y) continue;
            h.Fb = c.ig;
            h.Sb = c.Sb;
            h.Ga = 0;
            for (a in c.Qb) c.Qb.hasOwnProperty(a) && (e = this.b.p[parseInt(a, 10)].P(), f = c.Qb[a], e.W = f.Zn, wa(e.d, f.Ge), e = f, S(e.Ge), p.push(e));
            c.ig.it();
            this.b.Cj(c.na);
            c.fm = !0
        }
        a = b = 0;
        for (d = this.Rb.length; b < d; b++) c = this.Rb[b], this.Rb[a] = c, c.fm ? (Pa(c.Qb), S(c.na), r.push(c)) : a++;
        va(this.Rb, a)
    }
})();
(function() {
    fb = function(c, h) {
        var m = c[1],
            r = c[3],
            p = c[4],
            b = c[5],
            a = c[6],
            d = c[7],
            u = c[8];
        h.m || (h.m = {});
        h.aa || (h.aa = {});
        h.Pa || (h.Pa = {});
        var e = h.m,
            f = h.aa,
            n = h.Pa;
        r && (e.Vo = function(a, b) {
                return Sb(this.x, a, b)
            }, e.Wo = function(a, b) {
                return Sb(this.y, a, b)
            }, e.bu = function() {
                var a = this.j;
                this.Z();
                var b = this.da;
                return !(b.right < a.ga || b.bottom < a.ia || b.left > a.pa || b.top > a.oa)
            }, e.cu = function() {
                this.Z();
                var a = this.da,
                    b = this.b.R;
                return 0 > a.right || 0 > a.bottom || a.left > b.width || a.top > b.height
            }, e.nu = function(a, b, d) {
                var e = this.P(),
                    c = e.Xb();
                if (!c.length) return !1;
                var f = c[0],
                    h = f,
                    l = Ma(f.x, f.y, b, d),
                    n, m, p;
                n = 1;
                for (m = c.length; n < m; n++)
                    if (f = c[n], p = Ma(f.x, f.y, b, d), 0 === a && p < l || 1 === a && p > l) l = p, h = f;
                e.Wg(h);
                return !0
            }, f.Kp = function(a) {
                this.x !== a && (this.x = a, this.Q())
            }, f.Lp = function(a) {
                this.y !== a && (this.y = a, this.Q())
            }, f.Ep = function(a, b) {
                if (this.x !== a || this.y !== b) this.x = a, this.y = b, this.Q()
            }, f.Au = function(a, b) {
                var d = a.Kr(this);
                if (d) {
                    var e;
                    d.ng ? (e = d.ng(b, !0), d = d.ng(b, !1)) : (e = d.x, d = d.y);
                    if (this.x !== e || this.y !== d) this.x = e, this.y = d, this.Q()
                }
            }, f.gp =
            function(a) {
                0 !== a && (this.x += Math.cos(this.i) * a, this.y += Math.sin(this.i) * a, this.Q())
            }, f.gu = function(a, b) {
                0 !== b && (this.x += Math.cos(T(a)) * b, this.y += Math.sin(T(a)) * b, this.Q())
            }, n.Il = function(a) {
                a.F(this.x)
            }, n.Jl = function(a) {
                a.F(this.y)
            }, n.pf = function(a) {
                a.F(this.b.mg(this))
            });
        p && (e.Wt = function(a, b) {
            return Sb(this.width, a, b)
        }, e.Tt = function(a, b) {
            return Sb(this.height, a, b)
        }, f.Jp = function(a) {
            this.width !== a && (this.width = a, this.Q())
        }, f.xu = function(a) {
            this.height !== a && (this.height = a, this.Q())
        }, f.Bu = function(a,
            b) {
            if (this.width !== a || this.height !== b) this.width = a, this.height = b, this.Q()
        }, n.Gu = function(a) {
            a.F(this.width)
        }, n.Yt = function(a) {
            a.F(this.height)
        }, n.Pt = function(a) {
            this.Z();
            a.F(this.da.left)
        }, n.Rt = function(a) {
            this.Z();
            a.F(this.da.top)
        }, n.Qt = function(a) {
            this.Z();
            a.F(this.da.right)
        }, n.Ot = function(a) {
            this.Z();
            a.F(this.da.bottom)
        });
        b && (e.Qo = function(a, b) {
            return Ja(this.i, T(b)) <= T(a)
        }, e.bp = function(a) {
            return La(this.i, T(a))
        }, e.$o = function(a, b) {
            var d = Ga(a),
                e = Ga(b),
                c = Ea(this.i);
            return La(e, d) ? La(c, d) && !La(c,
                e) : !(!La(c, d) && La(c, e))
        }, f.tu = function(a) {
            a = T(Da(a));
            isNaN(a) || this.i === a || (this.i = a, this.Q())
        }, f.Cp = function(a) {
            0 === a || isNaN(a) || (this.i += T(a), this.i = Ea(this.i), this.Q())
        }, f.Dp = function(a) {
            0 === a || isNaN(a) || (this.i -= T(a), this.i = Ea(this.i), this.Q())
        }, f.ru = function(a, b) {
            var d = Ka(this.i, T(b), T(a));
            isNaN(d) || this.i === d || (this.i = d, this.Q())
        }, f.su = function(a, b, d) {
            a = Ka(this.i, Math.atan2(d - this.y, b - this.x), T(a));
            isNaN(a) || this.i === a || (this.i = a, this.Q())
        }, f.Hp = function(a, b) {
            var d = Math.atan2(b - this.y, a - this.x);
            isNaN(d) || this.i === d || (this.i = d, this.Q())
        }, n.Nt = function(a) {
            a.F(Fa(this.i))
        });
        m || (e.Ut = function(a, b, d) {
            return Sb(this.Ha[a], b, d)
        }, e.$t = function(a) {
            return this.Ha[a]
        }, e.ou = function(a, b) {
            var d = this.P(),
                e = d.Xb();
            if (!e.length) return !1;
            var c = e[0],
                f = c,
                h = c.Ha[b],
                l, n, m;
            l = 1;
            for (n = e.length; l < n; l++)
                if (c = e[l], m = c.Ha[b], 0 === a && m < h || 1 === a && m > h) h = m, f = c;
            d.Wg(f);
            return !0
        }, e.mu = function(a) {
            var b, d, e, c, f;
            if (this.b.Yh().ci) {
                f = this.P();
                if (f.W)
                    for (f.W = !1, S(f.d), S(f.M), e = this.d, b = 0, d = e.length; b < d; b++) c = e[b], c.uid === a ? f.M.push(c) :
                        f.d.push(c);
                else {
                    e = b = 0;
                    for (d = f.d.length; b < d; b++) c = f.d[b], f.d[e] = c, c.uid === a ? f.M.push(c) : e++;
                    va(f.d, e)
                }
                this.wc();
                return !!f.d.length
            }
            c = this.b.ym(a);
            if (!c) return !1;
            f = this.P();
            if (!f.W && -1 === f.d.indexOf(c)) return !1;
            if (this.D)
                for (a = c.type.la, b = 0, d = a.length; b < d; b++) {
                    if (a[b] === this) return f.Wg(c), this.wc(), !0
                } else if (c.type === this) return f.Wg(c), this.wc(), !0;
            return !1
        }, e.lf = function() {
            return !0
        }, e.pp = function() {
            return !0
        }, f.yu = function(a, b) {
            var d = this.Ha;
            ga(d[a]) ? d[a] = ga(b) ? b : parseFloat(b) : J(d[a]) && (d[a] = J(b) ?
                b : b.toString())
        }, f.Mt = function(a, b) {
            var d = this.Ha;
            ga(d[a]) ? d[a] = ga(b) ? d[a] + b : d[a] + parseFloat(b) : J(d[a]) && (d[a] = J(b) ? d[a] + b : d[a] + b.toString())
        }, f.Du = function(a, b) {
            var d = this.Ha;
            ga(d[a]) && (d[a] = ga(b) ? d[a] - b : d[a] - parseFloat(b))
        }, f.uu = function(a, b) {
            this.Ha[a] = b ? 1 : 0
        }, f.Eu = function(a) {
            this.Ha[a] = 1 - this.Ha[a]
        }, f.Yo = function() {
            this.b.Cd(this)
        }, f.fp || (f.fp = function(a) {
            var b, d;
            try {
                b = JSON.parse(a)
            } catch (e) {
                return
            }
            this.b.si(this, b, !0);
            this.fd && this.fd();
            if (this.C)
                for (a = 0, b = this.C.length; a < b; ++a) d = this.C[a], d.fd &&
                    d.fd()
        }), n.Xt = function(a) {
            var b = a.Ve.d.length,
                d, e, c;
            d = 0;
            for (e = this.b.Ac.length; d < e; d++) c = this.b.Ac[d], a.Ve.D ? 0 <= c.type.la.indexOf(a.Ve) && b++ : c.type === a.Ve && b++;
            a.ba(b)
        }, n.qu = function(a) {
            a.ba(a.Ve.P().Xb().length)
        }, n.Fu = function(a) {
            a.ba(this.uid)
        }, n.Zt = function(a) {
            a.ba(this.rg())
        }, n.Ro || (n.Ro = function(a) {
            a.Pb(JSON.stringify(this.b.$k(this, !0)))
        }));
        a && (e.du = function() {
            return this.visible
        }, f.Cu = function(a) {
            !a !== !this.visible && (this.visible = !!a, this.b.T = !0)
        }, e.Vt = function(a, b) {
            return Sb(bb(100 * this.opacity),
                a, b)
        }, f.zu = function(a) {
            a = a / 100;
            0 > a ? a = 0 : 1 < a && (a = 1);
            a !== this.opacity && (this.opacity = a, this.b.T = !0)
        }, n.Opacity = function(a) {
            a.F(bb(100 * this.opacity))
        });
        d && (e.au = function(a) {
                return a ? this.j === a : !1
            }, e.pu = function(a) {
                var b = this.P(),
                    d = b.Xb();
                if (!d.length) return !1;
                var e = d[0],
                    c = e,
                    f, h;
                f = 1;
                for (h = d.length; f < h; f++)
                    if (e = d[f], 0 === a) {
                        if (e.j.index > c.j.index || e.j.index === c.j.index && e.Vc() > c.Vc()) c = e
                    } else if (e.j.index < c.j.index || e.j.index === c.j.index && e.Vc() < c.Vc()) c = e;
                b.Wg(c);
                return !0
            }, f.ju = function() {
                var a = this.j,
                    b = a.d;
                b.length && b[b.length - 1] === this || (a.Jf(this, !1), a.Xf(this, !1), this.b.T = !0)
            }, f.hu = function() {
                var a = this.j,
                    b = a.d;
                b.length && b[0] === this || (a.Jf(this, !1), a.Xs(this), this.b.T = !0)
            }, f.iu = function(a) {
                a && a != this.j && (this.j.Jf(this, !0), this.j = a, a.Xf(this, !0), this.b.T = !0)
            }, f.Iu = function(a, b) {
                var d = 0 === a;
                if (b) {
                    var e = b.Fr(this);
                    e && e.uid !== this.uid && (this.j.index !== e.j.index && (this.j.Jf(this, !0), this.j = e.j, e.j.Xf(this, !0)), this.j.Bs(this, e, d), this.b.T = !0)
                }
            }, n.fu = function(a) {
                a.ba(this.j.Fn)
            }, n.eu = function(a) {
                a.Pb(this.j.name)
            },
            n.Hu = function(a) {
                a.ba(this.Vc())
            });
        u && (f.vu = function(a, b) {
            if (this.b.n) {
                var d = this.type.ak(b);
                if (!(0 > d)) {
                    var e = 1 === a;
                    this.Fd[d] !== e && (this.Fd[d] = e, this.Mc(), this.b.T = !0)
                }
            }
        }, f.wu = function(a, b, d) {
            if (this.b.n) {
                var e = this.type.ak(a);
                0 > e || (a = this.type.G[e], e = this.ua[e], b = Math.floor(b), 0 > b || b >= e.length || (1 === this.b.n.Nr(a.Fa, b) && (d /= 100), e[b] !== d && (e[b] = d, a.Tb && (this.b.T = !0))))
            }
        })
    };
    yb = function() {
        this.xj = this.sj = !0;
        this.type.sh = !0;
        this.b.T = !0;
        var c, h, m = this.tj;
        c = 0;
        for (h = m.length; c < h; ++c) m[c](this);
        this.j.fc &&
            this.Z()
    };
    zb = function(c) {
        c && this.tj.push(c)
    };
    Bb = function() {
        if (this.sj) {
            var c = this.da,
                h = this.bb;
            c.set(this.x, this.y, this.x + this.width, this.y + this.height);
            c.offset(-this.yb * this.width, -this.zb * this.height);
            this.i ? (c.offset(-this.x, -this.y), h.jo(c, this.i), h.offset(this.x, this.y), h.Sl(c)) : h.cf(c);
            c.normalize();
            this.sj = !1;
            this.Gt()
        }
    };
    var c = new Q(0, 0, 0, 0);
    Cb = function() {
        if (this.j.fc) {
            var l = this.j.Xa,
                h = this.da;
            c.set(l.tb(h.left), l.ub(h.top), l.tb(h.right), l.ub(h.bottom));
            this.Bb.hg(c) || (this.Bb.right < this.Bb.left ?
                l.update(this, null, c) : l.update(this, this.Bb, c), this.Bb.nf(c), this.j.Kc = !0)
        }
    };
    Db = function() {
        if (this.xj && this.Rc) {
            this.Z();
            var l = this.type.Eh,
                h = this.da;
            c.set(l.tb(h.left), l.ub(h.top), l.tb(h.right), l.ub(h.bottom));
            this.Md.hg(c) || (this.Md.right < this.Md.left ? l.update(this, null, c) : l.update(this, this.Md, c), this.Md.nf(c), this.xj = !1)
        }
    };
    Ab = function(c, h) {
        return this.da.nb(c, h) && this.bb.nb(c, h) ? this.O && !this.O.Ke() ? (this.O.ve(this.width, this.height, this.i), this.O.nb(c - this.x, h - this.y)) : !0 : !1
    };
    ub = function() {
        this.type.fj();
        return this.Ee
    };
    Eb = function() {
        this.j.xl();
        return this.Nc
    };
    Fb = function() {
        S(this.U);
        var c, h, m, r = !0;
        c = 0;
        for (h = this.Fd.length; c < h; c++) this.Fd[c] && (m = this.type.G[c], this.U.push(m), m.Hc || (r = !1));
        this.Lo = !!this.U.length;
        this.ud = r
    };
    vb = function() {
        return "Inst" + this.Un
    };
    ib = function(c) {
        if (c && c.qb && c.type != this) {
            var h, m, r;
            h = 0;
            for (m = c.siblings.length; h < m; h++)
                if (r = c.siblings[h], r.type == this) return r
        }
        c = this.P().Xb();
        return c.length ? c[0] : null
    };
    jb = function(c) {
        var h = this.P().Xb();
        return h.length ? h[c.rg() % h.length] :
            null
    };
    hb = function() {
        if (this.Nf && !this.D) {
            var c, h;
            c = 0;
            for (h = this.d.length; c < h; c++) this.d[c].Ee = c;
            var m = c,
                r = this.b.Ac;
            c = 0;
            for (h = r.length; c < h; ++c) r[c].type === this && (r[c].Ee = m++);
            this.Nf = !1
        }
    };
    sb = function(c) {
        if (c < this.d.length) return this.d[c];
        c -= this.d.length;
        var h = this.b.Ac,
            m, r;
        m = 0;
        for (r = h.length; m < r; ++m)
            if (h[m].type === this) {
                if (0 === c) return h[m];
                --c
            }
        return null
    };
    kb = function() {
        return this.wd[this.Sc]
    };
    lb = function() {
        this.Sc++;
        this.Sc === this.wd.length ? this.wd.push(new gb(this)) : (this.wd[this.Sc].W = !0, S(this.wd[this.Sc].M))
    };
    mb = function() {
        this.Sc++;
        this.Sc === this.wd.length && this.wd.push(new gb(this));
        var c = this.wd[this.Sc],
            h = this.wd[this.Sc - 1];
        h.W ? (c.W = !0, S(c.M)) : (c.W = !1, wa(c.d, h.d), wa(c.M, h.M))
    };
    nb = function() {
        this.Sc--
    };
    ob = function(c) {
        var h, m, r, p, b, a = 0;
        if (!this.D)
            for (h = 0, m = this.la.length; h < m; h++)
                for (b = this.la[h], r = 0, p = b.ta.length; r < p; r++) {
                    if (c === b.ta[r].name) return this.u.lastBehIndex = a, b.ta[r];
                    a++
                }
        h = 0;
        for (m = this.ta.length; h < m; h++) {
            if (c === this.ta[h].name) return this.u.lastBehIndex = a, this.ta[h];
            a++
        }
        return null
    };
    pb = function(c) {
        return this.Wh(c) ?
            this.u.lastBehIndex : -1
    };
    qb = function(c) {
        var h, m;
        h = 0;
        for (m = this.G.length; h < m; h++)
            if (this.G[h].name === c) return h;
        return -1
    };
    rb = function() {
        if (this.qb && !this.D) {
            var c, h, m, r, p, b, a;
            this.fj();
            b = this.P();
            var d = b.W,
                u = (c = this.b.Qa()) && c.Fb && c.Fb.lc;
            c = 0;
            for (h = this.Eb.length; c < h; c++)
                if (p = this.Eb[c], p !== this && (p.fj(), a = p.P(), a.W = d, !d)) {
                    S(a.d);
                    m = 0;
                    for (r = b.d.length; m < r; ++m) a.d[m] = p.bk(b.d[m].Ee);
                    if (u)
                        for (S(a.M), m = 0, r = b.M.length; m < r; ++m) a.M[m] = p.bk(b.M[m].Ee)
                }
        }
    };
    tb = function() {
        return "Type" + this.L
    };
    Sb = function(c, h, m) {
        if ("undefined" ===
            typeof c || "undefined" === typeof m) return !1;
        switch (h) {
            case 0:
                return c === m;
            case 1:
                return c !== m;
            case 2:
                return c < m;
            case 3:
                return c <= m;
            case 4:
                return c > m;
            case 5:
                return c >= m;
            default:
                return !1
        }
    }
})();
var ac = {};

function dc(c) {
    this.b = c
}
(function() {
    function c() {}
    var l = dc.prototype;
    l.qa = function(c) {
        this.fa = c;
        this.b = c.b
    };
    l.qa.prototype.S = function() {};
    l.ca = function(c) {
        this.type = c;
        this.b = c.b;
        this.Le = Array(256);
        this.mh = Array(256);
        this.yd = 0
    };
    var h = l.ca.prototype;
    h.S = function() {
        var c = this;
        this.b.pb || (jQuery(document).keydown(function(h) {
            c.Hs(h)
        }), jQuery(document).keyup(function(h) {
            c.Is(h)
        }))
    };
    var m = [32, 33, 34, 35, 36, 37, 38, 39, 40, 44];
    h.Hs = function(c) {
        var h = !1;
        window != window.top && -1 < m.indexOf(c.which) && (c.preventDefault(), h = !0, c.stopPropagation());
        if (this.Le[c.which]) this.mh[c.which] && !h && c.preventDefault();
        else {
            this.Le[c.which] = !0;
            this.yd = c.which;
            this.b.jc = !0;
            this.b.trigger(dc.prototype.m.lp, this);
            var b = this.b.trigger(dc.prototype.m.tp, this),
                a = this.b.trigger(dc.prototype.m.El, this);
            this.b.jc = !1;
            if (b || a) this.mh[c.which] = !0, h || c.preventDefault()
        }
    };
    h.Is = function(c) {
        this.Le[c.which] = !1;
        this.yd = c.which;
        this.b.jc = !0;
        this.b.trigger(dc.prototype.m.Cl, this);
        var h = this.b.trigger(dc.prototype.m.Gl, this),
            b = this.b.trigger(dc.prototype.m.Fl, this);
        this.b.jc = !1;
        if (h || b || this.mh[c.which]) this.mh[c.which] = !0, c.preventDefault()
    };
    h.If = function() {
        var c;
        for (c = 0; 256 > c; ++c)
            if (this.Le[c]) {
                this.Le[c] = !1;
                this.yd = c;
                this.b.trigger(dc.prototype.m.Cl, this);
                var h = this.b.trigger(dc.prototype.m.Gl, this),
                    b = this.b.trigger(dc.prototype.m.Fl, this);
                if (h || b) this.mh[c] = !0
            }
    };
    h.rb = function() {
        return {
            triggerKey: this.yd
        }
    };
    h.Kb = function(c) {
        this.yd = c.triggerKey
    };
    c.prototype.tp = function(c) {
        return c === this.yd
    };
    c.prototype.lp = function() {
        return !0
    };
    c.prototype.Cl = function() {
        return !0
    };
    c.prototype.Gl =
        function(c) {
            return c === this.yd
        };
    c.prototype.cp = function(c) {
        c = Math.floor(c);
        return 0 > c || c >= this.Le.length ? !1 : this.Le[c]
    };
    c.prototype.El = function(c) {
        return c === this.yd
    };
    c.prototype.Fl = function(c) {
        return c === this.yd
    };
    l.m = new c;
    l.aa = new function() {};
    l.Pa = new function() {}
})();

function ec(c) {
    this.b = c
}
(function() {
    function c() {}

    function l() {}
    var h = ec.prototype;
    h.qa = function(c) {
        this.fa = c;
        this.b = c.b
    };
    h.qa.prototype.S = function() {};
    h.ca = function(c) {
        this.type = c;
        this.b = c.b;
        this.uj = Array(4);
        this.Ao = this.dj = this.Qf = this.ge = this.fe = 0;
        this.hk = !1
    };
    var m = h.ca.prototype;
    m.S = function() {
        var c = this;
        if (!this.b.pb) {
            jQuery(document).mousemove(function(a) {
                c.Ks(a)
            });
            jQuery(document).mousedown(function(a) {
                c.Js(a)
            });
            jQuery(document).mouseup(function(a) {
                c.Ls(a)
            });
            jQuery(document).dblclick(function(a) {
                c.Gs(a)
            });
            var b = function(a) {
                c.Ns(a)
            };
            document.addEventListener("mousewheel", b, !1);
            document.addEventListener("DOMMouseScroll", b, !1)
        }
    };
    var r = {
        left: 0,
        top: 0
    };
    m.Ks = function(c) {
        var b = this.b.pb ? r : jQuery(this.b.canvas).offset();
        this.fe = c.pageX - b.left;
        this.ge = c.pageY - b.top
    };
    m.Bn = function() {
        return 0 < this.b.xb ? !0 : 0 <= this.fe && 0 <= this.ge && this.fe < this.b.width && this.ge < this.b.height
    };
    m.Js = function(c) {
        this.Bn() && (this.uj[c.which] = !0, this.b.jc = !0, this.b.trigger(ec.prototype.m.kp, this), this.Qf = c.which - 1, this.dj = 0, this.b.trigger(ec.prototype.m.Dl, this),
            this.b.trigger(ec.prototype.m.nj, this), this.b.jc = !1)
    };
    m.Ls = function(c) {
        this.uj[c.which] && (this.b.Bm && !this.b.Wd && c.preventDefault(), this.b.Bm = !0, this.uj[c.which] = !1, this.b.jc = !0, this.Qf = c.which - 1, this.b.trigger(ec.prototype.m.yp, this), this.b.jc = !1)
    };
    m.Gs = function(c) {
        this.Bn() && (c.preventDefault(), this.b.jc = !0, this.Qf = c.which - 1, this.dj = 1, this.b.trigger(ec.prototype.m.Dl, this), this.b.trigger(ec.prototype.m.nj, this), this.b.jc = !1)
    };
    m.Ns = function(c) {
        this.Ao = 0 > (c.wheelDelta ? c.wheelDelta : c.detail ? -c.detail :
            0) ? 0 : 1;
        this.hk = !1;
        this.b.jc = !0;
        this.b.trigger(ec.prototype.m.Ap, this);
        this.b.jc = !1;
        this.hk && db(c) && c.preventDefault()
    };
    c.prototype.Dl = function(c, b) {
        return c === this.Qf && b === this.dj
    };
    c.prototype.kp = function() {
        return !0
    };
    c.prototype.yp = function(c) {
        return c === this.Qf
    };
    c.prototype.nj = function(c, b, a) {
        return c !== this.Qf || b !== this.dj ? !1 : this.b.At(a, this.fe, this.ge)
    };
    c.prototype.Ap = function(c) {
        this.hk = !0;
        return c === this.Ao
    };
    h.m = new c;
    h.aa = new function() {};
    l.prototype.Il = function(c, b) {
        var a, d, h, e, f;
        fa(b) ? (a =
            this.b.og(0), d = a.scale, h = a.Oc, e = a.nc, f = a.i, a.scale = 1, a.Oc = 1, a.nc = 1, a.i = 0, c.F(a.mb(this.fe, this.ge, !0)), a.scale = d, a.Oc = h, a.nc = e, a.i = f) : (a = ga(b) ? this.b.og(b) : this.b.ck(b)) ? c.F(a.mb(this.fe, this.ge, !0)) : c.F(0)
    };
    l.prototype.Jl = function(c, b) {
        var a, d, h, e, f;
        fa(b) ? (a = this.b.og(0), d = a.scale, h = a.Oc, e = a.oc, f = a.i, a.scale = 1, a.Oc = 1, a.oc = 1, a.i = 0, c.F(a.mb(this.fe, this.ge, !1)), a.scale = d, a.Oc = h, a.oc = e, a.i = f) : (a = ga(b) ? this.b.og(b) : this.b.ck(b)) ? c.F(a.mb(this.fe, this.ge, !1)) : c.F(0)
    };
    h.Pa = new l
})();

function Y(c) {
    this.b = c
}
(function() {
    function c() {
        if (0 === this.Ij.length) {
            var a = document.createElement("canvas");
            a.width = this.width;
            a.height = this.height;
            var b = a.getContext("2d");
            this.Mf ? b.drawImage(this.V, this.Ef, this.Ff, this.width, this.height, 0, 0, this.width, this.height) : b.drawImage(this.V, 0, 0, this.width, this.height);
            this.Ij = a.toDataURL("image/png")
        }
        return this.Ij
    }

    function l() {}

    function h(a) {
        a[0] = 0;
        a[1] = 0;
        a[2] = 0;
        n.push(a)
    }

    function m(a, b) {
        return a < b ? "" + a + "," + b : "" + b + "," + a
    }

    function r(a, b, d, c) {
        b = b.uid;
        d = d.uid;
        var e = m(b, d);
        if (a.hasOwnProperty(e)) a[e][2] =
            c;
        else {
            var f = n.length ? n.pop() : [0, 0, 0];
            f[0] = b;
            f[1] = d;
            f[2] = c;
            a[e] = f
        }
    }

    function p(a, b, d) {
        b = m(b.uid, d.uid);
        a.hasOwnProperty(b) && (h(a[b]), delete a[b])
    }

    function b(a, b, d) {
        b = m(b.uid, d.uid);
        if (a.hasOwnProperty(b)) return g = a[b][2], !0;
        g = -2;
        return !1
    }

    function a() {}
    var d = Y.prototype;
    d.qa = function(a) {
        this.fa = a;
        this.b = a.b
    };
    var u = d.qa.prototype;
    u.S = function() {
        if (!this.D) {
            var a, b, d, e, f, g, h, k, n;
            this.vc = [];
            this.vg = !1;
            a = 0;
            for (b = this.wb.length; a < b; a++) {
                f = this.wb[a];
                h = {};
                h.name = f[0];
                h.speed = f[1];
                h.loop = f[2];
                h.Wk = f[3];
                h.Xk =
                    f[4];
                h.On = f[5];
                h.L = f[6];
                h.frames = [];
                d = 0;
                for (e = f[7].length; d < e; d++) g = f[7][d], k = {}, k.kl = g[0], k.ll = g[1], k.Ef = g[2], k.Ff = g[3], k.width = g[4], k.height = g[5], k.duration = g[6], k.yb = g[7], k.zb = g[8], k.kk = g[9], k.Hi = g[10], k.Pn = g[11], k.Mf = 0 !== k.width, k.Ij = "", k.Lu = c, n = {
                    left: 0,
                    top: 0,
                    right: 1,
                    bottom: 1
                }, k.gl = n, k.ja = null, (n = this.b.ur(g[0])) ? k.V = n : (k.V = new Image, k.V.gq = g[0], k.V.cm = g[1], k.V.Zp = null, this.b.No(k.V, g[0])), h.frames.push(k), this.vc.push(k);
                this.wb[a] = h
            }
        }
    };
    u.Jo = function() {
        var a, b, d;
        a = 0;
        for (b = this.d.length; a < b; a++) d =
            this.d[a], d.Hh = d.hc.ja
    };
    u.Gi = function() {
        if (!this.D) {
            var a, b, d;
            a = 0;
            for (b = this.vc.length; a < b; ++a) d = this.vc[a], d.V.Zp = null, d.ja = null;
            this.vg = !1;
            this.Jo()
        }
    };
    u.Kk = function() {
        if (!this.D && this.d.length) {
            var a, b, d;
            a = 0;
            for (b = this.vc.length; a < b; ++a) d = this.vc[a], d.ja = this.b.n.ti(d.V, !1, this.b.N, d.Pn);
            this.Jo()
        }
    };
    u.Dk = function() {
        if (!this.D && !this.vg && this.b.n) {
            var a, b, d;
            a = 0;
            for (b = this.vc.length; a < b; ++a) d = this.vc[a], d.ja = this.b.n.ti(d.V, !1, this.b.N, d.Pn);
            this.vg = !0
        }
    };
    u.tl = function() {
        if (!this.D && !this.d.length &&
            this.vg) {
            var a, b, d;
            a = 0;
            for (b = this.vc.length; a < b; ++a) d = this.vc[a], this.b.n.deleteTexture(d.ja), d.ja = null;
            this.vg = !1
        }
    };
    var e = [];
    u.Pk = function(a) {
        var b, d, c;
        S(e);
        b = 0;
        for (d = this.vc.length; b < d; ++b) c = this.vc[b].V, -1 === e.indexOf(c) && (a.drawImage(c, 0, 0), e.push(c))
    };
    d.ca = function(a) {
        this.type = a;
        this.b = a.b;
        a = this.type.wb[0].frames[0].Hi;
        this.Nb ? this.O.gh(a) : this.O = new Wa(a)
    };
    var f = d.ca.prototype;
    f.S = function() {
        this.visible = 0 === this.H[0];
        this.Rc = 0 !== this.H[3];
        1 === this.type.wb.length && 1 === this.type.wb[0].frames.length ||
            0 === this.type.wb[0].speed || this.b.yo(this);
        this.ka = this.tm(this.H[1]) || this.type.wb[0];
        this.r = this.H[2];
        0 > this.r && (this.r = 0);
        this.r >= this.ka.frames.length && (this.r = this.ka.frames.length - 1);
        var a = this.ka.frames[this.r];
        this.O.gh(a.Hi);
        this.yb = a.yb;
        this.zb = a.zb;
        this.fg = this.ka.speed;
        this.Pd = this.ka.Xk;
        this.Nb ? this.Qc.reset() : this.Qc = new Ua;
        this.nd = this.Qc.Y;
        this.pe = !0;
        this.gd = 0;
        this.oe = !0;
        this.Bj = this.Nl = "";
        this.aq = 0;
        this.Aj = -1;
        this.type.Dk();
        var b, d, c, e, f, g, h, a = 0;
        for (b = this.type.wb.length; a < b; a++)
            for (e =
                this.type.wb[a], d = 0, c = e.frames.length; d < c; d++) f = e.frames[d], 0 === f.width && (f.width = f.V.width, f.height = f.V.height), f.Mf && (h = f.V, g = f.gl, g.left = f.Ef / h.width, g.top = f.Ff / h.height, g.right = (f.Ef + f.width) / h.width, g.bottom = (f.Ff + f.height) / h.height, 0 === f.Ef && 0 === f.Ff && f.width === h.width && f.height === h.height && (f.Mf = !1));
        this.hc = this.ka.frames[this.r];
        this.Hh = this.hc.ja
    };
    f.rb = function() {
        var a = {
            a: this.ka.L,
            f: this.r,
            cas: this.fg,
            fs: this.nd,
            ar: this.gd,
            at: this.Qc.Y,
            rt: this.Pd
        };
        this.pe || (a.ap = this.pe);
        this.oe || (a.af =
            this.oe);
        return a
    };
    f.Kb = function(a) {
        var b = this.Br(a.a);
        b && (this.ka = b);
        this.r = a.f;
        0 > this.r && (this.r = 0);
        this.r >= this.ka.frames.length && (this.r = this.ka.frames.length - 1);
        this.fg = a.cas;
        this.nd = a.fs;
        this.gd = a.ar;
        this.Qc.reset();
        this.Qc.Y = a.at;
        this.pe = a.hasOwnProperty("ap") ? a.ap : !0;
        this.oe = a.hasOwnProperty("af") ? a.af : !0;
        a.hasOwnProperty("rt") ? this.Pd = a.rt : this.Pd = this.ka.Xk;
        this.hc = this.ka.frames[this.r];
        this.Hh = this.hc.ja;
        this.O.gh(this.hc.Hi);
        this.yb = this.hc.yb;
        this.zb = this.hc.zb
    };
    f.rj = function(a) {
        this.r =
            a ? 0 : this.ka.frames.length - 1;
        this.pe = !1;
        this.Nl = this.ka.name;
        this.b.trigger(Y.prototype.m.jp, this);
        this.b.trigger(Y.prototype.m.ip, this);
        this.gd = 0
    };
    f.wh = function() {
        return this.Qc.Y
    };
    f.cc = function() {
        this.Qc.add(this.b.mg(this));
        this.Bj.length && this.mq();
        0 <= this.Aj && this.nq();
        var a = this.Qc.Y,
            b = this.ka,
            d = b.frames[this.r],
            c = d.duration / this.fg;
        this.pe && a >= this.nd + c && (this.oe ? this.r++ : this.r--, this.nd += c, this.r >= b.frames.length && (b.On ? (this.oe = !1, this.r = b.frames.length - 2) : b.loop ? this.r = this.Pd : (this.gd++,
            this.gd >= b.Wk ? this.rj(!1) : this.r = this.Pd)), 0 > this.r && (b.On ? (this.r = 1, this.oe = !0, b.loop || (this.gd++, this.gd >= b.Wk && this.rj(!0))) : b.loop ? this.r = this.Pd : (this.gd++, this.gd >= b.Wk ? this.rj(!0) : this.r = this.Pd)), 0 > this.r ? this.r = 0 : this.r >= b.frames.length && (this.r = b.frames.length - 1), a > this.nd + b.frames[this.r].duration / this.fg && (this.nd = a), a = b.frames[this.r], this.mf(d, a), this.b.T = !0)
    };
    f.tm = function(a) {
        var b, d, c;
        b = 0;
        for (d = this.type.wb.length; b < d; b++)
            if (c = this.type.wb[b], cb(c.name, a)) return c;
        return null
    };
    f.Br =
        function(a) {
            var b, d, c;
            b = 0;
            for (d = this.type.wb.length; b < d; b++)
                if (c = this.type.wb[b], c.L === a) return c;
            return null
        };
    f.mq = function() {
        var a = this.ka.frames[this.r],
            b = this.tm(this.Bj);
        this.Bj = "";
        !b || cb(b.name, this.ka.name) && this.pe || (this.ka = b, this.fg = b.speed, this.Pd = b.Xk, 0 > this.r && (this.r = 0), this.r >= this.ka.frames.length && (this.r = this.ka.frames.length - 1), 1 === this.aq && (this.r = 0), this.pe = !0, this.nd = this.Qc.Y, this.oe = !0, this.mf(a, this.ka.frames[this.r]), this.b.T = !0)
    };
    f.nq = function() {
        var a = this.ka.frames[this.r],
            b = this.r;
        this.r = N(this.Aj);
        0 > this.r && (this.r = 0);
        this.r >= this.ka.frames.length && (this.r = this.ka.frames.length - 1);
        b !== this.r && (this.mf(a, this.ka.frames[this.r]), this.nd = this.Qc.Y, this.b.T = !0);
        this.Aj = -1
    };
    f.mf = function(a, b) {
        var d = a.width,
            c = a.height,
            e = b.width,
            f = b.height;
        d != e && (this.width *= e / d);
        c != f && (this.height *= f / c);
        this.yb = b.yb;
        this.zb = b.zb;
        this.O.gh(b.Hi);
        this.Q();
        this.hc = b;
        this.Hh = b.ja;
        d = 0;
        for (c = this.C.length; d < c; d++) e = this.C[d], e.Ms && e.Ms(a, b);
        this.b.trigger(Y.prototype.m.mf, this)
    };
    f.Tc = function(a) {
        a.globalAlpha =
            this.opacity;
        var b = this.hc,
            d = b.Mf,
            c = b.V,
            e = this.x,
            f = this.y,
            g = this.width,
            h = this.height;
        if (0 === this.i && 0 <= g && 0 <= h) e -= this.yb * g, f -= this.zb * h, this.b.Gc && (e = Math.round(e), f = Math.round(f)), d ? a.drawImage(c, b.Ef, b.Ff, b.width, b.height, e, f, g, h) : a.drawImage(c, e, f, g, h);
        else {
            this.b.Gc && (e = Math.round(e), f = Math.round(f));
            a.save();
            var k = 0 < g ? 1 : -1,
                n = 0 < h ? 1 : -1;
            a.translate(e, f);
            1 === k && 1 === n || a.scale(k, n);
            a.rotate(this.i * k * n);
            e = 0 - this.yb * ja(g);
            f = 0 - this.zb * ja(h);
            d ? a.drawImage(c, b.Ef, b.Ff, b.width, b.height, e, f, ja(g), ja(h)) :
                a.drawImage(c, e, f, ja(g), ja(h));
            a.restore()
        }
    };
    f.xe = function(a) {
        this.Gb(a)
    };
    f.Gb = function(a) {
        a.Ob(this.Hh);
        a.Ye(this.opacity);
        var b = this.hc,
            d = this.bb;
        if (this.b.Gc) {
            var c = Math.round(this.x) - this.x,
                e = Math.round(this.y) - this.y;
            b.Mf ? a.$c(d.Ma + c, d.Na + e, d.kb + c, d.lb + e, d.eb + c, d.fb + e, d.$a + c, d.ab + e, b.gl) : a.ah(d.Ma + c, d.Na + e, d.kb + c, d.lb + e, d.eb + c, d.fb + e, d.$a + c, d.ab + e)
        } else b.Mf ? a.$c(d.Ma, d.Na, d.kb, d.lb, d.eb, d.fb, d.$a, d.ab, b.gl) : a.ah(d.Ma, d.Na, d.kb, d.lb, d.eb, d.fb, d.$a, d.ab)
    };
    f.Hr = function(a) {
        var b = this.hc,
            d, c;
        d =
            0;
        for (c = b.kk.length; d < c; d++)
            if (cb(a, b.kk[d][0])) return d;
        return -1
    };
    f.ng = function(a, b) {
        var d = this.hc,
            c = d.kk,
            e;
        J(a) ? e = this.Hr(a) : e = a - 1;
        e = N(e);
        if (0 > e || e >= c.length) return b ? this.x : this.y;
        var f = (c[e][1] - d.yb) * this.width,
            c = c[e][2],
            c = (c - d.zb) * this.height,
            d = Math.cos(this.i);
        e = Math.sin(this.i);
        var g = f * d - c * e,
            c = c * d + f * e,
            f = g + this.x,
            c = c + this.y;
        return b ? f : c
    };
    var n = [],
        g = -2,
        y = [];
    l.prototype.op = function(a) {
        if (!a) return !1;
        var d = this.b,
            c = d.Yh(),
            e = c.type,
            f = null;
        c.u.collmemory ? f = c.u.collmemory : (f = {}, c.u.collmemory = f);
        c.u.spriteCreatedDestroyCallback || (c.u.spriteCreatedDestroyCallback = !0, d.Qp(function(a) {
            var b = c.u.collmemory;
            a = a.uid;
            var d, e;
            for (d in b) b.hasOwnProperty(d) && (e = b[d], e[0] === a || e[1] === a) && (h(b[d]), delete b[d])
        }));
        var k = e.P(),
            n = a.P(),
            k = k.Xb(),
            m, l, u, q, v, F, I, D = this.b.ed,
            B = D - 1,
            P = d.Qa().Fb;
        for (l = 0; l < k.length; l++) {
            u = k[l];
            n.W ? (u.Z(), this.b.Xh(u.j, a, u.da, y), m = y) : m = n.Xb();
            for (q = 0; q < m.length; q++) v = m[q], d.me(u, v) || d.bq(u, v) ? (F = b(f, u, v), F = !F || g < B, r(f, u, v, D), F && (d.Ki(P.na), F = e.P(), I = a.P(), F.W = !1, I.W = !1, e === a ? (F.d.length =
                2, F.d[0] = u, F.d[1] = v, e.wc()) : (F.d.length = 1, I.d.length = 1, F.d[0] = u, I.d[0] = v, e.wc(), a.wc()), P.jt(), d.We(P.na))) : p(f, u, v);
            S(y)
        }
        return !1
    };
    var F = null,
        I = new ca,
        k = !1,
        q = [],
        D = new Q(0, 0, 0, 0);
    u.finish = function(a) {
        if (k) {
            if (a) {
                var b = this.b.Qa().Fb.lc;
                a = F.P();
                var d = I.zd(),
                    c, e;
                if (a.W) {
                    a.W = !1;
                    S(a.d);
                    c = 0;
                    for (e = d.length; c < e; ++c) a.d[c] = d[c];
                    if (b)
                        for (S(a.M), c = 0, e = F.d.length; c < e; ++c) d = F.d[c], I.contains(d) || a.M.push(d)
                } else if (b)
                    for (b = a.d.length, c = 0, e = d.length; c < e; ++c) a.d[b + c] = d[c], Aa(a.M, d[c]);
                else wa(a.d, d);
                F.wc()
            }
            I.clear();
            k = !1
        }
    };
    l.prototype.dp = function(a) {
        if (a) {
            var b = !1,
                d, c, e, f = this.b.Yh(),
                g = f.type,
                f = f.ci;
            d = a.P();
            c = this.b.Qa().Fb.lc;
            var h;
            d.W ? (this.Z(), D.nf(this.da), D.offset(0, 0), this.b.Xh(this.j, a, D, q), h = q) : h = c ? this.b.ns() && !d.M.length && d.d.length ? d.d : d.M : d.d;
            F = a;
            k = g !== a && !f;
            d = 0;
            for (c = h.length; d < c; d++)
                if (e = h[d], this.b.me(this, e)) {
                    b = !0;
                    if (f) break;
                    g !== a && I.add(e)
                }
            S(q);
            a = b
        } else a = !1;
        return a
    };
    l.prototype.ip = function(a) {
        return cb(this.Nl, a)
    };
    l.prototype.jp = function() {
        return !0
    };
    l.prototype.mf = function() {
        return !0
    };
    d.m =
        new l;
    a.prototype.Mp = function(a, b, d) {
        if (a && b && (b = this.b.Fj(a, b, this.ng(d, !0), this.ng(d, !1)))) {
            "undefined" !== typeof b.i && (b.i = this.i, b.Q());
            this.b.Zb++;
            var c, e, f;
            this.b.trigger(Object.getPrototypeOf(a.fa).m.lf, b);
            if (b.qb)
                for (c = 0, e = b.siblings.length; c < e; c++) f = b.siblings[c], this.b.trigger(Object.getPrototypeOf(f.type.fa).m.lf, f);
            this.b.Zb--;
            c = this.b.Er();
            d = !1;
            if (fa(c.u.Spawn_LastExec) || c.u.Spawn_LastExec < this.b.Be) d = !0, c.u.Spawn_LastExec = this.b.Be;
            if (a != this.type && (a = a.P(), a.W = !1, d ? (S(a.d), a.d[0] = b) :
                    a.d.push(b), b.qb))
                for (c = 0, e = b.siblings.length; c < e; c++) f = b.siblings[c], a = f.type.P(), a.W = !1, d ? (S(a.d), a.d[0] = f) : a.d.push(f)
        }
    };
    d.aa = new a;
    d.Pa = new function() {}
})();

function fc(c) {
    this.b = c
}
(function() {
    function c() {
        return a.length ? a.pop() : {}
    }

    function l(b) {
        var c, e;
        c = 0;
        for (e = b.length; c < e; c++) a.push(b[c]);
        S(b)
    }

    function h(a) {
        return a.length && " " === a.charAt(a.length - 1) ? a.substring(0, a.length - 1) : a
    }

    function m() {}
    var r = fc.prototype;
    r.S = function() {
        r.aa.Jp = function(a) {
            this.width !== a && (this.width = a, this.ff = !0, this.Q())
        }
    };
    r.qa = function(a) {
        this.fa = a;
        this.b = a.b
    };
    var p = r.qa.prototype;
    p.S = function() {};
    p.Gi = function() {
        if (!this.D) {
            var a, b, c;
            a = 0;
            for (b = this.d.length; a < b; a++) c = this.d[a], c.Yc = null, c.Ue = null,
                c.Lb = null
        }
    };
    r.ca = function(a) {
        this.type = a;
        this.b = a.b;
        this.Nb ? S(this.$d) : this.$d = [];
        this.ff = !0
    };
    p = r.ca.prototype;
    p.S = function() {
        this.text = this.H[0];
        this.visible = 0 === this.H[1];
        this.font = this.H[2];
        this.color = this.H[3];
        this.ai = this.H[4];
        this.hj = this.H[5];
        this.zl = 0 === this.H[7];
        this.Ak = this.mi = this.width;
        this.li = this.height;
        this.Ck = this.H[8];
        this.Sh = this.qf = "";
        this.vo = this.wo = this.Zg = 0;
        this.Os();
        this.Lb = this.Ue = this.Yc = null;
        this.Dn = !1;
        this.Jg = this.b.ed;
        this.Nb ? this.je.set(0, 0, 1, 1) : this.je = new Q(0, 0, 1, 1);
        this.b.n && this.b.yo(this)
    };
    p.Os = function() {
        var a = this.font.split(" "),
            b;
        for (b = 0; b < a.length; b++)
            if ("pt" === a[b].substr(a[b].length - 2, 2)) {
                this.Zg = parseInt(a[b].substr(0, a[b].length - 2));
                this.Sk = Math.ceil(this.Zg / 72 * 96) + 4;
                0 < b && (this.Sh = a[b - 1]);
                this.qf = a[b + 1];
                for (b += 2; b < a.length; b++) this.qf += " " + a[b];
                break
            }
    };
    p.rb = function() {
        return {
            t: this.text,
            f: this.font,
            c: this.color,
            ha: this.ai,
            va: this.hj,
            wr: this.zl,
            lho: this.Ck,
            fn: this.qf,
            fs: this.Sh,
            ps: this.Zg,
            pxh: this.Sk,
            tw: this.wo,
            th: this.vo,
            lrt: this.Jg
        }
    };
    p.Kb = function(a) {
        this.text =
            a.t;
        this.font = a.f;
        this.color = a.c;
        this.ai = a.ha;
        this.hj = a.va;
        this.zl = a.wr;
        this.Ck = a.lho;
        this.qf = a.fn;
        this.Sh = a.fs;
        this.Zg = a.ps;
        this.Sk = a.pxh;
        this.wo = a.tw;
        this.vo = a.th;
        this.Jg = a.lrt;
        this.ff = !0;
        this.Ak = this.mi = this.width;
        this.li = this.height
    };
    p.cc = function() {
        if (this.b.n && this.Lb && 300 <= this.b.ed - this.Jg) {
            var a = this.j;
            this.Z();
            var b = this.da;
            if (b.right < a.ga || b.bottom < a.ia || b.left > a.pa || b.top > a.oa) this.b.n.deleteTexture(this.Lb), this.Yc = this.Ue = this.Lb = null
        }
    };
    p.Hf = function() {
        this.Yc = this.Ue = null;
        this.b.n &&
            this.Lb && this.b.n.deleteTexture(this.Lb);
        this.Lb = null
    };
    p.wh = function() {
        this.font = this.Sh + " " + this.Zg.toString() + "pt " + this.qf;
        this.ff = !0;
        this.b.T = !0
    };
    p.Tc = function(a, b) {
        a.font = this.font;
        a.textBaseline = "top";
        a.fillStyle = this.color;
        a.globalAlpha = b ? 1 : this.opacity;
        var c = 1;
        b && (c = this.j.Ib(), a.save(), a.scale(c, c));
        if (this.ff || this.width !== this.Ak) this.type.fa.Op(this.text, this.$d, a, this.width, this.zl), this.ff = !1, this.Ak = this.width;
        this.Z();
        var c = b ? 0 : this.bb.Ma,
            f = b ? 0 : this.bb.Na;
        this.b.Gc && (c = c + .5 | 0, f = f +
            .5 | 0);
        0 === this.i || b || (a.save(), a.translate(c, f), a.rotate(this.i), f = c = 0);
        var h = f + this.height,
            g = this.Sk,
            g = g + this.Ck,
            m, l;
        1 === this.hj ? f += Math.max(this.height / 2 - this.$d.length * g / 2, 0) : 2 === this.hj && (f += Math.max(this.height - this.$d.length * g - 2, 0));
        for (l = 0; l < this.$d.length && !(m = c, 1 === this.ai ? m = c + (this.width - this.$d[l].width) / 2 : 2 === this.ai && (m = c + (this.width - this.$d[l].width)), a.fillText(this.$d[l].text, m, f), f += g, f >= h - g); l++);
        (0 !== this.i || b) && a.restore();
        this.Jg = this.b.ed
    };
    p.Gb = function(a) {
        if (!(1 > this.width ||
                1 > this.height)) {
            var b = this.ff || this.Dn;
            this.Dn = !1;
            var c = this.j.Ib(),
                f = this.j.Hb(),
                h = this.je,
                g = c * this.width,
                m = c * this.height,
                l = Math.ceil(g),
                p = Math.ceil(m),
                k = this.b.B / 2,
                q = this.b.A / 2;
            this.Ue || (this.Yc = document.createElement("canvas"), this.Yc.width = l, this.Yc.height = p, this.mi = l, this.li = p, b = !0, this.Ue = this.Yc.getContext("2d"));
            if (l !== this.mi || p !== this.li) this.Yc.width = l, this.Yc.height = p, this.Lb && (a.deleteTexture(this.Lb), this.Lb = null), b = !0;
            b && (this.Ue.clearRect(0, 0, l, p), this.Tc(this.Ue, !0), this.Lb || (this.Lb =
                a.Vb(l, p, this.b.N, this.b.Wd)), a.Jt(this.Yc, this.Lb, this.b.Wd));
            this.mi = l;
            this.li = p;
            a.Ob(this.Lb);
            a.Ye(this.opacity);
            a.pc();
            a.translate(-k, -q);
            a.ec();
            var r = this.bb,
                b = this.j.ma(r.Ma, r.Na, !0, !0),
                k = this.j.ma(r.Ma, r.Na, !1, !0),
                q = this.j.ma(r.kb, r.lb, !0, !0),
                z = this.j.ma(r.kb, r.lb, !1, !0),
                t = this.j.ma(r.eb, r.fb, !0, !0),
                L = this.j.ma(r.eb, r.fb, !1, !0),
                V = this.j.ma(r.$a, r.ab, !0, !0),
                r = this.j.ma(r.$a, r.ab, !1, !0);
            if (this.b.Gc || 0 === this.i && 0 === f) var Z = (b + .5 | 0) - b,
                E = (k + .5 | 0) - k,
                b = b + Z,
                k = k + E,
                q = q + Z,
                z = z + E,
                t = t + Z,
                L = L + E,
                V = V + Z,
                r =
                r + E;
            0 === this.i && 0 === f ? (q = b + l, z = k, t = q, L = k + p, V = b, r = L, h.right = 1, h.bottom = 1) : (h.right = g / l, h.bottom = m / p);
            a.$c(b, k, q, z, t, L, V, r, h);
            a.pc();
            a.scale(c, c);
            a.Qi(-this.j.Hb());
            a.translate((this.j.ga + this.j.pa) / -2, (this.j.ia + this.j.oa) / -2);
            a.ec();
            this.Jg = this.b.ed
        }
    };
    var b = [];
    r.Np = function(a) {
        S(b);
        for (var c = "", e, f = 0; f < a.length;)
            if (e = a.charAt(f), "\n" === e) c.length && (b.push(c), c = ""), b.push("\n"), ++f;
            else if (" " === e || "\t" === e || "-" === e) {
            do c += a.charAt(f), f++; while (f < a.length && (" " === a.charAt(f) || "\t" === a.charAt(f)));
            b.push(c);
            c = ""
        } else f < a.length && (c += e, f++);
        c.length && b.push(c)
    };
    var a = [];
    r.Op = function(a, b, e, f, h) {
        if (a && a.length)
            if (2 >= f) l(b);
            else {
                if (100 >= a.length && -1 === a.indexOf("\n")) {
                    var g = e.measureText(a).width;
                    if (g <= f) {
                        l(b);
                        b.push(c());
                        b[0].text = a;
                        b[0].width = g;
                        return
                    }
                }
                this.Pp(a, b, e, f, h)
            }
        else l(b)
    };
    r.Pp = function(d, m, e, f, n) {
        n && (this.Np(d), d = b);
        var g = "",
            l, r, p, k = 0;
        for (p = 0; p < d.length; p++) "\n" === d[p] ? (k >= m.length && m.push(c()), g = h(g), r = m[k], r.text = g, r.width = e.measureText(g).width, k++, g = "") : (l = g, g += d[p], r = e.measureText(g).width,
            r >= f && (k >= m.length && m.push(c()), l = h(l), r = m[k], r.text = l, r.width = e.measureText(l).width, k++, g = d[p], n || " " !== g || (g = "")));
        g.length && (k >= m.length && m.push(c()), g = h(g), r = m[k], r.text = g, r.width = e.measureText(g).width, k++);
        for (p = k; p < m.length; p++) a.push(m[p]);
        m.length = k
    };
    r.m = new function() {};
    m.prototype.Fp = function(a) {
        ga(a) && 1E9 > a && (a = Math.round(1E10 * a) / 1E10);
        a = a.toString();
        this.text !== a && (this.text = a, this.ff = !0, this.b.T = !0)
    };
    r.aa = new m;
    r.Pa = new function() {}
})();

function gc(c) {
    this.b = c
}
(function() {
    var c = gc.prototype;
    c.qa = function(c) {
        this.fa = c;
        this.b = c.b
    };
    var l = c.qa.prototype;
    l.S = function() {
        this.D || (this.V = new Image, this.V.cm = this.ll, this.b.No(this.V, this.kl), this.ja = this.pattern = null)
    };
    l.Gi = function() {
        this.D || (this.ja = null)
    };
    l.Kk = function() {
        if (!this.D && this.d.length) {
            this.ja || (this.ja = this.b.n.ti(this.V, !0, this.b.N, this.ml));
            var c, m;
            c = 0;
            for (m = this.d.length; c < m; c++) this.d[c].ja = this.ja
        }
    };
    l.Dk = function() {
        this.D || this.ja || !this.b.n || (this.ja = this.b.n.ti(this.V, !0, this.b.N, this.ml))
    };
    l.tl = function() {
        this.D || this.d.length || !this.ja || (this.b.n.deleteTexture(this.ja), this.ja = null)
    };
    l.Pk = function(c) {
        c.drawImage(this.V, 0, 0)
    };
    c.ca = function(c) {
        this.type = c;
        this.b = c.b
    };
    l = c.ca.prototype;
    l.S = function() {
        this.visible = 0 === this.H[0];
        this.je = new Q(0, 0, 0, 0);
        this.Em = !1;
        this.V = this.type.V;
        this.b.n ? (this.type.Dk(), this.ja = this.type.ja) : (this.type.pattern || (this.type.pattern = this.b.X.createPattern(this.type.V, "repeat")), this.pattern = this.type.pattern)
    };
    l.fd = function() {
        this.Em = !1;
        this.V = this.type.V
    };
    l.Hf = function() {
        this.b.n && this.Em && this.ja && (this.b.n.deleteTexture(this.ja), this.ja = null)
    };
    l.Tc = function(c) {
        c.globalAlpha = this.opacity;
        c.save();
        c.fillStyle = this.pattern;
        var m = this.x,
            l = this.y;
        this.b.Gc && (m = Math.round(m), l = Math.round(l));
        var p = -(this.yb * this.width),
            b = -(this.zb * this.height),
            a = p % this.V.width,
            d = b % this.V.height;
        0 > a && (a += this.V.width);
        0 > d && (d += this.V.height);
        c.translate(m, l);
        c.rotate(this.i);
        c.translate(a, d);
        c.fillRect(p - a, b - d, this.width, this.height);
        c.restore()
    };
    l.xe = function(c) {
        this.Gb(c)
    };
    l.Gb = function(c) {
        c.Ob(this.ja);
        c.Ye(this.opacity);
        var m = this.je;
        m.right = this.width / this.V.width;
        m.bottom = this.height / this.V.height;
        var l = this.bb;
        if (this.b.Gc) {
            var p = Math.round(this.x) - this.x,
                b = Math.round(this.y) - this.y;
            c.$c(l.Ma + p, l.Na + b, l.kb + p, l.lb + b, l.eb + p, l.fb + b, l.$a + p, l.ab + b, m)
        } else c.$c(l.Ma, l.Na, l.kb, l.lb, l.eb, l.fb, l.$a, l.ab, m)
    };
    c.m = new function() {};
    c.aa = new function() {};
    c.Pa = new function() {}
})();

function hc(c) {
    this.b = c
}
(function() {
    var c = hc.prototype;
    c.qa = function(c) {
        this.behavior = c;
        this.b = c.b
    };
    c.qa.prototype.S = function() {};
    c.ca = function(c, l) {
        this.type = c;
        this.behavior = c.behavior;
        this.k = l;
        this.b = c.b
    };
    var l = c.ca.prototype;
    l.S = function() {
        var c = this.H[0];
        this.Ed = this.H[1];
        this.Vh = this.H[2];
        this.Xp = 0 !== this.H[3];
        this.dl = 0 !== this.H[4];
        this.ob = Math.cos(this.k.i) * c;
        this.Ua = Math.sin(this.k.i) * c;
        this.ni = this.k.x;
        this.oi = this.k.y;
        this.Ec = this.k.i;
        this.cj = 0;
        this.enabled = 0 !== this.H[5]
    };
    l.rb = function() {
        return {
            acc: this.Ed,
            g: this.Vh,
            dx: this.ob,
            dy: this.Ua,
            lx: this.ni,
            ly: this.oi,
            lka: this.Ec,
            t: this.cj,
            e: this.enabled
        }
    };
    l.Kb = function(c) {
        this.Ed = c.acc;
        this.Vh = c.g;
        this.ob = c.dx;
        this.Ua = c.dy;
        this.ni = c.lx;
        this.oi = c.ly;
        this.Ec = c.lka;
        this.cj = c.t;
        this.enabled = c.e
    };
    l.cc = function() {
        if (this.enabled) {
            var c = this.b.mg(this.k),
                l, r;
            this.k.i !== this.Ec && (this.dl && (l = Ma(0, 0, this.ob, this.Ua), this.ob = Math.cos(this.k.i) * l, this.Ua = Math.sin(this.k.i) * l), this.Ec = this.k.i);
            0 !== this.Ed && (l = Ma(0, 0, this.ob, this.Ua), r = 0 === this.ob && 0 === this.Ua ? this.k.i : Ia(0, 0, this.ob,
                this.Ua), l += this.Ed * c, 0 > l && (l = 0), this.ob = Math.cos(r) * l, this.Ua = Math.sin(r) * l);
            0 !== this.Vh && (this.Ua += this.Vh * c);
            this.ni = this.k.x;
            this.oi = this.k.y;
            if (0 !== this.ob || 0 !== this.Ua)
                if (this.k.x += this.ob * c, this.k.y += this.Ua * c, this.cj += Ma(0, 0, this.ob * c, this.Ua * c), this.dl && (this.k.i = Ia(0, 0, this.ob, this.Ua), this.k.Q(), this.Ec = this.k.i), this.k.Q(), this.Xp && (l = this.b.ef(this.k))) this.b.ft(this.k, l), l = Ma(0, 0, this.ob, this.Ua), r = this.b.$p(this.k, this.ni, this.oi), this.ob = Math.cos(r) * l, this.Ua = Math.sin(r) * l, this.k.x +=
                    this.ob * c, this.k.y += this.Ua * c, this.k.Q(), this.dl && (this.Ec = this.k.i = r, this.k.Q()), this.b.ct(this.k, this.ob / l, this.Ua / l, Math.max(2.5 * l * c, 30)) || this.b.dt(this.k)
        }
    };
    c.m = new function() {};
    c.aa = new function() {};
    c.Pa = new function() {}
})();

function ic(c) {
    this.b = c
}
(function() {
    function c() {}
    var l = ic.prototype;
    l.qa = function(c) {
        this.behavior = c;
        this.b = c.b
    };
    l.qa.prototype.S = function() {};
    l.ca = function(c, h) {
        this.type = c;
        this.behavior = c.behavior;
        this.k = h;
        this.b = c.b
    };
    var h = l.ca.prototype;
    h.S = function() {
        this.Kl = 1 === this.H[0];
        this.kg = this.H[1];
        this.qh = this.H[2];
        this.Nh = this.H[3];
        this.kq = this.H[4];
        this.ac = this.Kl ? 0 : 3;
        this.Nb ? this.rc.reset() : this.rc = new Ua;
        this.Se = this.k.opacity ? this.k.opacity : 1;
        this.Kl && (0 === this.kg ? (this.ac = 1, 0 === this.qh && (this.ac = 2)) : (this.k.opacity =
            0, this.b.T = !0))
    };
    h.rb = function() {
        return {
            fit: this.kg,
            wt: this.qh,
            fot: this.Nh,
            s: this.ac,
            st: this.rc.Y,
            mo: this.Se
        }
    };
    h.Kb = function(c) {
        this.kg = c.fit;
        this.qh = c.wt;
        this.Nh = c.fot;
        this.ac = c.s;
        this.rc.reset();
        this.rc.Y = c.st;
        this.Se = c.mo
    };
    h.cc = function() {
        this.rc.add(this.b.mg(this.k));
        0 === this.ac && (this.k.opacity = this.rc.Y / this.kg * this.Se, this.b.T = !0, this.k.opacity >= this.Se && (this.k.opacity = this.Se, this.ac = 1, this.rc.reset(), this.b.trigger(ic.prototype.m.qp, this.k)));
        1 === this.ac && this.rc.Y >= this.qh && (this.ac = 2,
            this.rc.reset(), this.b.trigger(ic.prototype.m.zp, this.k));
        2 === this.ac && 0 !== this.Nh && (this.k.opacity = this.Se - this.rc.Y / this.Nh * this.Se, this.b.T = !0, 0 > this.k.opacity && (this.k.opacity = 0, this.ac = 3, this.rc.reset(), this.b.trigger(ic.prototype.m.rp, this.k), 1 === this.kq && this.b.Cd(this.k)))
    };
    h.wh = function() {
        this.ac = 0;
        this.rc.reset();
        0 === this.kg ? (this.ac = 1, 0 === this.qh && (this.ac = 2)) : (this.k.opacity = 0, this.b.T = !0)
    };
    c.prototype.rp = function() {
        return !0
    };
    c.prototype.qp = function() {
        return !0
    };
    c.prototype.zp = function() {
        return !0
    };
    l.m = new c;
    l.aa = new function() {};
    l.Pa = new function() {}
})();

function jc(c) {
    this.b = c
}
(function() {
    function c() {}
    var l = {},
        h = jc.prototype;
    h.Gf = function() {
        var b, a;
        for (b in l) l.hasOwnProperty(b) && (a = l[b], a.he.unsetReady(), a.dh = !0)
    };
    h.qa = function(b) {
        this.behavior = b;
        this.b = b.b
    };
    h.qa.prototype.S = function() {
        this.Ug = [];
        this.Nd = []
    };
    h.ca = function(b, a) {
        this.type = b;
        this.behavior = b.behavior;
        this.k = a;
        this.b = b.b
    };
    var m = h.ca.prototype;
    m.S = function() {
        this.Ba = this.H[0];
        3 > this.Ba && (this.Ba = 3);
        this.zc = this.H[1];
        this.Cs = this.H[2];
        this.Xc = this.H[3];
        this.Ed = this.H[4];
        this.Jj = this.H[5];
        this.vh = T(this.H[6]);
        this.Oi = 0 !== this.H[7];
        this.Lh = 0 !== this.H[8];
        this.enabled = 0 !== this.H[9];
        this.ug = this.Rh = this.Ci = this.Ag = !1;
        this.Ab = 0;
        this.Ec = this.Pc = this.k.i;
        this.bh = this.bd = this.ad = this.Aa = 0;
        this.Te = Math.ceil(this.b.R.width / this.Ba);
        this.Df = Math.ceil(this.b.R.height / this.Ba);
        this.wa = [];
        this.gg = !1;
        this.Mj = this.Lj = 0;
        this.ei = this.ii = !1;
        this.Xl = this.Wl = 0;
        this.om = !0;
        var b = this;
        this.Nb || (this.Qs = function() {
            b.ii || (b.ei = !1, b.fq(), b.ug = 0 < b.wa.length, b.Ab = 0, b.b.trigger(jc.prototype.m.xp, b.k), b.Oj())
        }, this.Ps = function() {
            b.ii ||
                (b.ei = !1, b.Zl(), b.ug = !1, b.Ag = !1, b.Ab = 0, b.b.trigger(jc.prototype.m.sp, b.k), b.Oj())
        })
    };
    m.Hf = function() {
        this.ii = !0;
        this.gg = !1
    };
    m.rb = function() {
        var b = {
            cs: this.Ba,
            cb: this.zc,
            ms: this.Xc,
            acc: this.Ed,
            dec: this.Jj,
            av: this.vh,
            re: this.Oi,
            de: this.Lh,
            im: this.Ag,
            mfs: this.Ci,
            ftmwm: this.Rh,
            hp: this.ug,
            mn: this.Ab,
            a: this.Pc,
            lka: this.Ec,
            s: this.Aa,
            rx: this.ad,
            ry: this.bd,
            ra: this.bh,
            myhc: this.Te,
            myvc: this.Df,
            path: this.wa,
            en: this.enabled,
            fr: this.om,
            obs: [],
            costs: []
        };
        this.ei ? (b.dfp = !0, b.dpx = this.Wl, b.dpy = this.Xl) : (b.dfp = this.gg,
            b.dpx = this.Lj, b.dpy = this.Mj);
        var a, c;
        a = 0;
        for (c = this.type.Ug.length; a < c; a++) b.obs.push(this.type.Ug[a].L);
        a = 0;
        for (c = this.type.Nd.length; a < c; a++) b.costs.push({
            sid: this.type.Nd[a].Gn.L,
            cost: this.type.Nd[a].am
        });
        return b
    };
    m.Kb = function(b) {
        this.Ba = b.cs;
        this.zc = b.cb;
        this.Xc = b.ms;
        this.Ed = b.acc;
        this.Jj = b.dec;
        this.vh = b.av;
        this.Oi = b.re;
        this.Lh = b.de;
        this.Ag = b.im;
        this.Ci = b.mfs;
        this.Rh = b.ftmwm;
        this.ug = b.hp;
        this.Ab = b.mn;
        this.Pc = b.a;
        this.Ec = b.lka;
        this.Aa = b.s;
        this.ad = b.rx;
        this.bd = b.ry;
        this.bh = b.ra;
        this.Te = b.myhc;
        this.Df = b.myvc;
        this.wa = b.path;
        this.enabled = b.en;
        this.om = b.fr;
        this.gg = b.dfp;
        this.Lj = b.dpx;
        this.Mj = b.dpy;
        S(this.type.Ug);
        var a = b.obs,
            c, h, e;
        c = 0;
        for (h = a.length; c < h; c++)(e = this.b.uf(a[c])) && this.type.Ug.push(e);
        S(this.type.Nd);
        b = b.costs;
        c = 0;
        for (h = b.length; c < h; c++)(e = this.b.uf(b[c].sid)) && this.type.Nd.push({
            Gn: e,
            am: b[c].cost
        });
        this.tf().he.setDiagonals(this.Lh)
    };
    m.fd = function() {
        this.tf().dh = !0
    };
    m.cc = function() {
        if (this.enabled && this.Ag) {
            this.Oi && this.k.i !== this.Ec && (this.Pc = this.k.i);
            var b = this.b.mg(this.k),
                a, c, h, e, f, l = this.k,
                g = Math.min(.4 * this.Xc, 2 * Math.abs(this.k.width)),
                m = Math.max(1.5 * this.Aa, 30);
            this.Ab < this.wa.length ? (e = this.wa[this.Ab].x, f = this.wa[this.Ab].y, h = Ma(this.ad, this.bd, e, f), h < 3 * m * b && (this.Ab++, this.ad = e, this.bd = f, this.Ab < this.wa.length && (e = this.wa[this.Ab].x, f = this.wa[this.Ab].y))) : (e = this.wa[this.wa.length - 1].x, f = this.wa[this.wa.length - 1].y);
            this.bh = Ia(this.ad, this.bd, e, f);
            h = Ma(l.x, l.y, this.ad, this.bd);
            h < g && this.Ab < this.wa.length && (this.Rh ? this.Rh = !1 : g = m * b, this.ad += Math.cos(this.bh) *
                g, this.bd += Math.sin(this.bh) * g);
            a = Ia(l.x, l.y, this.ad, this.bd);
            c = Ja(this.Pc, a);
            g = Ma(l.x, l.y, this.wa[this.wa.length - 1].x, this.wa[this.wa.length - 1].y);
            m = this.Xc * this.Xc / (2 * this.Jj);
            1 < h && (this.Pc = Ka(this.Pc, a, this.vh * b), .5 >= Ca(c) ? c = this.Xc : 120 <= Ca(c) || this.Ci && 0 === this.Ab ? (c = 0, this.Ci = !0) : (a = c / this.vh, h = Ma(l.x, l.y, this.ad, this.bd), h = h / (2 * Math.sin(c)), c = h * c / a, 0 > c && (c = 0), c > this.Xc && (c = this.Xc)), g < m && (c = Math.min(c, g / m * this.Xc + this.Xc / 40)), this.Aa += this.Ed * b, this.Aa > c && (this.Aa = c));
            l.x += Math.cos(this.Pc) *
                this.Aa * b;
            l.y += Math.sin(this.Pc) * this.Aa * b;
            this.Oi && (this.Ec = l.i = this.Pc);
            l.Q();
            this.Ab === this.wa.length && Ma(l.x, l.y, e, f) < Math.max(3 * this.Aa * b, 10) && (this.ug = this.Ag = !1, this.Aa = this.Ab = 0, this.b.trigger(jc.prototype.m.mp, l))
        }
    };
    m.aj = function() {
        this.enabled && (this.Ar(), this.Oj())
    };
    m.Oj = function() {
        this.gg && !this.ii && (this.gg = !1, this.pq(this.k.x, this.k.y, this.Lj, this.Mj))
    };
    m.tf = function() {
        var b = "" + this.Ba + "," + this.zc;
        l.hasOwnProperty(b) || (l[b] = {
            he: new window.Pathfinder,
            cells: null,
            dh: !1,
            Xe: []
        });
        return l[b]
    };
    m.Ar = function() {
        var b = this.tf();
        if (!b.he.isReady() || b.dh || b.Xe.length) {
            var a, c, h, e, f, l, g, m, p, r;
            if (!b.he.isReady() || b.dh) {
                a = [];
                e = a.length = this.Te;
                f = this.Df;
                for (c = 0; c < e; ++c)
                    for (a[c] = [], a[c].length = f, h = 0; h < f; ++h) a[c][h] = this.Vn(c, h);
                b.cells = a;
                b.he.init(this.Te, this.Df, a, this.Lh);
                b.dh = !1;
                S(b.Xe)
            } else if (b.Xe.length) {
                l = 0;
                for (g = b.Xe.length; l < g; ++l) {
                    a = b.Xe[l];
                    m = a[0];
                    p = a[1];
                    e = a[2];
                    c = a[3];
                    a = [];
                    e = e - m;
                    f = c - p;
                    a.length = e;
                    for (c = 0; c < e; ++c)
                        for (a[c] = [], a[c].length = f, h = 0; h < f; ++h) r = this.Vn(m + c, p + h), a[c][h] = r, b.cells[m +
                            c][p + h] = r;
                    b.he.updateRegion(m, p, e, f, a)
                }
                S(b.Xe)
            }
        }
    };
    m.Zl = function() {
        var b, a;
        b = 0;
        for (a = this.wa.length; b < a; b++) window.freeResultNode(this.wa[b]);
        S(this.wa)
    };
    m.fq = function() {
        var b = this.tf().he.pathList;
        this.Zl();
        var a, c, h, e;
        a = 0;
        for (c = b.length; a < c; a++) h = b[a], e = window.allocResultNode(), e.x = (h.x + .5) * this.Ba, e.y = (h.y + .5) * this.Ba, this.wa.push(e)
    };
    var r = [],
        p = new Q;
    m.Vn = function(b, a) {
        var c, h, e, f, l, g = 0;
        p.left = b * this.Ba - this.zc;
        p.top = a * this.Ba - this.zc;
        p.right = (b + 1) * this.Ba + this.zc;
        p.bottom = (a + 1) * this.Ba + this.zc;
        if (0 === this.Cs) {
            if (this.b.Bt(p)) return window.PF_OBSTACLE
        } else {
            this.b.Am(this.k.j, this.type.Ug, p, r);
            c = 0;
            for (h = r.length; c < h; ++c)
                if (this.b.jl(p, r[c])) return S(r), window.PF_OBSTACLE;
            r.length = 0
        }
        c = 0;
        for (h = this.type.Nd.length; c < h; c++) {
            e = this.type.Nd[c].Gn;
            l = this.type.Nd[c].am;
            this.b.Xh(this.k.j, e, p, r);
            e = 0;
            for (f = r.length; e < f; ++e) this.b.jl(p, r[e]) && (g += l);
            S(r)
        }
        return g
    };
    m.pq = function(b, a, c, h) {
        var e = this.tf().he;
        if (e.isReady()) {
            this.ei = !0;
            this.Wl = c;
            this.Xl = h;
            b = Math.floor(b / this.Ba);
            a = Math.floor(a / this.Ba);
            c = Math.floor(c / this.Ba);
            h = Math.floor(h / this.Ba);
            var f, l, g, m, p, r, k;
            if (e.at(c, h) === window.PF_OBSTACLE) {
                f = 1E6;
                for (m = g = l = 0; m < this.Te; m++)
                    for (p = 0; p < this.Df; p++) e.at(m, p) !== window.PF_OBSTACLE && (r = c - m, k = h - p, r = r * r + k * k, r < f && (f = r, l = m, g = p));
                c = l;
                h = g
            }
            e.findPath(b, a, c, h, this.Qs, this.Ps)
        }
    };
    c.prototype.xp = function() {
        return !0
    };
    c.prototype.sp = function() {
        return !0
    };
    c.prototype.mp = function() {
        return !0
    };
    h.m = new c;
    m.wh = function(b, a, c, h) {
        var e = Math.min(b, c) - this.zc,
            f = Math.min(a, h) - this.zc;
        b = Math.max(b, c) + this.zc;
        a = Math.max(a,
            h) + this.zc;
        e = Math.max(Math.floor(e / this.Ba), 0);
        f = Math.max(Math.floor(f / this.Ba), 0);
        b = Math.min(Math.ceil(b / this.Ba), this.Te);
        a = Math.min(Math.ceil(a / this.Ba), this.Df);
        e >= b || f >= a || this.tf().Xe.push([e, f, b, a])
    };
    h.aa = new function() {};
    h.Pa = new function() {}
})();

function kc(c) {
    this.b = c
}
(function() {
    var c = kc.prototype;
    c.qa = function(c) {
        this.behavior = c;
        this.b = c.b
    };
    c.qa.prototype.S = function() {};
    c.ca = function(c, h) {
        this.type = c;
        this.behavior = c.behavior;
        this.k = h;
        this.b = c.b
    };
    c = c.ca.prototype;
    c.S = function() {};
    c.cc = function() {
        this.k.Z();
        var c = this.k.da,
            h = this.k.j.Ca;
        (0 > c.right || 0 > c.bottom || c.left > h.width || c.top > h.height) && this.b.Cd(this.k)
    }
})();

function lc(c) {
    this.b = c;
    this.fl = this.Ui = this.ih = this.el = 0
}
(function() {
    var c = lc.prototype;
    c.qa = function(c) {
        this.behavior = c;
        this.b = c.b
    };
    c.qa.prototype.S = function() {};
    c.ca = function(c, l) {
        this.type = c;
        this.behavior = c.behavior;
        this.k = l;
        this.b = c.b
    };
    var l = c.ca.prototype;
    l.S = function() {
        this.enabled = 0 !== this.H[0]
    };
    l.rb = function() {
        return {
            smg: this.behavior.el,
            ss: this.behavior.ih,
            se: this.behavior.Ui,
            smd: this.behavior.fl
        }
    };
    l.Kb = function(c) {
        this.behavior.el = c.smg;
        this.behavior.ih = c.ss;
        this.behavior.Ui = c.se;
        this.behavior.fl = c.smd
    };
    l.cc = function() {};
    l.aj = function() {
        if (this.enabled) {
            var c =
                this.behavior.Di.zd(),
                l = 0,
                r = 0,
                p, b, a, d = 0;
            p = 0;
            for (b = c.length; p < b; p++) {
                a: {
                    a = c[p];
                    for (var u = void 0, e = void 0, f = void 0, u = 0, e = a.C.length; u < e; ++u)
                        if (f = a.C[u], f.behavior instanceof lc) {
                            a = f;
                            break a
                        }
                    a = null
                }
                a && a.enabled && (l += c[p].x, r += c[p].y, ++d)
            }
            c = this.k.j.Ca;
            b = this.b.Jb.Y;
            a = p = 0;
            b >= this.behavior.ih && b < this.behavior.Ui && (p = this.behavior.el * Math.min(this.b.gf, 1), 0 === this.behavior.fl && (p *= 1 - (b - this.behavior.ih) / (this.behavior.Ui - this.behavior.ih)), b = Math.random() * Math.PI * 2, a = Math.random() * p, p = Math.cos(b) * a, a *=
                Math.sin(b));
            c.bl(l / d + p);
            c.cl(r / d + a)
        }
    };
    c.aa = new function() {}
})();

function cc(c) {
    this.b = c
}(function() {
    var c = cc.prototype;
    c.qa = function(c) {
        this.behavior = c;
        this.b = c.b
    };
    c.qa.prototype.S = function() {};
    c.ca = function(c, l) {
        this.type = c;
        this.behavior = c.behavior;
        this.k = l;
        this.b = c.b
    };
    var l = c.ca.prototype;
    l.S = function() {
        this.k.u.solidEnabled = 0 !== this.H[0]
    };
    l.cc = function() {};
    c.m = new function() {};
    c.aa = new function() {}
})();

function bc() {
    return [ec, dc, fc, gc, Y, lc, hc, kc, cc, ic, jc, X.prototype.m.Uo, Y.prototype.aa.Hp, Y.prototype.Pa.Il, Y.prototype.Pa.Jl, Y.prototype.aa.gp, X.prototype.aa.Po, fc.prototype.aa.Fp, dc.prototype.m.cp, Y.prototype.aa.Mp, Y.prototype.m.op, Y.prototype.aa.Yo, Y.prototype.m.dp, Y.prototype.aa.Ep, X.prototype.aa.Ip, X.prototype.aa.Gp, X.prototype.m.Zo, X.prototype.aa.Xo, X.prototype.Pa.random, ec.prototype.m.nj, X.prototype.aa.Bp, dc.prototype.m.El, X.prototype.m.Bl, Y.prototype.aa.Dp, Y.prototype.aa.Cp, Y.prototype.m.Vo,
        Y.prototype.aa.Kp, X.prototype.Pa.ws, Y.prototype.m.Wo, Y.prototype.aa.Lp, X.prototype.Pa.vs
    ]
};